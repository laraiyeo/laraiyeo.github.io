  ...
  const intervalId = setInterval(async () => {
    try {
      ...
      const r = await pool.query(
        "SELECT id, user_id, betslip_data, status FROM betslips WHERE id = $1",
        [betslipId]
      );
      if (r.rows.length === 0) {
      ...
      for (const evId of Array.from(
        ...
        try {
          ...
          console.error(
            "[Watcher] Failed to fetch summary for",
            evId,
            e.message || e
          );
        }
      ...
      for (const bet of betsArr) {
        ...
        if (!summary) {
          ...
          if (
            ...
            !bet.prop &&
            (bet.type === "moneyline" ||
              (!bet.line && (bet.team || bet.selection || bet.description)))
          ) {
            const competitors =
          ...
          if (
            ...
            const lineNum =
              ...
              parseFloat(bet.line || bet.betValue || 0);
          ...
          if (newState === null && bet.line && bet.team) {
            const competitors =
              summary.header?.competitions?.[0]?.competitors || [];
            const betTeam = competitors.find(
              (c) => c.team?.abbreviation === bet.team
            );
            const opp = competitors.find(
              (c) => c.team?.abbreviation !== bet.team
            );
          ...
          if (newState === null && (bet.playerId || bet.player)) {
            ...
            const playerObj = players.find((pTeam) => {
              ...
              return athletes.find(
                (a) => a.athlete?.id == (bet.playerId || bet.player)
              );
            });
            ...
            if (playerObj) {
              ...
              if (athlete) {
                ...
                if (
                  String(raw).toLowerCase().startsWith("o") ||
                  String(raw).toLowerCase().startsWith("u")
                ) {
                  const isOver = String(raw).toLowerCase().startsWith("o");
                  const lineNum =
                  ...
                  let current = 0;
                  if (bet.statType) {
                    const idx = labels.indexOf(
                      String(bet.statType).toUpperCase()
                    );
                    current =
                      idx >= 0 ? parseFloat(athlete.stats?.[idx]) || 0 : 0;
                  } else {
                    const firstVal =
                      athlete.stats &&
                      athlete.stats.find((s) => !isNaN(parseFloat(s)));
        ...
        if (prev !== newState) {
          if (newState === "won") {
            try {
              ...
              console.error(
                "[Watcher] sendPushNotification error",
                e.message || e
              );
            }
          }
      ...
      if (allFinal) {
        ...
        if (fresh.status !== newStatus) {
          try {
            await pool.query("UPDATE betslips SET status = $1 WHERE id = $2", [
              newStatus,
              betslipId,
            ]);
            ...
            console.error(
              "[Watcher] Error updating betslip status",
              e.message || e
            );
          }
        }
      ...
      console.error(
        "[Watcher] interval error for",
        betslipId,
        tickErr.message || tickErr
      );
    }
  }, 5000);
...
router.delete("/:id/watch", authMiddleware, async (req, res) => {
  ...
  try {
    if (!betslipWatchers[betslipId])
      return res.json({ message: "Not watching", watching: false });
