...
(async () => {
  try {
    ...
    console.log('\nVerify token...');
    r = await axios.post(base + '/api/auth/verify', {}, { headers: { Authorization: 'Bearer ' + token } });
    ...
    r = await axios.post(base + '/api/profile/push-token', { pushToken: sampleExpo, platform: 'ios' }, { headers: { Authorization: 'Bearer ' + token } });
    console.log('Push-token upsert response:', r.data);

    console.log('\nGet betslips (should be empty)');
    r = await axios.get(base + '/api/betslips', { headers: { Authorization: 'Bearer ' + token } });
    console.log('Betslips count:', (r.data.betslips || []).length);

    console.log('\nPlacing a simple bet...');
    const betPayload = { betslipData: { bets: [] }, totalStake: 1, potentialPayout: 1.8 };
    r = await axios.post(base + '/api/betslips', betPayload, { headers: { Authorization: 'Bearer ' + token } });
    console.log('Place bet response:', r.data);

    const betslipId = r.data.betslipId;
    console.log('\nStart watcher for betslip via API');
    r = await axios.post(base + `/api/betslips/${betslipId}/watch`, {}, { headers: { Authorization: 'Bearer ' + token } });
