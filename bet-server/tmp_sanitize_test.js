...
function sanitizeDrive(lastDrive, dataCurrent) {
  ...
  driveOut.plays = plays.map((pl) => {
    const { id, sequenceNumber, awayScore, homeScore, scoringPlay, priority, modified, wallClock, teamParticipants, ...rest } = pl || {};
    const clean = rest || {};
  ...
  const currentTypeId = String((dataCurrent?.type?.id) || (lastDrive?.type?.id) || '');
  const firstAll = (driveOut.allStart && driveOut.allStart.length > 0 && driveOut.allStart[0]) || null;
  if (currentTypeId === '53' || currentTypeId === '52') {
    const directY = (dataCurrent?.start?.yardLine) ?? (lastDrive?.start?.yardLine) ?? null;
    if (directY != null) driveOut.start = { ...(driveOut.start || {}), yardLine: directY };
