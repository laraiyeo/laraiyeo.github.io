...
async function sendPushNotification(userId, title, body, data = {}) {
  try {
    ...
    let pushToken = null;
    try {
      const r = await pool.query("SELECT push_token FROM users WHERE id = $1", [userId]);
      if (r.rows.length > 0 && r.rows[0].push_token) pushToken = r.rows[0].push_token;
    } catch (e) {
    ...
    if (!pushToken) {
      try {
        const r2 = await pool.query("SELECT expo_push_token FROM push_tokens WHERE user_id = $1 ORDER BY updated_at DESC LIMIT 1", [userId]);
        if (r2.rows.length > 0 && r2.rows[0].expo_push_token) pushToken = r2.rows[0].expo_push_token;
      } catch (e) {
...
async function sendBetResultNotification(betslipId) {
  ...
  async function broadcastToAll(title, body, data = {}) {
    try {
      const r = await pool.query('SELECT expo_push_token FROM push_tokens WHERE expo_push_token IS NOT NULL');
      ...
      const messages = tokens.map((t) => ({ to: t, sound: 'default', title, body, data, priority: 'high' }));
      ...
      await pool.query('INSERT INTO push_notifications (title, body, data) VALUES ($1, $2, $3)', [title, body, JSON.stringify(data)]);
