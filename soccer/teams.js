...
const LEAGUE_COMPETITIONS = {
  ...
  "ksa.1": [{ code: "ksa.kings.cup", name: "Saudi King's Cup", logo: "2490" }],
};
...
function getCompetitionName(leagueCode) {
  ...
  const mainLeague = Object.values(LEAGUES).find(
  ...
  for (const [mainLeagueCode, competitions] of Object.entries(
    LEAGUE_COMPETITIONS,
  )) {
...
function isDomesticCup(leagueCode) {
  ...
  const isMainLeague = Object.values(LEAGUES).some(
    (league) => league.code === leagueCode,
  );
  return !isMainLeague;
}
...
async function fetchGamesFromAllCompetitions(tuesdayRange) {
  ...
  console.log(
    `Fetching games from ${allCompetitionsToCheck.length} competitions in parallel:`,
  ...
  const fetchPromises = allCompetitionsToCheck.map(async (competition) => {
    try {
      ...
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/${competition.code}/scoreboard?dates=${tuesdayRange}`,
      );
      ...
      if (response.ok) {
        ...
        console.log(
          `Failed to fetch from ${competition.code}: ${response.status}`,
        );
        return [];
      }
    } catch (error) {
  ...
  if (competitionGames.length > 0) {
    const competitions = [
      ...new Set(competitionGames.map((g) => g.competitionName)),
    ];
...
function selectBestGameForTeam(teamGames) {
  ...
  console.log(
    `Team has ${competitionGames.length} competition games and ${leagueGames.length} league games`,
  );

  // Get current/upcoming games (live or future)
  const activeCompetitionGames = competitionGames.filter(
    (game) =>
      game.status.type.state === "in" || game.status.type.state === "pre",
  );

  const activeLeagueGames = leagueGames.filter(
    (game) =>
      game.status.type.state === "in" || game.status.type.state === "pre",
  );
  ...
  if (allActiveGames.length > 0) {
    ...
    const sortedActiveGames = allActiveGames.sort((a, b) => {
      ...
      if (a.status.type.state === "in" && b.status.type.state !== "in")
        return -1;
      if (b.status.type.state === "in" && a.status.type.state !== "in")
        return 1;
    ...
    console.log(
      `Showing active game: ${sortedActiveGames[0].competitionName} (${sortedActiveGames[0].status.type.state}) on ${new Date(sortedActiveGames[0].date).toLocaleDateString()}`,
    );
  ...
  const allFinishedGames = teamGames.filter(
  ...
  if (allFinishedGames.length > 0) {
    ...
    console.log(
      `Showing most recent finished game: ${sortedFinishedGames[0].competitionName}`,
    );
...
async function fetchAndDisplayTeams() {
  try {
    ...
    const competitionCodes = allCompetitionsToCheck
      .map((c) => c.code)
      .sort()
      .join(",");
    ...
    if (dataCache.has(cacheKey)) {
      ...
      if (now - cached.timestamp < CACHE_DURATION) {
        console.log(
          ...
          currentLeague,
          "with competitions:",
          cached.competitions || competitionCodes,
        );
    ...
    if (container) {
      container.innerHTML =
        '<div style="text-align: center; padding: 20px; color: red;">Error loading teams. Please try again.</div>';
    }
  }
}
...
async function displayTeamsData(teams, games) {
  try {
    ...
    const competitionCodes = allCompetitionsToCheck
      ...
      .sort()
      .join(",");
    const competitionGameIds = competitionGames
      .map((g) => `${g.competitionCode}:${g.id}`)
      .sort()
      .join(",");
    const leagueGameIds = leagueGames
      .map((g) => g.id)
      .sort()
      .join(",");
    ...
    console.log(
      `Competition games: ${competitionGames.length}, League games: ${leagueGames.length}`,
    );

    // Log details about competition games for debugging
    if (competitionGames.length > 0) {
      console.log(
        `Competition games found:`,
        competitionGames.map(
          (g) =>
            `${g.competitionName}: ${g.competitions[0].competitors[0].team.displayName} vs ${g.competitions[0].competitors[1].team.displayName} (${new Date(g.date).toLocaleDateString()})`,
        ),
      );
    }
    ...
    for (const team of teams) {
      ...
      teamCard.style.backgroundColor = [
        "2950",
        "3243",
        "21964",
        "929",
...
function buildGameCard(game, team, data) {
  ...
  const leagueName = game.isDomesticCup
  ...
  const awayIsWinner =
    (awayShootoutScore || awayTeam.score) >
    (homeShootoutScore || homeTeam.score);
  const homeIsWinner =
    (homeShootoutScore || homeTeam.score) >
    (awayShootoutScore || awayTeam.score);
  ...
  const record =
    game?.competitions?.[0]?.competitors?.find((c) => c.homeAway === "home")
      ?.records?.[0]?.summary || "No record available";
...
function setupLeagueButtons() {
  ...
  for (const [leagueName, leagueData] of Object.entries(LEAGUES)) {
    ...
    button.addEventListener("click", async () => {
      ...
      // Update active state
      document
        .querySelectorAll(".league-button")
      ...
      if (container) {
        container.innerHTML =
          '<div style="text-align: center; padding: 20px; color: white;">Loading teams...</div>';
      }
  ...
  if (urlMode) {
    ...
    if (panel) {
      ...
      message.style.cssText =
        "background: #fff3cd; border: 1px solid #ffeaa7; padding: 10px; margin-bottom: 10px; border-radius: 4px; color: #856404; font-weight: bold; text-align: center;";
      message.textContent =
        "🔒 Styles are locked by OBS link parameters. Controls are disabled.";
