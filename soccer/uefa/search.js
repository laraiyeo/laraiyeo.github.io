...
function saveSearchState() {
  ...
  url.searchParams.set('league', currentUefaLeague);
  ...
  if (teamId && startDate && endDate && league) {
    loadTeams().then(() => {
      ...
      if (team) {
        ...
        currentUefaLeague = league;
        localStorage.setItem("currentUefaLeague", league);
        setupLeagueButtons();
        performSearch();
      }
    });
    ...
    try {
      ...
      if (state.teamId && state.matches && state.matches.length > 0) {
        ...
        if (team) {
          ...
          currentUefaLeague = state.league || currentUefaLeague;
          setupLeagueButtons();
          displayResults(allMatches, selectedTeam);
        }
      }
...
async function fetchMatchesForPeriod(teamId, startDate, endDate) {
  ...
  const API_URL = `https://site.api.espn.com/apis/site/v2/sports/soccer/${currentUefaLeague}/scoreboard?dates=${dateRange}`;
...
function createMatchCard(match, teamId) {
  ...
  const competitionName = (match.season?.slug || 'UEFA')
    .replace(/-/g, ' ')
    .toUpperCase();
