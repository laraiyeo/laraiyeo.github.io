              ...
              if (statsResponse) {
                ...
                if (generalStats && generalStats.stats) {
                  ...
                  stats = {
                    ...
                    time: statMap.behindTime || statMap.totalTime || 'N/A',
                    laps: statMap.lapsCompleted || 'N/A',
                    pits: statMap.pitsTaken || '0',
                    fastestLap: statMap.fastestLap || '-',
                    fastestLapNumber: statMap.fastestLapNum || '-',
                    q1Time: statMap.qual1TimeMS && statMap.qual1TimeMS !== '0.000' ? statMap.qual1TimeMS : null,
                    q2Time: statMap.qual2TimeMS && statMap.qual2TimeMS !== '0.000' ? statMap.qual2TimeMS : null,
                    q3Time: statMap.qual3TimeMS && statMap.qual3TimeMS !== '0.000' ? statMap.qual3TimeMS : null
                  };
                  
                  // Track max laps for race competitions
                  if ((competitionKey === 'r' || competitionName.toLowerCase().includes('race')) && 
                  ...
                  if (competitionKey === 'q' || competitionName.toLowerCase().includes('qualifying')) {
                    if (stats.q3Time) {
...
function renderCompetitionResults(competitions) {
  ...
  const firstKey = competitionKeys[0];
  
  // Create tab buttons
  const tabButtons = competitionKeys.map(key => {
    const comp = competitions[key];
    return `<button class="competition-tab ${key === firstKey ? 'active' : ''}" data-competition="${key}">
      ${comp.name}
    </button>`;
  }).join('');
  ...
  const tabContents = competitionKeys.map(key => {
    const comp = competitions[key];
    const isVisible = key === firstKey ? 'block' : 'none';
    
    // Determine table structure based on competition type
    let tableHeaders = '';
    let resultsRows = '';
    
    if (key === 'r' || comp.name.toLowerCase().includes('race')) {
    ...
    } else if (key === 'q' || comp.name.toLowerCase().includes('qualifying')) {
      // Qualifying: POS, Driver, Q1, Q2, Q3, Laps
      tableHeaders = `
        <th class="pos-header">POS</th>
        <th class="driver-header">DRIVER</th>
        <th class="q-header">Q1</th>
        <th class="q-header">Q2</th>
        <th class="q-header">Q3</th>
        <th class="laps-header">LAPS</th>
      `;
  ...
  if (countdown.status === "in-progress" || countdown.status === "finished") {
    ...
    if (raceId) {
      ...
      if (statsContainer) {
        ...
        if (competitions) {
          statsContainer.innerHTML = renderCompetitionResults(competitions);
          
          // Add event listeners for competition tabs
    ...
    // For upcoming races, hide the stats section
    const statsContainer = document.getElementById("raceStats");
    if (statsContainer) {
...
async function initializeRaceInfo() {
  try {
    ...
    await fetchRaceInfo(); // Updated to use new function
    renderRaceInfo();
  } catch (error) {
