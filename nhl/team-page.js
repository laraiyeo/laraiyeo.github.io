    ...
    searchInput.addEventListener('input', (e) => {
      ...
      searchTimeout = setTimeout(() => {
        ...
        if (filteredPlayers.length > 0) {
          ...
          searchResults.innerHTML = `<div style="padding: 10px; color: #777; text-align: center;">No ${selectionPositionText} found matching "${query}"</div>`;
          searchResults.style.display = 'block';
        }
      }, 300);
    });
...
async function showPlayerDetails(playerId, firstName, lastName, jerseyNumber, position, headshotUrl) {
  try {
    ...
    searchInput.addEventListener('input', async (e) => {
      ...
      searchTimeout = setTimeout(async () => {
        ...
        if (filteredPlayers.length > 0) {
          ...
          searchResults.innerHTML = `<div style="padding: 10px; color: #777; text-align: center;">No ${playerTypeText} found matching "${query}"</div>`;
          searchResults.style.display = 'block';
        }
      }, 300);
    });
    ...
    const result = await fetchAthleteStats('hockey', 'nhl', seasonYear, playerId);

    console.log('Player stats data:', result.data);

    if (result.data.splits && result.data.splits.categories) {
      displayPlayerStatsInModal(result.data.splits.categories, statsContainer, selectedPlayer.position);
    } else {
...
async function showOverallStats() {
  ...
  const result = await fetchAthleteStats('hockey', 'nhl', seasonYear, selectedPlayer.id);
...
async function showPlayerComparison(player1, player2) {
  try {
    ...
    const [player1Result, player2Result] = await Promise.all([
      fetchAthleteStats('hockey', 'nhl', seasonYear1, player1.id),
      fetchAthleteStats('hockey', 'nhl', seasonYear2, player2.id)
    ]);

    displayPlayerComparison(player1Result.data.splits?.categories, player2Result.data.splits?.categories, statsComparisonContainer, playersForComparison[0].position, playersForComparison[1].position);
...
function displayPlayerStatsInModal(categories, container, playerPosition = null) {
  ...
  const playerTeamColor = (currentTeam && currentTeam.color) ? `#${currentTeam.color}` : '#000000';
  const statsHTML = `
...
window.capturePlayerStatsAsImage = async function(element) {
  try {
    ...
    const teamCol = typeof teamColor !== 'undefined' ? teamColor : (currentTeam && currentTeam.color) ? `#${currentTeam.color}` : '#000000';
