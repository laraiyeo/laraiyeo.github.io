...
export const getLiveMatchData = async (matchId, basicMatchData) => {
  ...
  try {
    ...
    console.log(
      `[CS2MatchService] Live snapshot data:`,
      snapshotData ? "Available" : "Missing"
    );
    console.log(
      `[CS2MatchService] Game state data:`,
      gameStateData ? "Available" : "Missing"
    );
    ...
    console.error(
      `[CS2MatchService] Error fetching live data for match ${matchId}:`,
      error
    );
    throw error;
  }
...
export const getMatchDetails = async (
  ...
  if (ongoingCalls.has(callKey) && ongoingCalls.get(callKey) !== null) {
    console.log(
      "Duplicate call detected, waiting for ongoing call to complete..."
    );
  ...
  // Store a placeholder to prevent duplicates - we'll replace it with the actual promise
  ongoingCalls.set(callKey, null);

  try {
    // Create the promise
    const promise = CS2MatchService.getCachedData(
      cacheKey,
      async () => {
        try {
          ...
          if (!matchId || !team1Id || !team2Id) {
            throw new Error(
              `Missing required parameters: matchId=${matchId}, team1Id=${team1Id}, team2Id=${team2Id}`
            );
          }

          // Use match start date for filtering, fallback to current date if not provided
          const filterDate = matchStartDate
            ? new Date(matchStartDate).toISOString().split("T")[0]
            : "2025-10-12";
          ...
          const apiCalls = [
            ...
            fetch(
              `${CS2_API_BASE}/api/v1/matches?page[offset]=0&page[limit]=10&sort=-start_date&filter[matches.status][in]=finished&filter[matches.team_ids][contains]=${team1Id},${team2Id}&filter[matches.start_date][lt]=${filterDate}&filter[matches.start_date][gt]=${currentYear}-01-01&filter[matches.discipline_id][eq]=1&with=teams,tournament`,
              { headers }
            ),
            // 2. Recent matches for team1 (2.txt)
            fetch(
              `${CS2_API_BASE}/api/v1/matches?scope=show-match-team-last-maps&page[offset]=0&page[limit]=5&sort=-start_date&filter[matches.status][in]=finished&filter[matches.team_ids][overlap]=${team1Id}&filter[matches.start_date][lt]=${filterDate}&filter[matches.start_date][gt]=${currentYear}-01-01&filter[matches.discipline_id][eq]=1&with=teams,tournament,games`,
              { headers }
            ),
            // 2. Recent matches for team2 (2.txt with team2 ID)
            fetch(
              `${CS2_API_BASE}/api/v1/matches?scope=show-match-team-last-maps&page[offset]=0&page[limit]=5&sort=-start_date&filter[matches.status][in]=finished&filter[teams.team_ids][overlap]=${team2Id}&filter[matches.start_date][lt]=${filterDate}&filter[matches.start_date][gt]=${currentYear}-01-01&filter[matches.discipline_id][eq]=1&with=teams,tournament,games`,
              { headers }
            ),
            // 4. Team1 players (4.txt)
            fetch(
              `${CS2_API_BASE}/api/v1/players?scope=show-match-lineup&page[offset]=0&page[limit]=7&filter[team_id][eq]=${team1Id}`,
              { headers }
            ),
            // 4. Team2 players (4.txt with team2 ID)
            fetch(
              `${CS2_API_BASE}/api/v1/players?scope=show-match-lineup&page[offset]=0&page[limit]=7&filter[team_id][eq]=${team2Id}`,
              { headers }
            ),
            // 5. Game details for this specific match (5.txt)
            fetch(
              `${CS2_API_BASE}/api/v1/games?sort=number&filter[games.match_id][eq]=${matchId}&with=winner_team_clan,loser_team_clan,game_side_results,game_rounds`,
              { headers }
            ),
            // 6. Detailed match info using match slug (6.txt)
            matchSlug
              ? fetch(
                  `${CS2_API_BASE}/api/v1/matches/${matchSlug}?scope=show-match&stream_language=en&with=teams,tournament_deep,stage`,
                  { headers }
                )
              : null,
            ...
            team1Slug
              ? fetch(
                  `${CS2_API_BASE}/api/v1/teams/${team1Slug}/map_pool?scope=show-match-team-map-pool&filter[begin_at_from]=${currentYear}-01-01&filter[begin_at_to]=${filterDate}`,
                  { headers }
                )
              : null,
            ...
            team2Slug
              ? fetch(
                  `${CS2_API_BASE}/api/v1/teams/${team2Slug}/map_pool?scope=show-match-team-map-pool&filter[begin_at_from]=${currentYear}-01-01&filter[begin_at_to]=${filterDate}`,
                  { headers }
                )
              : null,
          ];
          ...
          const [
            ...
            team1MapPoolResponse
              ? team1MapPoolResponse.json().catch((err) => {
                  console.warn("Team1 map pool failed:", err);
                  return null;
                })
              : null,
            team2MapPoolResponse
              ? team2MapPoolResponse.json().catch((err) => {
                  console.warn("Team2 map pool failed:", err);
                  return null;
                })
              : null,
          ]);
          ...
          console.log(
            "Match Details (6.txt) live_updates:",
            matchDetailsData?.live_updates
          );
          console.log(
            "Match Details full object keys:",
            matchDetailsData
              ? Object.keys(matchDetailsData)
              : "no matchDetailsData"
          );
          if (matchDetailsData?.live_updates) {
            console.log(
              "Live updates game_number:",
              matchDetailsData.live_updates.game_number
            );
            console.log(
              "Live updates map_name:",
              matchDetailsData.live_updates.map_name
            );
            console.log("Live updates team scores:", {
              team1_game_score:
                matchDetailsData.live_updates.team_1?.game_score,
              team2_game_score:
                matchDetailsData.live_updates.team_2?.game_score,
              team1_match_score:
                matchDetailsData.live_updates.team_1?.match_score,
              team2_match_score:
                matchDetailsData.live_updates.team_2?.match_score,
            });
          }
          ...
          if (
            ...
            extractedTeam1Slug =
              extractedTeam1Slug || matchDetailsData.team1.slug;
            extractedTeam2Slug =
              extractedTeam2Slug || matchDetailsData.team2.slug;
          }

          // Fallback to recent matches data for team slugs
          if (
            !extractedTeam1Slug &&
            team1RecentMatchesData?.results?.length > 0
          ) {
            const team1Match = team1RecentMatchesData.results[0];
            const team1InMatch =
              team1Match.team1_id === team1Id
                ? team1Match.team1
                : team1Match.team2;
          ...
          if (
            !extractedTeam2Slug &&
            team2RecentMatchesData?.results?.length > 0
          ) {
            const team2Match = team2RecentMatchesData.results[0];
            const team2InMatch =
              team2Match.team1_id === team2Id
                ? team2Match.team1
                : team2Match.team2;
          ...
          if (
            (!extractedTeam1Slug || !extractedTeam2Slug) &&
            headToHeadData?.results?.length > 0
          ) {
            const h2hMatch = headToHeadData.results[0];
            if (!extractedTeam1Slug) {
              const team1InH2H =
                h2hMatch.team1_id === team1Id ? h2hMatch.team1 : h2hMatch.team2;
              extractedTeam1Slug = team1InH2H?.slug;
            }
            if (!extractedTeam2Slug) {
              const team2InH2H =
                h2hMatch.team1_id === team2Id ? h2hMatch.team1 : h2hMatch.team2;
    ...
    // Store the actual promise to prevent duplicates
...
export const getTeamRecentMatches = async (teamId, matchStartDate) => {
  ...
  return CS2MatchService.getCachedData(
    cacheKey,
    async () => {
      try {
        const filterDate = matchStartDate
        ...
        const response = await fetch(
          `${CS2_API_BASE}/api/v1/matches?scope=show-match-team-last-maps&page[offset]=0&page[limit]=5&sort=-start_date&filter[matches.status][in]=finished&filter[matches.team_ids][overlap]=${teamId}&filter[matches.start_date][lt]=${filterDate}&filter[matches.start_date][gt]=${currentYear}-01-01&filter[matches.discipline_id][eq]=1&with=teams,tournament,games`,
          { headers }
        );
...
export const getTeamMapPool = async (teamSlug, matchStartDate) => {
  try {
    ...
    const currentYear = new Date().getFullYear();
    const response = await fetch(
      `${CS2_API_BASE}/api/v1/teams/${teamSlug}/map_pool?scope=show-match-team-map-pool&filter[begin_at_from]=${currentYear}-01-01&filter[begin_at_to]=${filterDate}`
    );
    const data = await response.json();
  ...
  return CS2MatchService.getCachedData(
    ...
    async () => {
      try {
        const headers = CS2MatchService.getBrowserHeaders();
        const response = await fetch(
          `${CS2_API_BASE}/api/v1/players?scope=show-match-lineup&page[offset]=0&page[limit]=7&filter[team_id][eq]=${teamId}`,
          { headers }
        );
        const data = await response.json();
...
export const formatCompleteMatchData = (combinedData) => {
  ...
  if (matchDetails && matchDetails.team1 && matchDetails.team2) {
    ...
    team1Data = {
      id:
        firstGame.winner_team_clan?.team?.id ||
        firstGame.loser_team_clan?.team?.id,
      name:
        firstGame.winner_team_clan?.clan_name ||
        firstGame.loser_team_clan?.clan_name,
      shortName:
        firstGame.winner_team_clan?.clan_name ||
        firstGame.loser_team_clan?.clan_name,
      logoUrl:
        firstGame.winner_team_clan?.team?.image_url ||
        firstGame.loser_team_clan?.team?.image_url,
    };
    team2Data = {
      id:
        firstGame.loser_team_clan?.team?.id ||
  ...
  return {
    id: matchId,
    eventName:
      matchDetails?.stage?.title ||
      matchDetails?.tournament?.name ||
      team1RecentMatches?.results?.[0]?.tournament?.name ||
      team2RecentMatches?.results?.[0]?.tournament?.name ||
      "CS2 Match",
    tournamentId: matchDetails?.tournament?.id || null,
    tournamentSlug: matchDetails?.tournament?.slug || null,
    team1: team1Data,
    team2: team2Data,
    ...
    pickban: matchDetails?.match_maps
      ? formatPickBanData(matchDetails.match_maps, team1Data, team2Data)
      : [],
...
export const formatMatchData = (rawData) => {
  ...
  return {
    ...
    maps: games.map((game) => ({
      ...
      name: game.map_name,
      displayName: getMapDisplayName(game.map_name),
      team1Score: getTeamScoreForGame(
        game,
        firstGame.winner_team_clan?.team?.id
      ),
      team2Score: getTeamScoreForGame(
        game,
        firstGame.loser_team_clan?.team?.id
      ),
      winner: game.winner_team_clan?.team?.id,
...
const calculateTeamScore = (games, teamId) => {
  if (!teamId) return 0;
  return games.filter((game) => game.winner_team_clan?.team?.id === teamId)
    .length;
};
...
export const formatSeriesData = (matchData) => {
  ...
  return {
    ...
    eventName:
      ...
      matchData.tournament?.nameShortened ||
      "CS2 Match",
    team1: {
  ...
  try {
    ...
    const [playerStatsResponse, gameDetailsResponse] = await Promise.all([
      ...
      seriesSlug && mapName
        ...
        : fetch(
            `${CS2_API_BASE}/api/v1/games/${gameId}?with=winner_team_clan,loser_team_clan,game_side_results,game_rounds`
          ),
    ]);
    ...
    if (
      ...
      (Array.isArray(playerStatsData) && playerStatsData.length === 0)
    ) {
      console.log(
        "⚠️ Player stats empty, fetching short_players_stats fallback"
      );
      try {
        const shortStatsResponse = await fetch(
          `${CS2_API_BASE}/api/v1/games/${gameId}/short_players_stats`
        );
        const shortStatsData = await shortStatsResponse.json();
        playerStatsData = shortStatsData;
        useShortStats = true;
        console.log(
          "✅ Successfully fetched short_players_stats:",
          shortStatsData
        );
      } catch (error) {
    ...
    console.log(
      ...
      JSON.stringify(gameDetailsData, null, 2)
    );
    ...
    async () => {
      try {
        ...
        const [hitGroupStatsResponse, killsMatrixResponse] = await Promise.all([
          fetch(
            `${CS2_API_BASE}/api/v1/games/${gameId}/rounds/${roundNumber}/hit_group_stats`,
            { headers }
          ),
          fetch(
            `${CS2_API_BASE}/api/v1/games/${gameId}/rounds/${roundNumber}/kills_matrix`,
            { headers }
          ),
        ]);
...
export const getWeaponStats = async (gameId) => {
  try {
    ...
    const response = await fetch(
      `${CS2_API_BASE}/api/v1/games/${gameId}/weapons_stats`
    );
    const data = await response.json();
...
export const getHitGroupStats = async (gameId) => {
  try {
    ...
    const headers = CS2MatchService.getBrowserHeaders();
    const response = await fetch(
      `${CS2_API_BASE}/api/v1/games/${gameId}/hit_group_stats`,
      { headers }
    );
    const data = await response.json();
