...
class NBADataService {
  ...
  async initializeData() {
    ...
    if (this.teamsCache && this.playersCache) {
      console.log('NBADataService: Data already cached, returning existing data');
      return this.getData();
    }

    // If already initializing, wait for that to complete
    if (this.isInitializing && this.initPromise) {
      console.log('NBADataService: Already initializing, waiting for completion');
      return await this.initPromise;
    }

    console.log('NBADataService: Starting data initialization...');
    ...
    try {
      ...
      const rosterPromises = teams.map(async (team) => {
        try {
          ...
          if (roster.athletes) {
            // NBA roster structure is a flat array, not grouped by position
      ...
      console.log(`NBA Data Service: Loaded ${teams.length} teams and ${allPlayers.length} players`);
  ...
  // Check if two players can be compared - NBA allows all positions to compare
