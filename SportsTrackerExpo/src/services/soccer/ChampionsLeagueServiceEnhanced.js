...
const CHAMPIONS_LEAGUE_BASE_URL =
...
export const ChampionsLeagueServiceEnhanced = {
  ...
  hasLiveEvents(games) {
    try {
      ...
      console.error(
        "ChampionsLeagueService: Error detecting live events",
        error,
      );
      return false;
    }
  ...
  getDataType(data, context) {
    try {
      ...
      console.error(
        "ChampionsLeagueService: Error determining data type",
        error,
      );
      return "scheduled";
    }
  ...
  async fetchGamesFromAllCompetitions(dateRange) {
    ...
    console.log(
      `Fetching Champions League games from ${allCompetitionsToCheck.length} competitions:`,
        ...
        if (response.ok) {
          ...
          competitionGames.forEach((game) => {
            ...
            game.competitionName =
              CHAMPIONS_LEAGUE_COMPETITIONS[competition.code]?.name ||
              competition.name;
  ...
  async getGameDetails(gameId, competitionHint = null) {
    try {
      ...
      if (effectiveHint) {
        ...
        const normalized = Object.keys(CHAMPIONS_LEAGUE_COMPETITIONS).find(
          (k) =>
            k === effectiveHint ||
            CHAMPIONS_LEAGUE_COMPETITIONS[k].name.toLowerCase() ===
        ...
        try {
          ...
          if (response.ok) {
            ...
            data.competitionName =
              CHAMPIONS_LEAGUE_COMPETITIONS[competition].name;
            return data;
          }
        } catch (err) {
      ...
      throw new Error(
        `Game ${gameId} not found in any Champions League competition`,
      );
    } catch (error) {
  ...
  async getStandings() {
    try {
      ...
      const response = await fetch(
        `https://cdn.espn.com/core/soccer/table?xhr=1&league=${leagueCode}&season=${year}`,
      );
      ...
      if (!hasValidStructure) {
        ...
        console.log(
          `UCL standings validation passed - found ${standingsData.content.standings.groups[0].standings.entries.length} entries`,
        );
      }
  ...
  async getTeam(teamId) {
    try {
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/teams/${teamId}`,
      );
      const data = await response.json();
      return data;
    } catch (error) {
  ...
  async searchTeams(query) {
    try {
      const response = await fetch(
        `${CHAMPIONS_LEAGUE_BASE_URL}/teams?limit=50`,
      );
      const data = await response.json();
      ...
      const teamPromises = teams.map(async (team) => {
        try {
          ...
          if (this.rosterCache.has(rosterCacheKey)) {
            ...
            const response = await fetch(
              `${CHAMPIONS_LEAGUE_BASE_URL}/teams/${teamId}/roster?season=${year}`,
            );
