...
export const SpainServiceEnhanced = {
  ...
  async fetchGamesFromAllCompetitions(dateRange) {
    ...
    console.log(
      `Fetching Spain games from ${allCompetitionsToCheck.length} competitions:`,
        ...
        if (response.ok) {
          ...
          competitionGames.forEach((game) => {
            ...
            game.competitionName =
              SPAIN_COMPETITIONS[competition.code]?.name || competition.name;
  ...
  async getGameDetails(gameId, competitionHint = null) {
    try {
      ...
      if (effectiveHint) {
        ...
        const normalized = Object.keys(SPAIN_COMPETITIONS).find(
          (k) =>
            k === effectiveHint ||
            SPAIN_COMPETITIONS[k].name.toLowerCase() ===
  ...
  async getTeam(teamId) {
    try {
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/teams/${teamId}`,
      );
      const data = await response.json();
      return data;
    } catch (error) {
    ...
    try {
      ...
      const teamPromises = teams.map(async (team) => {
        try {
          ...
          // Use cached roster when available to avoid re-fetching on each search
          if (this.rosterCache.has(rosterCacheKey)) {
            rosterData = this.rosterCache.get(rosterCacheKey);
          } else {
            const response = await fetch(
              `${SPAIN_BASE_URL}/teams/${teamId}/roster?season=${year}`,
            );
            rosterData = await response.json();
            // Cache successful roster responses (even empty arrays) to reduce load
            try {
