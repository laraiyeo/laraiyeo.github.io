...
export const EnglandServiceEnhanced = {
  ...
  async fetchGamesFromAllCompetitions(dateRange) {
    ...
    console.log(
      `Fetching England games from ${allCompetitionsToCheck.length} competitions:`,
        ...
        if (response.ok) {
          ...
          competitionGames.forEach((game) => {
            ...
            game.competitionName =
              ENGLAND_COMPETITIONS[competition.code]?.name || competition.name;
  ...
  async getScoreboard(dateFilter = "today") {
    ...
    return this.getCachedData(
      ...
      async () => {
        ...
        console.log(
          `Fetching England scoreboard for ${dateFilter}:`,
          dateRange,
        );
      ...
      async () => {
        ...
        if (!competitionHint) {
          try {
            ...
            console.log(
              "Could not fetch core event resource for hint:",
              coreErr,
            );
          }
        }
        ...
        if (effectiveHint) {
          ...
          const normalized = Object.keys(ENGLAND_COMPETITIONS).find(
            (k) =>
              k === effectiveHint ||
              ENGLAND_COMPETITIONS[k].name.toLowerCase() ===
        ...
        try {
          ...
          if (this.rosterCache.has(rosterCacheKey)) {
            ...
            const rosterResponse = await fetch(
              `${ENGLAND_BASE_URL}/teams/${teamId}/roster?season=${year}`,
            );
