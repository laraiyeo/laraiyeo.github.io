// Enhanced England Soccer Service
...
const ENGLAND_COMPETITIONS = {
  'eng.1': { name: 'Premier League', logo: '23', isPrimary: true },
  'eng.fa': { name: 'FA Cup', logo: '40', isPrimary: false },
  'eng.league_cup': { name: 'EFL Cup', logo: '41', isPrimary: false }
};
...
export const EnglandServiceEnhanced = {
  ...
  async fetchGamesFromAllCompetitions(dateRange) {
    ...
    // Get competitions for England
    const allCompetitionsToCheck = [
      { code: 'eng.fa', name: 'FA Cup' }, // Domestic cups FIRST (prioritized)
      { code: 'eng.league_cup', name: 'EFL Cup' },
      { code: 'eng.1', name: 'Premier League' } // Main league LAST
    ];
    
    console.log(`Fetching England games from ${allCompetitionsToCheck.length} competitions:`, allCompetitionsToCheck.map(c => c.code));
      ...
      try {
        ...
        if (response.ok) {
          ...
          competitionGames.forEach(game => {
            ...
            game.competitionName = ENGLAND_COMPETITIONS[competition.code]?.name || competition.name;
            game.isDomesticCup = competition.code !== 'eng.1';
            game.priority = competition.code !== 'eng.1' ? 1 : 2; // Competition = 1, League = 2
    ...
    console.log(`Total England games found: ${allGames.length}`);
    return allGames;
  },
  ...
  async getScoreboard(dateFilter = 'today') {
    ...
    return this.getCachedData(cacheKey, async () => {
      ...
      console.log(`Fetching England scoreboard for ${dateFilter}:`, dateRange);
  ...
  async getGameDetails(gameId, competitionHint = null) {
    ...
    return this.getCachedData(cacheKey, async () => {
          ...
          for (const comp of ['eng.1', 'eng.fa', 'eng.league_cup']) {
            ...
            if (coreResponse.ok) {
              ...
              if (seasonRef && typeof seasonRef === 'string') {
                // seasonRef example: http://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.fa/seasons/2024?lang=en&region=us
      ...
      let competitionOrder = ['eng.fa', 'eng.league_cup', 'eng.1'];
      const effectiveHint = competitionHint || detectedHint;
      if (effectiveHint) {
        // Normalize hint to a key if it matches one of our known codes
        const normalized = Object.keys(ENGLAND_COMPETITIONS).find(k => k === effectiveHint || ENGLAND_COMPETITIONS[k].name.toLowerCase() === String(effectiveHint).toLowerCase() || k === String(effectiveHint));
        if (normalized) {
      ...
      for (const competition of competitionOrder) {
        try {
          ...
          if (response.ok) {
            ...
            // to avoid getting Spanish/foreign competition names in England games
            data.competitionCode = competition;
            data.competitionName = ENGLAND_COMPETITIONS[competition].name;
            return data;
          }
        } catch (err) {
  ...
  async searchPlayers(query) {
    try {
      // Get all teams first
      const response = await fetch(`${ENGLAND_BASE_URL}/teams`);
      const data = await response.json();
      ...
      const teamPromises = teams.map(async (team) => {
        try {
          ...
          const rosterResponse = await fetch(`${ENGLAND_BASE_URL}/teams/${teamId}/roster?season=${getSoccerYear()}`);
