...
const FRANCE_BASE_URL =
  "https://site.api.espn.com/apis/site/v2/sports/soccer/fra.1";
...
export const FranceServiceEnhanced = {
  ...
  async fetchGamesFromAllCompetitions(dateRange) {
    ...
    console.log(
      `Fetching France games from ${allCompetitionsToCheck.length} competitions:`,
        ...
        if (response.ok) {
          ...
          competitionGames.forEach((game) => {
            ...
            game.competitionName =
              FRANCE_COMPETITIONS[competition.code]?.name || competition.name;
  ...
  async getScoreboard(dateFilter = 'today') {
    ...
    const cacheKey = `france_scoreboard_${dateFilter}`;
  ...
  async getGameDetails(gameId, competitionHint = null) {
    try {
      ...
      if (effectiveHint) {
        ...
        const normalized = Object.keys(FRANCE_COMPETITIONS).find(
          (k) =>
            k === effectiveHint ||
            FRANCE_COMPETITIONS[k].name.toLowerCase() ===
  ...
  async getTeam(teamId) {
    try {
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/fra.1/teams/${teamId}`,
      );
      const data = await response.json();
      return data;
    } catch (error) {
        ...
        try {
          ...
          if (this.rosterCache.has(rosterCacheKey)) {
            ...
            const rosterResponse = await fetch(
              `${FRANCE_BASE_URL}/teams/${teamId}/roster?season=${year}`,
            );
