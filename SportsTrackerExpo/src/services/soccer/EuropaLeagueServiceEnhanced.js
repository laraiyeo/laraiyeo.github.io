...
export const EuropaLeagueServiceEnhanced = {
  ...
  async fetchGamesFromAllCompetitions(dateRange) {
    ...
    console.log(
      `Fetching Europa League games from ${allCompetitionsToCheck.length} competitions:`,
        ...
        if (response.ok) {
          ...
          competitionGames.forEach((game) => {
            ...
            game.competitionName =
              EUROPA_LEAGUE_COMPETITIONS[competition.code]?.name ||
              competition.name;
  ...
  async getGameDetails(gameId, competitionHint = null) {
    try {
      ...
      if (effectiveHint) {
        ...
        const normalized = Object.keys(EUROPA_LEAGUE_COMPETITIONS).find(
          (k) =>
            k === effectiveHint ||
            EUROPA_LEAGUE_COMPETITIONS[k].name.toLowerCase() ===
      ...
      throw new Error(
        `Game ${gameId} not found in any Europa League competition`,
      );
    } catch (error) {
  ...
  async getTeam(teamId) {
    try {
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/teams/${teamId}`,
      );
      const data = await response.json();
      return data;
    } catch (error) {
        ...
        try {
          ...
          if (this.rosterCache.has(rosterCacheKey)) {
            ...
            const rosterResponse = await fetch(
              `${EUROPA_LEAGUE_BASE_URL}/teams/${teamId}/roster?season=${year}`,
            );
