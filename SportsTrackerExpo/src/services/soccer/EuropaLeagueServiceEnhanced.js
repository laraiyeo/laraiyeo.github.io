// Enhanced Europa League Soccer Service
...
const EUROPA_LEAGUE_COMPETITIONS = {
  'uefa.europa': { name: 'Europa League', logo: '2310', isPrimary: true },
  'uefa.europa_qual': { name: 'Europa League Qualifying', logo: '2310', isPrimary: false }
};
...
export const EuropaLeagueServiceEnhanced = {
  ...
  // Fetch games from all Europa League competitions
  async fetchGamesFromAllCompetitions(dateRange) {
    const allGames = [];
    
    // Get competitions for Europa League
    const allCompetitionsToCheck = [
      { code: 'uefa.europa_qual', name: 'Europa League Qualifying' }, // Qualifying FIRST (prioritized)
      { code: 'uefa.europa', name: 'Europa League' } // Main competition LAST
    ];
    
    console.log(`Fetching Europa League games from ${allCompetitionsToCheck.length} competitions:`, allCompetitionsToCheck.map(c => c.code));
      ...
      try {
        ...
        if (response.ok) {
          ...
          competitionGames.forEach(game => {
            ...
            game.competitionName = EUROPA_LEAGUE_COMPETITIONS[competition.code]?.name || competition.name;
            game.isQualifying = competition.code === 'uefa.europa_qual';
            game.priority = competition.code === 'uefa.europa_qual' ? 1 : 2; // Qualifying = 1, Main = 2
    ...
    console.log(`Total Europa League games found: ${allGames.length}`);
    return allGames;
  },
  ...
  async getGameDetails(gameId, competitionHint = null) {
    try {
      ...
      // (e.g. uefa.europa). Using that is the most reliable way to determine
      // the actual competition for the event.
      let detectedHint = null;
      if (!competitionHint) {
        try {
          // Try each Europa League competition to find the right one
          for (const comp of ['uefa.europa', 'uefa.europa_qual']) {
            ...
            if (coreResponse.ok) {
              ...
              if (seasonRef && typeof seasonRef === 'string') {
                // seasonRef example: http://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.europa/seasons/2024?lang=en&region=us
      ...
      let competitionOrder = ['uefa.europa_qual', 'uefa.europa'];
      const effectiveHint = competitionHint || detectedHint;
      if (effectiveHint) {
        // Normalize hint to a key if it matches one of our known codes
        const normalized = Object.keys(EUROPA_LEAGUE_COMPETITIONS).find(k => k === effectiveHint || EUROPA_LEAGUE_COMPETITIONS[k].name.toLowerCase() === String(effectiveHint).toLowerCase() || k === String(effectiveHint));
        if (normalized) {
      ...
      for (const competition of competitionOrder) {
        try {
          ...
          if (response.ok) {
            const data = await response.json();
            // For Europa League competitions, always use our mapping instead of API-provided names
            // to avoid getting other language competition names in Europa League games
            data.competitionCode = competition;
            data.competitionName = EUROPA_LEAGUE_COMPETITIONS[competition].name;
            return data;
          }
        } catch (err) {
      ...
      throw new Error(`Game ${gameId} not found in any Europa League competition`);
    } catch (error) {
      console.error('Error fetching Europa League game details:', error);
      throw error;
    }
  },
  ...
  async getStandings() {
    try {
      ...
      const leagueCode = 'uefa.europa'; // Europa League
  ...
  async getTeam(teamId) {
    try {
      const response = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/teams/${teamId}`);
      const data = await response.json();
      return data;
    } catch (error) {
  ...
  async searchPlayers(query) {
    try {
      // Get all teams first
      const response = await fetch(`${EUROPA_LEAGUE_BASE_URL}/teams`);
      const data = await response.json();
  ...
  getCompetitionInfo() {
    return {
      leagues: EUROPA_LEAGUE_COMPETITIONS,
      apiCode: 'uefa.europa'
    };
  },
