...
export class BackendMLBService {
  static BACKEND_URL = "https://sportsheart-baseball.up.railway.app";
  ...
  static async initialize() {
    try {
      ...
      if (healthStatus) {
        console.log(
          "BackendMLBService: Backend is active, using delta updates",
        );
      } else {
        console.log(
          "BackendMLBService: Backend unavailable, will use fallback",
        );
      }
      ...
      return {
        ...
        message: healthStatus
          ? "Backend ready for delta updates"
          : "Using fallback mode",
      };
    } catch (error) {
  ...
  static async getGames(options = {}) {
    try {
      ...
      try {
        const result = await this.getGamesFromBackend(
          startDate,
          endDate,
          forceRefresh,
        );
        ...
        console.error(
          "BackendMLBService: Backend games request failed:",
          error,
        );
        this.isBackendActive = false;
      }
  ...
  static async getGamesFromBackend(startDate, endDate, forceRefresh = false) {
    try {
      ...
      let lastSync = null;
      if (!forceRefresh) {
        lastSync = await AsyncStorage.getItem(
          `${this.GAMES_SYNC_KEY}_${cacheKey}`,
        );
      }
      ...
      if (data.hasChanges && data.data && data.data.events) {
        ...
        await AsyncStorage.setItem(
          `${this.GAMES_CACHE_KEY}_${cacheKey}`,
  ...
  static async getStandings(forceRefresh = false) {
    try {
      if (!this.isBackendActive) {
        console.log(
          "BackendMLBService: Backend inactive, using fallback for standings",
        );
      ...
      try {
        ...
        if (result && result.data && result.data.records) {
          console.log(
            "BackendMLBService: Returning standings with",
        ...
        console.error(
          "BackendMLBService: Backend standings request failed:",
          error,
        );
        this.isBackendActive = false;
      }
      ...
      // Fallback to direct API
      console.log(
        "BackendMLBService: Falling back to direct MLB API for standings",
      );
      return await MLBService.getStandings();
    } catch (error) {
    ...
    try {
      ...
      if (data.hasChanges && data.data) {
        ...
        if (cachedData) {
          ...
          console.log(
            "BackendMLBService: Using cached standings data, records:",
