...
export class NBAService {
  ...
  static STANDINGS_API_URL = "https://site.api.espn.com/apis/site/v2/sports/basketball/nba/standings";
  ...
  static async getStandings() {
    ...
    return this.getCachedData(cacheKey, async () => {
      const res = await fetch(this.STANDINGS_API_URL);
      const data = await res.json();
      return data;
    });
  }
      ...
      conferences.forEach(conference => {
        ...
        if (conference.children && conference.children.length > 0) {
          ...
          conference.children.forEach(division => {
            ...
            formatted[confName][divName] = (division.standings?.entries || []).map(entry => ({
              team: {
                id: entry.team?.id,
          ...
          // No divisions, direct standings
          formatted[confName]['teams'] = (conference.standings?.entries || []).map(entry => ({
            team: {
              id: entry.team?.id,
              displayName: entry.team?.displayName || '',
              abbreviation: entry.team?.abbreviation || '',
