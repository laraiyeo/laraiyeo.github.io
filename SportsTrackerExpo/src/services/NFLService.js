...
export class NFLService extends BaseCacheService {
  ...
  static async getScoreboard(startDate = null, endDate = null) {
    const cacheKey = `nfl_scoreboard_${startDate || "today"}_${
  ...
  static getPositionStatsForCard(
    positionGroup,
    boxScoreData,
    playerName,
    ...
    return this.getCachedData(cacheKey, async () => {
      ...
      for (let i = 0; i < drives.length; i += batchSize) {
        ...
        const batchResults = await Promise.all(
          batch.map(async (drive, driveIndex) => {
            try {
              ...
              const shouldFetchPlays =
                !isLiveUpdate || driveIndexFromEnd < recentDriveCount;
      ...
      const tryFetchStats = async (year, type) => {
        ...
        if (response.ok) {
          ...
          if (
            data.error &&
            data.error.code === 404 &&
            data.error.message === "No stats found."
          ) {
            return null;
          } else if (
            data.splits &&
            data.splits.categories &&
            data.splits.categories.length > 0
          ) {
            return data;
          }
  ...
  static async getGameSituation(gameId, gameDate = null) {
    try {
      ...
      if (event && event.competitions && event.competitions[0]) {
        return this.extractGameSituation({
          ...event,
  ...
  static extractGameSituation(selectedGame) {
    ...
    if (situation) {
      ...
      let ballPosition =
        situation.yardLine !== undefined && situation.yardLine !== null
          ? situation.yardLine
          : null;
