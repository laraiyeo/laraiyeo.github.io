...
export const ThemeProvider = ({ children }) => {
  ...
  const loadThemePreferences = async () => {
    try {
      ...
      const savedCustom = await AsyncStorage.getItem("customPalette");
      ...
      if (savedCustom !== null) {
        try {
          ...
          setCustomPaletteState({ ...customPalette, ...parsed });
        } catch (e) {
          console.warn("Failed parsing saved custom palette", e);
        }
      }
  ...
  const updateCustomPalette = async (newPalette) => {
    try {
      const merged = { ...customPalette, ...newPalette };
      setCustomPaletteState(merged);
      await AsyncStorage.setItem("customPalette", JSON.stringify(merged));
      // Select the custom palette after creating/updating it
      setCurrentColorPalette("custom");
      await AsyncStorage.setItem("colorPalette", "custom");

      // Update app icon to use the custom icon name
      await AppIconService.changeAppIcon(isDarkMode, "custom");

      // Per current request: set splash image/video to red for now
      try {
        await AsyncStorage.setItem("splashImage", "red");
        await AsyncStorage.setItem("splashVideo", "red");
      } catch (e) {
        // non-fatal
      }
    } catch (error) {
  ...
  const colorPalettes = { ...basePalettes, custom: customPalette };
  const colors = colorPalettes[currentColorPalette] || colorPalettes.red;
  ...
  const getTeamLogoUrl = (sport, teamId, size = "500") => {
    ...
    if (incomingSport === "mlb") {
      ...
      normalizedTeamId =
        abbreviationMap[teamId?.toUpperCase()] || teamId?.toLowerCase();
    } else {
