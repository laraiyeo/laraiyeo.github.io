...
function buildLiveTrackerUrl(uuid, profile = DEFAULT_PROFILE, lang = 'en', mode = '3d', sport = 'football') {
  if (!uuid) return null;
...
function buildWrapperUrl(uuid, wrapperBase = 'https://sportsheart.ca/widgets/livetracker.html', profile = DEFAULT_PROFILE, lang = 'en', mode = '3d') {
  if (!uuid) return wrapperBase;
  const u = encodeURIComponent(uuid);
  const p = encodeURIComponent(profile);
  return `${wrapperBase}?id=${u}&profile=${p}&lang=${encodeURIComponent(lang)}&mode=${encodeURIComponent(mode)}`;
}
