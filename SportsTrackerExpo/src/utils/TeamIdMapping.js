...
export const ESPN_TO_MLB_ID_MAPPING = Object.fromEntries(
  Object.entries(MLB_TO_ESPN_ID_MAPPING).map(([mlbId, espnId]) => [
    espnId,
    mlbId,
  ])
);
...
export const normalizeTeamIdForStorage = (teamId, sport) => {
  ...
  // Normalize sport string first so checks are case-insensitive and alias-friendly
  const normalizedSport = sportMapNormalize(sport);

  // For MLB teams, convert MLB ID to ESPN ID if it's a MLB ID
  if (normalizedSport === "mlb" && isMLBId(teamIdString)) {
    const espnId = convertMLBIdToESPNId(teamIdString);
    console.log(
      `[TEAM ID MAPPING] Normalizing MLB ID ${teamIdString} -> ESPN ID ${espnId} for storage`
    );
    // Append sport suffix for storage
    return addSportSuffix(espnId || teamIdString, normalizedSport);
  }

  // For other sports, return as-is (already ESPN IDs) with normalized sport suffix
  return addSportSuffix(teamIdString, normalizedSport);
};
...
export const getAPITeamId = (teamId, sport) => {
  ...
  if (effectiveSport === "mlb") {
    ...
    if (isESPNMLBId(baseId)) {
      ...
      console.log(
        `[TEAM ID MAPPING] Converting ESPN ID ${baseId} -> MLB ID ${mlbId} for API call`
      );
      return mlbId || baseId;
    }
    return baseId;
  ...
  if (
    (effectiveSport && effectiveSport.includes("soccer")) ||
    effectiveSport.includes("uefa") ||
    effectiveSport.includes("champions") ||
    effectiveSport.includes("europa")
  ) {
    const m = String(baseId).match(/^(\d+)/);
    if (m) {
      console.log(
        `[TEAM ID MAPPING] Fallback: extracted numeric API id ${m[1]} from ${baseId}`
      );
      return m[1];
    }
  }
...
export const migrateFavoritesToESPNIds = (favorites) => {
  ...
  return favorites.map((favorite) => {
    ...
    if (normalizedId !== teamId || favorite.sport == null) {
      console.log(
        `[TEAM ID MAPPING] Migrating favorite: ${teamId} -> ${normalizedId} (${normalizedSport})`
      );
