...
export class ChatUtils {
    ...
    try {
      ...
      if (typeof Intl !== "undefined" && Intl.DateTimeFormat) {
        try {
          ...
          console.warn(
            "Intl API failed, falling back to toLocaleString:",
            intlError,
          );
        }
      }
      ...
      console.warn(
        "Error converting to ET, using local time with UTC offset approximation:",
        error,
      );
    ...
    try {
      ...
      if (typeof Intl !== "undefined" && Intl.DateTimeFormat) {
        try {
          ...
          console.warn(
            "Intl API failed for date conversion, falling back:",
            intlError,
          );
        }
      }
  ...
  static isGameToday(gameDate) {
    try {
      ...
      const isToday =
        nowYear === gameYear && nowMonth === gameMonth && nowDay === gameDay;
      ...
      if (nowHour < 2) {
        ...
        return (
          yesterdayYear === gameYear &&
          yesterdayMonth === gameMonth &&
          yesterdayDay === gameDay
        );
      }
  ...
  static isChatAvailable(gameData) {
    ...
    const possibleDateFields = [
      ...
      { key: "datetime.dateTime", value: gameData.datetime?.dateTime },
      { key: "datetime.officialDate", value: gameData.datetime?.officialDate },
      { key: "datetime.originalDate", value: gameData.datetime?.originalDate },
      {
        key: "gameData.datetime.dateTime",
        value: gameData.gameData?.datetime?.dateTime,
      },
      {
        key: "gameData.datetime.officialDate",
        value: gameData.gameData?.datetime?.officialDate,
      },
      {
        key: "gameData.datetime.originalDate",
        value: gameData.gameData?.datetime?.originalDate,
      },
      { key: "competitions[0].date", value: gameData.competitions?.[0]?.date },
      {
        key: "competitions[0].startDate",
        value: gameData.competitions?.[0]?.startDate,
      },
      // ESPN-specific fields (often nested under header)
      {
        key: "header.competitions[0].date",
        value: gameData.header?.competitions?.[0]?.date,
      },
      {
        key: "header.competitions[0].startDate",
        value: gameData.header?.competitions?.[0]?.startDate,
      },
      ...
      {
        key: "awayTeam.gameDate || homeTeam.gameDate",
        value: gameData.awayTeam?.gameDate || gameData.homeTeam?.gameDate,
      },
    ];
    ...
    for (const field of possibleDateFields) {
      ...
      try {
        ...
        console.debug(
          `[ChatUtils] candidate ${field.key} => raw: ${String(field.value)}, normalizedValid: ${parsedValid}, normalizedUTC: ${normalized ? normalized.toISOString() : "<invalid>"}, parsedET: ${parsedET ? parsedET.toString() : "<invalid>"}`,
        );

        if (parsedValid && this.isGameToday(normalized)) {
          console.info(
            `[ChatUtils] Chat available: matched field ${field.key} => ${parsedET ? parsedET.toDateString() : normalized.toDateString()}`,
          );
          return true;
        }
      } catch (e) {
  ...
  static async cleanupOldMessages(daysOld = 2) {
    try {
      ...
      console.log(
        `Starting cleanup of messages older than ${daysOld} days (before ${cutoffDate.toISOString()})`,
      );
      ...
      for (const collectionName of chatCollections) {
        ...
        for (let i = 0; i < snapshot.docs.length; i += batchSize) {
          ...
          try {
            ...
            console.warn(
              `Failed to delete batch of ${batch.length} messages:`,
              batchError.message,
            );
      ...
      if (
        ...
        console.info(
          "Chat cleanup skipped due to insufficient permissions:",
          error.message || error,
        );
      } else {
  ...
  static scheduleCleanup(intervalHours = 24, daysOld = 2) {
    ...
    this.cleanupOldMessages(daysOld).catch((error) => {
      if (
        ...
        console.info(
          "Chat cleanup skipped: Insufficient permissions (disabling future cleanups)",
        );
    ...
    const cleanupInterval = setInterval(() => {
      ...
      this.cleanupOldMessages(daysOld).catch((error) => {
        if (
          ...
          console.info(
            "Scheduled cleanup skipped: Insufficient permissions (disabling future cleanups)",
          );
          this.CLEANUP_ENABLED = false;
          clearInterval(cleanupInterval);
        } else {
    ...
    console.log(
      `Scheduled automatic chat cleanup every ${intervalHours} hours for messages older than ${daysOld} days (dev mode only)`,
    );
  }
      ...
      if (gameDate) {
        try {
          ...
          if (isNaN(month) || isNaN(day) || isNaN(year)) {
            console.warn("NaN values in date formatting:", {
              month,
              day,
              year,
              gameDateET,
  ...
  static getGameDateFromData(gameData) {
    ...
    for (const c of candidates) {
      if (c.value != null) {
        ...
        console.debug(
          `[ChatUtils] Candidate ${c.key} raw=${String(c.value)} normalized=${normalized ? normalized.toISOString() : "<invalid>"}`,
        );
  ...
  static debugTimezone(gameDate, gameData = null) {
    ...
    if (gameDate) {
      ...
      try {
        ...
        console.log(
          "Is game today (with 2am extension)?",
          this.isGameToday(gameDate),
        );
      } catch (gameError) {
