...
// ── Polling intervals (NBA has no ticking clock) ────────────────────
...
const getStatusPriority = (game) => {
  if (isNbaGameLive(game)) return 1;
  if (isNbaGameFinished(game)) return 3;
  return 2;
};
...
const getNbaLogoUrl = (team, isDarkMode) => {
  const abbrev = (team?.abbreviation || "").toLowerCase();
  if (!abbrev) return null;
  const themeSuffix = isDarkMode ? "-dark" : "";
  return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nba/500${themeSuffix}/scoreboard/${abbrev}.png&w=200&h=200`;
};

// ── Polling interval calculation ────────────────────────────────────
const getNbaGamePollingInterval = (game, nowMs) => {
  if (isNbaGameLive(game)) return INTERVAL_LIVE;
  if (isNbaGameFinished(game)) return INTERVAL_PRE_FAR;
...
const getPollingInterval = (groups) => {
  ...
  allGames.forEach((game) => {
    desired = Math.min(desired, getNbaGamePollingInterval(game, now));
  });
  return desired;
};
    ...
    >
      <View style={dateBarStyles.wrapper}>
        ...
        >
          ...
          >
            <Defs>
              <LinearGradient id="dfL_nba" x1="0%" y1="0%" x2="100%" y2="0%">
                <Stop
                  offset="0%"
          ...
          >
            <Defs>
              <LinearGradient id="dfR_nba" x1="100%" y1="0%" x2="0%" y2="0%">
                <Stop
                  offset="0%"
                  stopColor={theme.background}
                  stopOpacity="1"
...
// ── Grid view ───────────────────────────────────────────────────────
const NBA_GRID_H_PAD = 16;
const NBA_GRID_GAP = 8;
const NBA_CARD_WIDTH = (width - NBA_GRID_H_PAD * 2 - NBA_GRID_GAP) / 2;

const NBAGridCardGradient = ({
  gradId,
  awayColor,
  homeColor,
  fallbackColor,
  ...
  return (
    <Svg
      ...
      width={NBA_CARD_WIDTH}
      height={safeHeight}
      viewBox={`0 0 ${NBA_CARD_WIDTH} ${safeHeight}`}
...
const NBAGridCard = ({
  ...
  return (
    <TouchableOpacity
      style={[
        nbaGridStyles.card,
        { backgroundColor: theme.surfaceSecondary, width: NBA_CARD_WIDTH },
      ]}
      onLayout={(e) => {
    ...
    >
      ...
      <View style={nbaGridStyles.cardTop}>
        <Text
          style={[
            nbaGridStyles.statusText,
            {
        ...
        <LiveViewerBadge
          ...
          scale={0.7}
          style={nbaGridStyles.cardBadge}
        />
      </View>

      <View style={nbaGridStyles.teamsRow}>
        <View style={nbaGridStyles.teamSide}>
          {isScheduled ? (
            awayLogo && !awayLogoError ? (
              <Image
                cachePolicy="memory-disk"
                source={{ uri: awayLogo }}
                style={nbaGridStyles.teamLogo}
                contentFit="contain"
              ...
              <View
                style={[
                  nbaGridStyles.teamLogoPlaceholder,
            ...
            <View style={nbaGridStyles.scoreCell}>
              <Text
                style={[
                  nbaGridStyles.scoreText,
                  {
                    color: awayFav
                      ? colors.primary
                      : awayWins
              ...
              {awayLogo && !awayLogoError && (
                <Image
                  ...
                  source={{ uri: awayLogo }}
                  style={[
                    nbaGridStyles.scoreLogoOverlay,
                    { opacity: isFinished && !awayWins ? 0.55 : 1 },
                  ]}
                  contentFit="contain"
                  onError={() => setAwayLogoError(true)}
        ...
        <View
          style={[nbaGridStyles.divider, { backgroundColor: theme.border }]}
        />
      ...
      <View
        style={[nbaGridStyles.cardFooter, { borderTopColor: theme.border }]}
      >
        <Text
          style={[nbaGridStyles.venueText, { color: theme.textSecondary }]}
          numberOfLines={1}
        >
          {game.venue || ""}
        </Text>
...
const NBAGridSection = ({
  ...
  <View style={nbaGridStyles.container}>
    {groups.map((group) => (
      <View key={group.dateKey} style={nbaGridStyles.groupWrapper}>
        <View style={nbaGridStyles.groupHeaderRow}>
          <View
            style={[
              nbaGridStyles.groupBubble,
          ...
          >
            ...
            <Text
              style={[
                nbaGridStyles.groupBubbleCount,
        ...
        <View style={nbaGridStyles.cardsRow}>
...
const NBAScoreboardScreen = ({ navigation }) => {
  ...
  useEffect(() => {
    AsyncStorage.getItem("viewMode_nba").then((val) => {
  ...
  const toggleViewMode = () => {
    setIsGridView((v) => {
      const next = !v;
      AsyncStorage.setItem("viewMode_nba", next ? "grid" : "list");
      return next;
    });
  };
  ...
  const loadData = useCallback(
    async (filter, silent = false, background = false, force = false) => {
      ...
      const promise = (async () => {
        ...
        try {
          const data = await NBAService.getScoreboard(filter, filter);
          ...
          console.error("NBA scoreboard fetch error:", err);
          setGroups([]);
          return [];
        } finally {
          setLoading(false);
  ...
  const schedulePolling = useCallback(
    (filter, latestGroups) => {
      ...
      const hasLiveGames = allGames.some((game) => isNbaGameLive(game));
...
const nbaGridStyles = StyleSheet.create({
  container: {
    paddingHorizontal: NBA_GRID_H_PAD,
    marginBottom: 24,
  },
  groupWrapper: {
    marginBottom: 16,
