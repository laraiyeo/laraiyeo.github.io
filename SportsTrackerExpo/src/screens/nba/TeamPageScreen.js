...
const TeamPageScreen = ({ route, navigation }) => {
  ...
  const statsConfiguration = {
    ...
    // 'Shooting': { limit: 4, statNames: ['fieldGoalPercentage', 'threePointPercentage', 'freeThrowPercentage', 'effectiveFieldGoalPercentage'] },
    // 'Rebounding': { limit: 3, statNames: ['reboundsPerGame', 'offensiveRebounds', 'defensiveRebounds'] }
  };
  ...
  const deriveHeaderStats = () => {
    ...
    if (differential === null && teamData?.differential !== undefined)
      differential = teamData.differential;
    if (differential === null && teamData?.team?.differential !== undefined)
      differential = teamData.team.differential;
    if (differential === null && teamRecord?.differential !== undefined)
      differential = teamRecord.differential;
    let diffKind = null;
  ...
  useEffect(() => {
    try {
      ...
      console.log("NBA TeamHeader debug:", {
        teamsUrl,
        headerStats,
  ...
  const fetchTeamData = async () => {
    try {
      ...
      if (data && data.team) {
          ...
          return fetch(scheduleUrl)
            ...
            .then((j) => {
              ...
              try {
                ...
                console.log("NBA TeamPage: schedule response", {
                  type,
                  scheduleUrl,
            ...
            .catch((err) => {
              console.log(
                "NBA TeamPage: schedule fetch failed for type",
                type,
                "url",
                scheduleUrl,
                err
        ...
        try {
          console.log("NBA TeamPage: chosen season type and events", {
            chosenType,
  ...
  const fetchCurrentGame = async (eventsParam) => {
    try {
      ...
      if (events && events.length > 0) {
        ...
        if (inWindowEvent) {
          console.log(
            "NBA fetchCurrentGame: selected in-window game as current:",
            {
  ...
  const fetchAllMatches = async (eventsParam) => {
    try {
      ...
      let events = eventsParam || cachedEvents.current;
      console.log(
        "NBA fetchAllMatches: eventsParam length:",
      ...
      if (cachedEvents.byType) {
        try {
          ...
          cachedEvents.current = events;
          console.log(
            "NBA fetchAllMatches: combined events from byType, total:",
            events.length
          );
        } catch (e) {
          ...
          cachedEvents.current = events;
          console.log(
            "NBA fetchAllMatches: fallback fetch for type",
            chosenType,
            "events:",
            events.length
          );
        ...
        cachedEvents.current = events;
        console.log(
          "NBA fetchAllMatches: fresh fetch for type",
          chosenType,
          "events:",
          events.length
        );
      ...
      try {
        if (Array.isArray(events) && events.length > 0 && !events[0].homeTeam) {
          ...
          cachedEvents.current = events;
          console.log(
            "NBA fetchAllMatches: normalized events for rendering, total:",
            events.length
          );
        }
      } catch (e) {
      ...
      console.log(
        "NBA fetchAllMatches: processing",
        events.length,
        "events for past/future classification"
      );
      console.log("NBA fetchAllMatches: today window", {
        startOfTodayLocal,
        endOfWindowLocal,
      });
      ...
      for (const ev of events) {
        ...
        if (
          ...
          console.log("NBA fetchAllMatches: skipping event in today window:", {
            id: ev.id,
            date: ev.date,
            evDate,
          });
        ...
        if (
          ...
          console.log(
            "NBA fetchAllMatches: filtering out currentGame from future list:",
            ev.id
          );
          continue;
        }
      ...
      console.log(
        "NBA fetchAllMatches: final results - past:",
        past.length,
        "future:",
        future.length
      );
  ...
  const fetchRoster = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          "NBA TeamPage: roster API failed, status:",
          response.status
        );
        setRoster([]);
      }
  ...
  const fetchTeamStats = async () => {
    ...
    try {
      ...
      console.log(
        "NBA TeamPage: fetching stats for team id (v2):",
        currentTeamId
      );
      ...
      outerLoop: for (const t of typesToTry) {
        for (const year of yearsToTry) {
          try {
            ...
            // eslint-disable-next-line no-console
            console.log(
              "NBA TeamPage: trying stats type",
              t,
              "year",
              year,
              statsUrl
            ...
            if (!resp.ok) {
              console.log(
                "NBA TeamPage: stats v2 request failed for type",
                t,
                "status",
                resp.status
              );
            ...
            const hasStatistics =
              json.statistics && Object.keys(json.statistics).length > 0;
            const hasResultsCategories =
              json.results &&
              json.results.stats &&
              Array.isArray(json.results.stats.categories) &&
              json.results.stats.categories.length > 0;
            const hasSplitsCategories =
              json.splits &&
              ((Array.isArray(json.splits) &&
                ...
                (json.splits.categories &&
                  Array.isArray(json.splits.categories) &&
                  json.splits.categories.length > 0));
            if (
              json &&
              (hasGroups ||
                hasCategories ||
              ...
              v2data = json;
              console.log(
                "NBA TeamPage: got v2 stats for type",
                t,
                "year",
                year
              );
              ...
              console.log(
                "NBA TeamPage: v2 type",
                t,
                "year",
                year,
                "returned HTTP 200 but no usable data. keys:",
                Object.keys(json),
                "hasGroups:",
                hasGroups,
                "hasCategories:",
            ...
            // eslint-disable-next-line no-console
            console.log(
              "NBA TeamPage: error fetching v2 stats for type",
              t,
              "year",
              year,
              e
      ...
      if (v2data) {
        ...
        if (categories.length) {
          console.log(
            "NBA TeamPage: normalized categories count:",
            categories.length
          );
          console.log(
            "NBA TeamPage: sample categories:",
          ...
          console.log(
            "NBA TeamPage: setting teamStats with structure:",
            Object.keys(finalStats)
          );
          setTeamStats(finalStats);
        } else {
          console.log(
            "NBA TeamPage: no categories could be normalized from v2 stats response"
          );
          setTeamStats({});
        }
      } else {
        console.log(
          "NBA TeamPage: no v2 stats found for any type; falling back to site API (match NFL behavior)"
        );
        ...
        try {
          ...
          if (response.ok) {
            const data = await response.json();
            console.log(
              "NBA TeamPage: fallback stats API response (site) keys:",
              Object.keys(data)
            );
            console.log(
              "NBA TeamPage: fallback stats API response (site) structure:",
              data
            );
            ...
            try {
              ...
              if (fallbackCategories.length) {
                ...
                console.log(
                  "NBA TeamPage: normalized fallback categories count:",
                  fallbackCategories.length
                );
                setTeamStats(finalStats);
              } else {
                ...
                console.log(
                  "NBA TeamPage: could not normalize fallback stats; using raw site response"
                );
                setTeamStats(data);
              }
            } catch (e) {
              console.log(
                "NBA TeamPage: error normalizing fallback stats response",
                e
              );
              setTeamStats(data);
            }
            ...
            console.log(
              "NBA TeamPage: fallback stats API failed, status:",
              response.status
            );
            setTeamStats({});
          }
  ...
  const getPlayerHeadshotUrl = (player) => {
    ...
    if (id)
      return `https://a.espncdn.com/combiner/i?img=/i/headshots/nba/players/${id}.png&w=88&h=88`;
  ...
  const renderMatchCard = (game) => {
    const competition =
      getCompetitionName(game.competition?.id || "nba") || "NBA";
                  ...
                  <View style={styles.rosterPlayerRow}>
                    <Image
                      source={{
                        uri:
                          player.headshot ||
                          "https://via.placeholder.com/40x40?text=NBA",
                      }}
                      style={styles.playerHeadshot}
                      defaultSource={{
                        uri: "https://via.placeholder.com/40x40?text=NBA",
                      }}
                    />
  ...
  const renderRosterTab = () => {
    ...
    const guards = roster.filter((player) => {
    ...
    const centers = roster.filter((player) => {
      const pos = (player.position || "").toString().toUpperCase();
      return pos.includes("C") || pos.includes("CENTER");
    });
    const others = roster.filter((player) => {
      const pos = (player.position || "").toString().toUpperCase();
      return (
        !guards.includes(player) &&
        !forwards.includes(player) &&
        !centers.includes(player)
      );
    });
    ...
    return (
      ...
      >
        {renderRosterSection("Guards", guards, "guards")}
        {renderRosterSection("Forwards", forwards, "forwards")}
        {renderRosterSection("Centers", centers, "centers")}
  ...
  const renderStatsContent = () => {
    ...
    const orderPriority = (name) => {
      ...
      if (n.includes("offens") || n.includes("attack") || n.includes("scoring"))
        return 0;
      if (n.includes("defens") || n.includes("opponent") || n.includes("stop"))
        return 1;
      if (n.includes("general") || n.includes("overall") || n.includes("team"))
        return 2;
      return 3;
    };
