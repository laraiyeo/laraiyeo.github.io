...
const NBAPlayerPageScreen = ({ route, navigation }) => {
  ...
  const getTeamLogoUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    if (failedLogos.has(primaryKey)) {
      return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nba/${useDarkMode ? '500' : '500-dark'}/${normalized}.png&w=200&h=200`;
    }
    return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nba/${useDarkMode ? '500-dark' : '500'}/${normalized}.png&w=200&h=200`;
  };
  ...
  const getTeamLogoFallbackUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    const normalized = String(normalizedToken).toLowerCase();
    return { uri: `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nba/500/${normalized}.png&w=200&h=200` };
  };
  ...
  const getTileStatsForSeason = (item) => {
    try {
      ...
      const threePt = findStatByNames(['threePointFieldGoalsMade', 'threePointers', '3PM']);
      ...
      tileStats = candidates.slice(0, 3).map(s => makeStatEntry(s)).filter(Boolean);
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const basicPlayerData = {
        ...
        headshot: { href: `https://a.espncdn.com/combiner/i?img=/i/headshots/nba/players/full/${playerId}.png` }
      };
      ...
      if (statsData && statsData.splits && Array.isArray(statsData.splits.categories)) {
        try {
          ...
          const preferredByCategory = {
            general: [
              'gamesPlayed', 'avgMinutes', 'plusMinus', 'PER', 'doubleDouble', 'fouls'
            ],
            offensive: [
              'points', 'fieldGoalsMade', 'fieldGoalsAttempted', 'threePointFieldGoalsMade', 'freeThrowsMade', 'avgPoints'
            ],
            defensive: [
              'blocks', 'steals', 'defensiveRebounds', 'personalFouls'
            ],
          ...
          const chooseStatsForCategory = (cat) => {
            ...
            if (chosen.length < 6) {
              for (const s of allStats) {
                ...
                const lname = s.name.toLowerCase();
                if (lname.includes('points') || lname.includes('rebounds') || lname.includes('assists') || lname.includes('field') || lname.includes('three') || lname.includes('free') || lname.includes('blocks') || lname.includes('steals') || lname.includes('games') || lname.includes('minutes')) {
                  pushStat(s);
