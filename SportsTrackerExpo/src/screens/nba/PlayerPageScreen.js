...
const NBAPlayerPageScreen = ({ route, navigation }) => {
  ...
  const getTeamLogoUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    if (failedLogos.has(primaryKey)) {
      return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nba/${
  ...
  const getTeamLogoFallbackUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    const normalized = String(normalizedToken).toLowerCase();
    return {
      uri: `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nba/500/${normalized}.png&w=200&h=200`,
    };
  };
  ...
  const getTileStatsForSeason = (item) => {
    try {
      ...
      const fg = findStatByNames(["fieldGoalsMade", "fieldGoals", "fg"]);
      const threePt = findStatByNames([
        "threePointFieldGoalsMade",
        "threePointers",
        "3PM",
      ]);
      const ft = findStatByNames(["freeThrowsMade", "freeThrows", "ft"]);

      // Prioritize most important stats
      const candidates = [pts, reb, ast, fg, threePt, ft].filter(Boolean);
      tileStats = candidates
        .slice(0, 3)
        .map((s) => makeStatEntry(s))
        .filter(Boolean);

      // If we don't have enough stats, fall back to first category's first 3 stats
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      // Use NBA-specific year logic for current stats
      const seasonYear = getNBAYear();

      const siteUrl = `https://site.api.espn.com/apis/site/v2/sports/basketball/nba/athletes/${playerId}`;
      const splitsUrl = `https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba/athletes/${playerId}/splits?season=${seasonYear}`;
      ...
      const gamelogUrl = `https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba/athletes/${playerId}/gamelog?season=${seasonYear}`;
          ...
          const chooseStatsForCategory = (cat) => {
            ...
            if (chosen.length < 6) {
              for (const s of allStats) {
                ...
                // simple heuristic: pick stats containing category keywords or common basketball stat words
                const lname = s.name.toLowerCase();
                if (
                  lname.includes("points") ||
                  lname.includes("rebounds") ||
                  lname.includes("assists") ||
                  lname.includes("field") ||
                  lname.includes("three") ||
                  lname.includes("free") ||
                  lname.includes("blocks") ||
                  lname.includes("steals") ||
                  lname.includes("games") ||
                  lname.includes("minutes")
                ) {
                  pushStat(s);
                ...
                categories: [
                  {
                    ...
                    stats: [
                      ...
                      },
                      {
                        ...
                        value: statsArr[7] || 0,
                        displayValue: (statsArr[7] || 0).toString(),
                      },
                      {
                        name: "assists",
                        displayName: "Assists",
                      ...
                      },
                      {
                        ...
                        displayName: "Steals",
                        value: statsArr[10] || 0,
                        displayValue: (statsArr[10] || 0).toString(),
                      },
                      {
                        name: "blocks",
                        displayName: "Blocks",
                      ...
                      },
                      {
                        ...
                        value: statsArr[11] || 0,
                        displayValue: (statsArr[11] || 0).toString(),
                      },
                      {
                        name: "fieldGoalPct",
                        displayName: "FG %",
