...
const DraftScreen = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const rounds =
        json.rounds && json.rounds.length
          ? json.rounds.map((r) => r.round)
          : null;
      if (rounds && rounds.length) setSelectedRound(rounds[0]);
      else if (json.picks && json.picks.length)
        setSelectedRound(json.picks[0].round);
    } catch (e) {
  ...
  const rounds = useMemo(() => {
    ...
    if (data.picks)
      return Array.from(new Set(data.picks.map((p) => p.round))).sort(
        (a, b) => a - b
      );
    return [];
  }, [data]);
  ...
  const picksForRound = useMemo(() => {
    ...
    return picks
      .filter((p) => Number(p.round) === Number(selectedRound))
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <View style={[styles.roundsRow, { justifyContent: "center" }]}>
        {rounds.map((r) => (
          ...
          >
            <Text
              style={{ color: selectedRound === r ? "#fff" : theme.text }}
            >{`Round ${r}`}</Text>
          </TouchableOpacity>
        ))}
      </View>
      ...
      <FlatList
        ...
        contentContainerStyle={{ padding: 12, marginTop: -10 }}
        renderItem={({ item }) => {
          ...
          const getTeamLogo = (t) => {
            ...
            return (
              (isDarkMode
                ? `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nfl/500-dark/${t.abbreviation.toLowerCase()}.png&w=200&h=200`
                : `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nfl/500/${t.abbreviation.toLowerCase()}.png&w=200&h=200`) ||
              null
            );
          };
          ...
          const posRank =
            (attr.find((a) => a.name === "posRank" || a.name === "rank") || {})
              .displayValue || "";
          const ovrRank =
            (attr.find((a) => a.name === "overall" || a.name === "ovr") || {})
              .displayValue || "";
                      ...
                      {showTrade ? (
                        <View style={styles.expandBottomRow}>
                          ...
                          <View style={styles.tradeColumn}>
                            ...
                            >
                              {(tradeNote ?? "").charAt(0).toUpperCase() +
                                (tradeNote ?? "").slice(1)}
                            </Text>
                          </View>
                        </View>
