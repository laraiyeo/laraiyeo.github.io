...
const NFLPlayerPageScreen = ({ route, navigation }) => {
  ...
  const getHeadshotSource = (p) => {
    try {
      ...
      if (id)
        return {
          uri: convertToHttps(
            `https://a.espncdn.com/combiner/i?img=/i/headshots/nfl/players/${id}.png&w=88&h=88`
          ),
        };
    } catch (e) {
      // fall through
  ...
  const getTeamLogoUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    if (failedLogos.has(primaryKey)) {
      return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nfl/${
  ...
  const getTeamLogoFallbackUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    const normalized = String(normalizedToken).toLowerCase();
    return {
      uri: `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nfl/500/${normalized}.png&w=200&h=200`,
    };
  };
  ...
  const positionCategoryFromDisplay = (posDisplay) => {
    ...
    if (p.includes("running") || p === "rb" || p.includes("running back"))
      return "RB";
    if (
      p.includes("wide") ||
      p.includes("receiver") ||
        ...
        if (
          ...
          switch (posCat) {
            ...
            case "RB": {
              ...
              const att = findStatInCategories(
                ["rushing", "rush"],
                [
                  "rushingAttempts",
                  "rushAttempts",
            ...
            case "WR": {
              ...
              const rec = findStatInCategories(
                ["receiv", "rec", "receiving"],
                ["receptions", "rec", "targets"]
              );
              const recFallback =
                rec || findStatByNames(["receptions", "rec", "targets"]);
              const y = findStatByNames([
                "receivingyards",
                "receivingYards",
                "netTotalYards",
            ...
            case "PK": {
              const fg = findStatByNames([
                "fieldgoals",
                "fieldGoals",
                "fieldGoalsMade",
                "fgmade",
              ]);
              const xp = findStatByNames([
                "extrapoints",
                "extraPoints",
                "extraPointsMade",
                "pat",
            ...
            case "DB": {
              const tot = findStatByNames([
                "tackles",
                "totaltackles",
                "totalTackles",
              ]);
              const sack = findStatByNames(["sacks", "sack"]);
              const pd = findStatByNames([
                "passdeflections",
                "passesdefended",
                "pd",
                "passdefense",
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const basicPlayerData = {
        ...
        headshot: {
          href: `https://a.espncdn.com/combiner/i?img=/i/headshots/nfl/players/full/${playerId}.png`,
        },
      };
                  ...
                  return (
                    ...
                    >
                      <Image
                        ...
                        source={
                          getStableTeamLogoUrl({ abbreviation: teamAbbr })
                            ? {
              ...
              >
                ...
                >
                  <View style={styles.mlbTeamLogos}>
                    <Image
                      key={`nfl-game-player-team-${game.id}-${
    ...
    try {
      ...
      if (game && game.id) {
        try {
          ...
          console.warn(
            "Unable to fetch game stats for player:",
            e?.message || e
          );
          setSelectedGameStatsDetails(null);
        }
              ...
              >
                ...
                >
                  <View style={styles.modalTeamContainer}>
                    <View style={styles.mlbTeamLogos}>
                      <Image
                        key={`nfl-modal-game-player-team-${g.id}-${
              ...
              >
                <View style={styles.careerTileHeader}>
                  <View style={styles.careerLogosRow}>
                    {logos.map((l, li) => (
                      <Image
                        key={`nfl-career-logo-${item.season}-${li}-${l}`}
                        source={
