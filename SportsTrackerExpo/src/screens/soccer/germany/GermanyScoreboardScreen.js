...
const GermanyScoreboardScreen = ({ navigation, route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('GermanyScoreboardScreen: Screen focused');
      setIsScreenFocused(true);
      
      return () => {
        console.log('GermanyScoreboardScreen: Screen unfocused, clearing intervals');
  ...
  useEffect(() => {
    console.log('GermanyScoreboardScreen: Main useEffect triggered for filter:', selectedDateFilter, 'focused:', isScreenFocused);
  ...
  useEffect(() => {
    console.log('GermanyScoreboardScreen: Preload useEffect triggered, hasPreloaded:', hasPreloadedRef.current);
    // Only preload if we haven't done it before
    if (hasPreloadedRef.current) {
      console.log('GermanyScoreboardScreen: Skipping preload, already done');
      return;
    }
    
    // Mark that we're doing preloading
    hasPreloadedRef.current = true;
    console.log('GermanyScoreboardScreen: Starting preload for other filters');
    
    // Preload the other filters in the background after initial load
    const preloadTimer = setTimeout(() => {
      if (selectedDateFilter !== 'yesterday') {
        console.log('GermanyScoreboardScreen: Preloading yesterday data');
        loadScoreboard(true, 'yesterday');
      }
      if (selectedDateFilter !== 'upcoming') {
        console.log('GermanyScoreboardScreen: Preloading upcoming data');
  ...
  const loadScoreboard = async (silentUpdate = false, dateFilter = selectedDateFilter) => {
    console.log('GermanyScoreboardScreen: loadScoreboard called - silentUpdate:', silentUpdate, 'dateFilter:', dateFilter);
    const now = Date.now();
    ...
    if (isCacheValid && !silentUpdate) {
      console.log('GermanyScoreboardScreen: Using cached data for', dateFilter);
      setGames(cachedData);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('GermanyScoreboardScreen: Fetching fresh data for', dateFilter);
      ...
      const processedGames = await Promise.all((data.events || []).map(async (game) => {
        // Get team logos
        const awayLogo = await GermanyServiceEnhanced.getTeamLogoWithFallback(game.competitions[0]?.competitors[1]?.team?.id);
        const homeLogo = await GermanyServiceEnhanced.getTeamLogoWithFallback(game.competitions[0]?.competitors[0]?.team?.id);
      ...
      if (dateFilter === selectedDateFilter) {
        ...
        if (currentHash !== lastUpdateHash) {
          ...
          console.log('GermanyScoreboardScreen: Data updated for', dateFilter);
        }
      }

      setLoading(false);
    } catch (error) {
      console.error('GermanyScoreboardScreen: Error loading scoreboard:', error);
      if (!silentUpdate) {
        setLoading(false);
  ...
  const handleDateFilterChange = (filter) => {
    ...
    console.log('GermanyScoreboardScreen: Changing filter to:', filter);
    ...
    if (isCacheValid) {
      console.log('GermanyScoreboardScreen: Using cached data for filter change to:', filter);
      setGames(cachedData);
      setLoading(false);
    } else {
      console.log('GermanyScoreboardScreen: No valid cache for filter:', filter, '- will fetch fresh data');
    }
  };
  ...
  const handleGamePress = (game) => {
    console.log('GermanyScoreboardScreen: Game pressed:', game.id);
    navigation.navigate('GermanyGameDetails', {
      gameId: game.id,
      sport: 'German',
      competition: game.competitionName || 'Germany',
        ...
        <View style={styles.matchContent}>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              { color: isFavorite(homeTeam?.team?.id, 'bundesliga') ? colors.primary : theme.text },
              matchStatus.isPost && homeTeam?.score < awayTeam?.score && styles.losingTeamName
            ]}>
              {isFavorite(homeTeam?.team?.id, 'bundesliga') ? '★ ' : ''}{homeTeam?.team?.abbreviation || homeTeam?.team?.displayName || 'TBD'}
            </Text>
          </View>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              styles.teamAbbreviation, 
              { color: isFavorite(awayTeam?.team?.id, 'bundesliga') ? colors.primary : theme.text },
              matchStatus.isPost && awayTeam?.score < homeTeam?.score && styles.losingTeamName
            ]}>
              {isFavorite(awayTeam?.team?.id, 'bundesliga') ? '★ ' : ''}{awayTeam?.team?.abbreviation || awayTeam?.team?.displayName || 'TBD'}
            </Text>
          </View>
        </View>
...
export default GermanyScoreboardScreen;
