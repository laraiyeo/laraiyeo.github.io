...
const GermanyTeamPageScreen = ({ route, navigation }) => {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      if (currentGame) {
        ...
        const competition = currentGame.leagueCode || 'ger.1' || (currentGame.competitions?.[0]?.league?.id) || null;
  ...
  useEffect(() => {
    console.log('GermanyTeamPageScreen received - teamId:', teamId, 'teamName:', teamName);
    fetchTeamData();
  ...
  const fetchTeamData = async () => {
    try {
      ...
      const url = `https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/teams/${teamId}`;
  ...
  const fetchTeamRecord = async (teamId) => {
    try {
      ...
      const standingsData = await GermanyServiceEnhanced.getStandings();
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      // Fetch from all German competitions in parallel
      const germanCompetitions = ['ger.1', 'ger.dfb_pokal', 'ger.super_cup'];
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, 'bundesliga')) {
              ...
              console.log('GermanyTeamPageScreen: team is favorited - updating persisted currentGame:', currentGamePayload);
  ...
  const fetchRoster = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/teams/${teamId}/roster?season=${currentYear}`
      );
      const data = await response.json();
      ...
      if (data.athletes && data.athletes.length > 0) {
        ...
        const prevResponse = await fetch(
          `https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/teams/${teamId}/roster?season=${currentYear - 1}`
        );
  ...
  const fetchTeamStats = async () => {
    ...
    try {
      ...
      // Fetch statistics for different German competitions
      const seasonTypes = [
        { id: '1', name: 'Bundesliga', leagueCode: 'ger.1' },
        { id: '7', name: 'DFB Pokal', leagueCode: 'ger.dfb_pokal' },
        { id: '8', name: 'German Super Cup', leagueCode: 'ger.super_cup' }
      ];
  ...
  const getCompetitionName = (leagueCode) => {
    if (!leagueCode) return 'Bundesliga';
      ...
      <View style={[styles.teamHeader, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.teamInfo}>
          ...
          <Text allowFontScaling={false} style={[styles.teamDivision, { color: theme.textSecondary }]}>
            {teamData.standingSummary || 'Germany'}
          </Text>
      ...
      >
        ...
        <View style={[styles.leagueHeader, { backgroundColor: theme.surfaceSecondary }]}>
          <Text allowFontScaling={false} style={[styles.leagueText, { color: colors.primary }]}>
            {getCompetitionName(game.leagueCode) || competition?.name || competition?.league?.name || 'Bundesliga'}
          </Text>
        </View>
                ...
                return (
                  <TouchableOpacity 
                    ...
                    onPress={() => {
                      ...
                      navigation.navigate('GermanyPlayerPage', {
                        playerId: player.id,
...
export default GermanyTeamPageScreen;
