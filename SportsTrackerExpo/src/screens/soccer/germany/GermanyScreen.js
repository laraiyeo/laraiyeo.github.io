...
import { GermanyService } from '../../../services/soccer/GermanyService';
...
const GermanyScreen = () => {
  ...
  const leagueId = route.params?.leagueId || 'germany';
  const leagueName = route.params?.leagueName || 'Germany';
  ...
  const loadMatches = async () => {
    try {
      const data = await GermanyService.getScoreboard();
  ...
  const loadStandings = async () => {
    try {
      const data = await GermanyService.getStandings();
