...
const GermanySearchScreen = ({ route, navigation }) => {
  const { sport } = route?.params || { sport: 'German' };
  ...
  const searchTeams = async (query) => {
    try {
      const teamsData = await GermanyServiceEnhanced.searchTeams(query);
      
      if (teamsData && Array.isArray(teamsData)) {
        const processedTeams = await Promise.all(
          teamsData.map(async (teamData) => {
            try {
              const logo = await GermanyServiceEnhanced.getTeamLogoWithFallback(teamData.team.id);
              return {
                id: teamData.team.id,
                type: 'team',
  ...
  const searchPlayers = async (query) => {
    try {
      const playersData = await GermanyServiceEnhanced.searchPlayers(query);
        ...
        const playersWithTeamInfo = await Promise.all(
          playersData.map(async (playerData) => {
            ...
            if (playerData.teamId) {
              try {
                const teamResponse = await fetch(convertToHttps(`https://site.api.espn.com/apis/site/v2/sports/soccer/ger.1/teams/${playerData.teamId}`));
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.searchHeader, { backgroundColor: theme.surface }]}>
        ...
        <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
          Search for {(sport || 'German').toUpperCase()} teams and players
        </Text>
      </View>
...
export default GermanySearchScreen;
