...
const GermanyGameDetailsScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      const isLive = gameData && gameData.header?.competitions?.[0]?.status?.type?.state === 'in';
      if (isLive) {
        console.log('Stream modal closed, immediately fetching Germany game data');
        ...
        <View style={styles.matchContainer}>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              {isFavorite(homeTeam?.team?.id, 'bundesliga') ? '★ ' : ''}{homeTeam?.team?.displayName}
            </Text>
          </View>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              {isFavorite(awayTeam?.team?.id, 'bundesliga') ? '★ ' : ''}{awayTeam?.team?.displayName}
            </Text>
          </View>
        </View>
  ...
  const renderMiniField = (coordinate, coordinate2, eventType = 'gen', teamSide = 'home', teamColor = '#007bff') => {
    ...
    const renderPlayerPopup = () => {
      ...
      if (selectedPlayer.teamType === 'home') {
        teamColor = GermanyServiceEnhanced.getTeamColorWithAlternateLogic(homeTeamData?.team) || '#007bff';
      } else if (selectedPlayer.teamType === 'away') {
        teamColor = GermanyServiceEnhanced.getTeamColorWithAlternateLogic(awayTeamData?.team) || '#28a745';
      }
              ...
              {currentStreamType && availableStreams[currentStreamType] && (
                <WebView
                  ...
                  onShouldStartLoadWithRequest={(request) => {
                    console.log('Germany WebView navigation request:', request.url);
                    ...
                    if (requestDomain !== currentDomain || hasPopupKeywords) {
                      console.log('Blocked Germany popup/cross-domain navigation:', request.url);
                      return false;
                    }
                  ...
                  onOpenWindow={(syntheticEvent) => {
                    ...
                    console.log('Blocked Germany popup window:', nativeEvent.targetUrl);
              ...
              >
                ...
                {shareCardPlay && (() => {
                  ...
                  if (!contextTeamColor && scoringTeam) {
                    ...
                    teamColor = GermanyServiceEnhanced.getTeamColorWithAlternateLogic(scoringTeam) || teamColor;
