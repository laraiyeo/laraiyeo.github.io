...
const FranceGameDetailsScreen = ({ route, navigation }) => {
  ...
  const updatePlaysDataIncremental = (fetchedPlays) => {
    ...
    setPlaysData((prevPlays) => {
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log(
          "[FranceGameDetails] incremental update: added=",
          addedCount,
          "patched=",
          patchedCount
        );
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      ...
      if (isLive) {
        console.log(
          "Stream modal closed, immediately fetching France game data"
        );
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log("[FranceGameDetails] loadGameDetails START", {
        gameId,
        silentUpdate,
      });
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log(
          "[FranceGameDetails] Game data updated - hash changed",
          currentHash
        );
      ...
      setLoading(false);
      console.log("[FranceGameDetails] loadGameDetails END", {
        gameId,
        silentUpdate,
      });
    } catch (error) {
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId
        ? FranceServiceEnhanced.getTeamLogoWithFallback(homeTeamId)
        : null,
      awayTeamId
        ? FranceServiceEnhanced.getTeamLogoWithFallback(awayTeamId)
        : null,
    ]);
  ...
  const renderMatchHeader = () => {
    ...
    // Get team colors
    const homeColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(
      homeTeam?.team
    );
    const awayColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(
      awayTeam?.team
    );
  ...
  const renderStatsTab = () => {
    ...
    let homeColor =
      FranceServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) ||
      "#007bff";
    let awayColor =
      FranceServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) ||
      "#28a745";
  ...
  const fetchLineupData = async () => {
    ...
    try {
      console.log(
        "[FranceGameDetails] Fetching lineup data for gameId:",
        gameId
      );
    ...
    try {
      ...
      try {
        ...
        if (coreResp.ok) {
          ...
          console.log(
            "[FranceGameDetails] core event resource responded with",
            coreResp.status
          );
        }
      } catch (coreErr) {
        console.log(
          "[FranceGameDetails] Error fetching core event resource:",
          coreErr
        );
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log(
              "[FranceGameDetails] statRef fetch failed",
              statRef,
              sResp.status
            );
          ...
          console.log(
            "[FranceGameDetails] Failed to fetch statRef for competitor",
            comp?.id,
            err
          );
          return { comp, rawText: null, parsed: null };
      ...
      try {
        teams.forEach((team) => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === "object") {
              ...
              console.log(
                `[FranceGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`
              );
            }
          }
        });
        ...
        console.log(
          "[FranceGameDetails] Error attaching parsed stats to teams:",
          attachErr
        );
      }
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (isUserScrollingRef.current) {
      console.log(
        "[FranceGameDetails] skipping plays fetch - user is scrolling"
      );
      return;
    }
    ...
    if (playsFetchingRef.current) {
      console.debug(
        "[FranceGameDetails] plays fetch already in progress - skipping"
      );
      return;
    }
    ...
    console.log("[FranceGameDetails] fetchPlaysData START", {
      gameId,
      lastUpdateHash,
    ...
    try {
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log(
          "[FranceGameDetails] plays changed - applying incremental update",
        ...
        console.debug(
          "[FranceGameDetails] plays hash unchanged - skipping merge"
        );
      }

      console.log(
        "[FranceGameDetails] fetchPlaysData END - items",
        fetchedPlays.length
      );
    } catch (error) {
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log(
          "[FranceGameDetails] user stopped scrolling - updates will resume"
        );
      }, 600);
    };
                ...
                {currentStreamType && availableStreams[currentStreamType] && (
                  <WebView
                    ...
                    onShouldStartLoadWithRequest={(request) => {
                      console.log(
                        "France WebView navigation request:",
                        request.url
                      );
                      ...
                      if (requestDomain !== currentDomain || hasPopupKeywords) {
                        console.log(
                          "Blocked France popup/cross-domain navigation:",
                          request.url
                        );
                        return false;
                      }
