...
const FranceGameDetailsScreen = ({ route, navigation }) => {
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl = await FranceServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  let patchedCount = 0;
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log('[FranceGameDetails] incremental update: added=', addedCount, 'patched=', patchedCount);
        return updated;
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (showStreamModal) {
          console.log('Stream modal open, skipping France game update');
          return;
        }
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('[FranceGameDetails] useFocusEffect triggered');
      loadGameDetails();
  ...
  useEffect(() => {
    console.log('[FranceGameDetails] gameId changed - clearing data states');
    setPlaysData(null);
    setStatsData(null);
  }, [gameId]);
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      const isLive = gameData && gameData.header?.competitions?.[0]?.status?.type?.state === 'in';
      if (isLive) {
        console.log('Stream modal closed, immediately fetching France game data');
      ...
      if (scorerData?.id && shareCardPlay.team?.$ref) {
        try {
          ...
          if (teamResponse.ok) {
            ...
            if (gameId) {
              const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/fra.1/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${scorerData.id}/statistics/0?lang=en&region=us`;
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log('[FranceGameDetails] loadGameDetails START', { gameId, silentUpdate });
      console.debug('[FranceGameDetails] requesting game details for', gameId);
      const data = await FranceServiceEnhanced.getGameDetails(gameId);
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log('[FranceGameDetails] Game data updated - hash changed', currentHash);
        ...
        console.log('[FranceGameDetails] Lineup data updated:', lineupResult);
        
        // Do not forcibly clear playsData here; the plays effect will compare hashes and merge/refresh
      } else {
        console.debug('[FranceGameDetails] Game data hash unchanged');
      }

      setLoading(false);
      console.log('[FranceGameDetails] loadGameDetails END', { gameId, silentUpdate });
    } catch (error) {
      console.error('Error loading France game details:', error);
      if (!silentUpdate) {
        setLoading(false);
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId ? FranceServiceEnhanced.getTeamLogoWithFallback(homeTeamId) : null,
      awayTeamId ? FranceServiceEnhanced.getTeamLogoWithFallback(awayTeamId) : null
    ]);
  ...
  const fetchPlayerGameStats = async (playerId, teamId) => {
    ...
    try {
      ...
      const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/fra.1/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${playerId}/statistics/0?lang=en&region=us`;
  ...
  const renderMatchHeader = () => {
    ...
    // Get team colors
    const homeColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team);
    const awayColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team);
      ...
      <View style={[styles.headerContainer, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.competitionContainer}>
          <Text allowFontScaling={false} style={[styles.competitionText, { color: theme.textSecondary }]}>
            {gameData.competitionName || 'France'}
          </Text>
        </View>
  ...
  const renderStatsTab = () => {
    ...
    const getStat = (team, statName) => {
      try {
        ...
        console.log('[FranceGameDetails] getStat error:', err);
      }
      return 0;
    };
    ...
    let homeColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) || '#007bff';
    let awayColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) || '#28a745';
  ...
  const fetchLineupData = async () => {
    ...
    try {
      console.log('[FranceGameDetails] Fetching lineup data for gameId:', gameId);
      ...
      console.log('[FranceGameDetails] Lineup API response:', data);
      ...
      console.log('[FranceGameDetails] Lineup data extracted:', { 
      ...
      console.error('[FranceGameDetails] fetchLineupData error:', error);
  ...
  const mapTeamStats = (flattenedStats = []) => {
    try {
      ...
      console.log('[FranceGameDetails] mapTeamStats error:', err);
      return {};
    }
  };
    ...
    try {
      ...
      try {
        ...
        if (coreResp.ok) {
          ...
          console.log('[FranceGameDetails] core event resource responded with', coreResp.status);
        }
      } catch (coreErr) {
        console.log('[FranceGameDetails] Error fetching core event resource:', coreErr);
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log('[FranceGameDetails] statRef fetch failed', statRef, sResp.status);
          ...
          console.log('[FranceGameDetails] Failed to fetch statRef for competitor', comp?.id, err);
          return { comp, rawText: null, parsed: null };
        }
      });
        ...
        teams.forEach(team => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === 'object') {
              ...
              console.log(`[FranceGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`);
            }
          }
        });
      } catch (attachErr) {
        console.log('[FranceGameDetails] Error attaching parsed stats to teams:', attachErr);
      }
      ...
      console.error('[FranceGameDetails] fetchMatchStats error:', error);
      return null;
    }
  };
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (isUserScrollingRef.current) {
      console.log('[FranceGameDetails] skipping plays fetch - user is scrolling');
      return;
    }
    ...
    if (playsFetchingRef.current) {
      console.debug('[FranceGameDetails] plays fetch already in progress - skipping');
      return;
    }
    ...
    console.log('[FranceGameDetails] fetchPlaysData START', { gameId, lastUpdateHash, playsDataCount: playsData ? playsData.length : 0, silent });
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        console.warn('[FranceGameDetails] No plays data available in response');
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log('[FranceGameDetails] plays changed - applying incremental update', { playsHash, prev: lastPlaysHashRef.current });
        updatePlaysDataIncremental(fetchedPlays);
        lastPlaysHashRef.current = playsHash;
      } else {
        console.debug('[FranceGameDetails] plays hash unchanged - skipping merge');
      }

      console.log('[FranceGameDetails] fetchPlaysData END - items', fetchedPlays.length);
    } catch (error) {
      console.error('[FranceGameDetails] Error fetching plays data:', error);
      if (initialLoad) setPlaysData([]);
    } finally {
  ...
  useEffect(() => {
    console.log('[FranceGameDetails] Plays fetch useEffect triggered', {
    ...
    if (gameData && gameId && !playsData) {
      console.log('[FranceGameDetails] Fetching plays data for scorers box');
      fetchPlaysDataInternal({ silent: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log('[FranceGameDetails] user stopped scrolling - updates will resume');
      }, 600);
    };
    ...
    const renderedPlays = visiblePlays.map((play, index) => {
          ...
          if (playTeamId) {
            if (String(playTeamId) === String(awayId)) {
              teamSide = 'away';
              teamColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team || awayTeam) || '#28a745';
            } else if (String(playTeamId) === String(homeId)) {
              teamSide = 'home';
              teamColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team || homeTeam) || '#007bff';
            }
          }
  ...
  const renderMiniField = (coordinate, coordinate2, eventType = 'gen', teamSide = 'home', teamColor = '#007bff') => {
    ...
    const renderPlayerPopup = () => {
      ...
      if (selectedPlayer.teamType === 'home') {
        teamColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(homeTeamData?.team) || '#007bff';
      } else if (selectedPlayer.teamType === 'away') {
        teamColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(awayTeamData?.team) || '#28a745';
      }
              ...
              {currentStreamType && availableStreams[currentStreamType] && (
                <WebView
                  ...
                  onShouldStartLoadWithRequest={(request) => {
                    console.log('France WebView navigation request:', request.url);
                    ...
                    if (requestDomain !== currentDomain || hasPopupKeywords) {
                      console.log('Blocked France popup/cross-domain navigation:', request.url);
                      return false;
                    }
                  ...
                  onOpenWindow={(syntheticEvent) => {
                    ...
                    console.log('Blocked France popup window:', nativeEvent.targetUrl);
              ...
              >
                ...
                {shareCardPlay && (() => {
                  ...
                  if (!contextTeamColor && scoringTeam) {
                    ...
                    teamColor = FranceServiceEnhanced.getTeamColorWithAlternateLogic(scoringTeam) || teamColor;
