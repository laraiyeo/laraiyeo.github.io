...
import { FranceService } from '../../../services/soccer/FranceService';
...
const FranceScreen = () => {
  ...
  const leagueId = route.params?.leagueId || 'france';
  const leagueName = route.params?.leagueName || 'France';
  ...
  const loadMatches = async () => {
    try {
      const data = await FranceService.getScoreboard();
  ...
  const loadStandings = async () => {
    try {
      const data = await FranceService.getStandings();
