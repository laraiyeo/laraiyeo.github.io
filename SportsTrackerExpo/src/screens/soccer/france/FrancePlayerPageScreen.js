...
const FrancePlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "FrancePlayerPageScreen received - playerId:",
      playerId,
      "playerName:",
      playerName,
      "teamId:",
    ...
    try {
      ...
      try {
        ...
        const siteResponse = await fetch(
          convertToHttps(
            `https://site.api.espn.com/apis/site/v2/sports/soccer/fra.1/athletes/${playerId}`,
          ),
          {
            method: "GET",
            headers: {
      ...
      try {
        const coreResponse = await fetch(
          convertToHttps(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/fra.1/athletes/${playerId}?lang=en&region=us`,
          ),
        );
        if (coreResponse.ok) {
    ...
    try {
      ...
      const statsPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(
            `Validating France player stats data for ${competition.name}:`,
            statsData,
          );
  ...
  const fetchGameLog = async () => {
    ...
    try {
      ...
      // Fetch game logs from all France competitions with year fallback
      const fetchPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(
            `Validating France game log data for ${competition}:`,
            gameLogData,
          );
