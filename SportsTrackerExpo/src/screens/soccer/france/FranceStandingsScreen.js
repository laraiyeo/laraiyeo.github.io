...
const FranceStandingsScreen = ({ navigation, route }) => {
  ...
  const loadStandings = async () => {
    try {
      setLoading(true);
      console.log('Loading France standings...');
      const data = await FranceServiceEnhanced.getStandings();
      ...
      if (data.children && data.children.length > 0) {
        ...
        const processedStandings = await Promise.all(
          standingsData.map(async (team) => {
            const logo = await FranceServiceEnhanced.getTeamLogoWithFallback(team.team.id);
            return { ...team, logo };
          })
        );
      ...
      console.error('Error loading France standings:', error);
      setLoading(false);
    }
  };
...
export default FranceStandingsScreen;
