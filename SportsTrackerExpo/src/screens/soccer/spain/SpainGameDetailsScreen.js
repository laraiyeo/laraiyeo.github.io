...
const HeaderGradient = ({ awayColor, homeColor, theme, height }) => (
  <View style={[StyleSheet.absoluteFill, { height }]} pointerEvents="none">
    ...
    >
      <Defs>
        <LinearGradient id="spainHeaderGrad" x1="00%" y1="0%" x2="100%" y2="0%">
      ...
      <Rect width={width} height={height} fill="url(#spainHeaderGrad)" />
    </Svg>
  </View>
);
...
const SPAINGameDetailsScreen = ({ route, navigation }) => {
      ...
      if (
        ...
        if (gameId && contextTeamId) {
          try {
            ...
            const rosterUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/events/${gameId}/competitions/${gameId}/competitors/${contextTeamId}/roster?lang=en&region=us`;
            ...
            if (foundAthleteId) {
              const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/events/${gameId}/competitions/${gameId}/competitors/${contextTeamId}/roster/${foundAthleteId}/statistics/0?lang=en&region=us`;
              console.log(
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl =
      await SpainServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      const isLive =
        ...
        gameData.header?.competitions?.[0]?.status?.type?.state === "in";
      if (isLive) {
        console.log("Stream modal closed, immediately fetching SPAIN game data");
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log("[SPAINGameDetails] loadGameDetails START", {
        gameId,
        silentUpdate,
      });
      ...
      setLoading(false);
      console.log("[SPAINGameDetails] loadGameDetails END", {
        gameId,
        silentUpdate,
      });
    } catch (error) {
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId
        ? SpainServiceEnhanced.getTeamLogoWithFallback(homeTeamId)
        : null,
      awayTeamId
        ? SpainServiceEnhanced.getTeamLogoWithFallback(awayTeamId)
        : null,
    ]);
  ...
  const fetchPlayerGameStats = async (playerId, teamId) => {
    ...
    try {
      ...
      const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${playerId}/statistics/0?lang=en&region=us`;
      ...
      >
        ...
        <View style={styles.simpleLeagueRow}>
          <Text
            ...
            numberOfLines={1}
          >
            {gameData.competitionName || "SPAIN World Cup"}
          </Text>
        </View>
            ...
            <View style={styles.simpleTeamInfo}>
              <Text
                ...
                style={[
                  ...
                  {
                    color: isFavorite(homeTeamData?.team?.id, "laliga")
                      ? colors.primary
                      : theme.text,
                    opacity: matchStatus.isPost ? (homeIsWinner ? 1 : 0.55) : 1,
                  },
                ...
                numberOfLines={2}
              >
                {isFavorite(homeTeamData?.team?.id, "laliga") ? "★ " : ""}
            ...
            <View style={styles.simpleTeamInfo}>
              <Text
                ...
                style={[
                  styles.simpleTeamName,
                  {
                    color: isFavorite(awayTeamData?.team?.id, "laliga")
                      ? colors.primary
                      : theme.text,
                    opacity: matchStatus.isPost ? (awayIsWinner ? 1 : 0.55) : 1,
                  },
                ...
                numberOfLines={2}
              >
                {isFavorite(awayTeamData?.team?.id, "laliga") ? "★ " : ""}
        ...
        {/* Scorers Box (SPAIN-specific addition) */}
  ...
  const renderStatsTab = () => {
    ...
    let homeColor =
      SpainServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) ||
      "#007bff";
    let awayColor =
      SpainServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) ||
      "#28a745";
  ...
  const fetchLineupData = async (sourceData = null) => {
    ...
    try {
      ...
      console.log("[SPAINGameDetails] Extracted lineups from summary:", {
