...
const SpainGameDetailsScreen = ({ route, navigation }) => {
  ...
  // Track game presence for live viewer count
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (showStreamModal) {
          console.log('Stream modal open, skipping Spain game update');
          return;
        }
      ...
      if (scorerData?.id && shareCardPlay.team?.$ref) {
        try {
          ...
          if (teamResponse.ok) {
            ...
            if (gameId) {
              const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${scorerData.id}/statistics/0?lang=en&region=us`;
  ...
  const renderMiniField = (coordinate, coordinate2, eventType = 'gen', teamSide = 'home', teamColor = '#007bff') => {
    ...
    const renderPlayerPopup = () => {
      ...
      if (selectedPlayer.teamType === 'home') {
        teamColor = SpainServiceEnhanced.getTeamColorWithAlternateLogic(homeTeamData?.team) || '#007bff';
      } else if (selectedPlayer.teamType === 'away') {
        teamColor = SpainServiceEnhanced.getTeamColorWithAlternateLogic(awayTeamData?.team) || '#28a745';
      }
              ...
              {currentStreamType && availableStreams[currentStreamType] && (
                <WebView
                  ...
                  onShouldStartLoadWithRequest={(request) => {
                    console.log('Spain WebView navigation request:', request.url);
                    ...
                    if (requestDomain !== currentDomain || hasPopupKeywords) {
                      console.log('Blocked Spain popup/cross-domain navigation:', request.url);
                      return false;
                    }
                  ...
                  onOpenWindow={(syntheticEvent) => {
                    ...
                    console.log('Blocked Spain popup window:', nativeEvent.targetUrl);
              ...
              >
                ...
                {shareCardPlay && (() => {
                  ...
                  if (!contextTeamColor && scoringTeam) {
                    ...
                    teamColor = SpainServiceEnhanced.getTeamColorWithAlternateLogic(scoringTeam) || teamColor;
