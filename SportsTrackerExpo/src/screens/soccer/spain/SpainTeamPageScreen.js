...
const SpainTeamPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "SpainTeamPageScreen received - teamId:",
      teamId,
      "teamName:",
      teamName,
    );
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      const spanishCompetitions = [
        "esp.1",
        "esp.copa_del_rey",
        "esp.super_cup",
      ];

      const competitionPromises = spanishCompetitions.map(
        async (leagueCode) => {
          try {
            ...
            if (!isValidData) {
              console.log(
                `No events data found for ${leagueCode} team ${teamId} in any year`,
              );
              return [];
            }
      ...
      if (allEvents.length > 0) {
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, "la liga")) {
              const eventId =
                foundCurrentGame.id ||
              ...
              console.log(
                "SpainTeamPageScreen: team is favorited - updating persisted currentGame:",
                currentGamePayload,
              );
  ...
  const fetchRoster = async () => {
    ...
    try {
      const year = getDomesticLeagueYear();
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/teams/${teamId}/roster?season=${year}`,
      );
    ...
    try {
      ...
      const statsPromises = seasonTypes.map(async (seasonType) => {
        try {
          const year = getDomesticLeagueYear();
          const response = await fetch(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${seasonType.leagueCode}/seasons/${year}/types/1/teams/${teamId}/statistics?lang=en&region=us`,
          );
          ...
          console.log(`Found stats for team ${teamId} in ${seasonType.name}`);
  ...
  const renderTeamHeader = () => {
    ...
    const isTeamFavorite = isFavorite(teamData.id, "la liga");
