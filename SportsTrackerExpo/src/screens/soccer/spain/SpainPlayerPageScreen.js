...
const SpainPlayerPageScreen = ({ route, navigation }) => {
  ...
  const getTeamLogoUrl = (teamId, useDarkMode = isDarkMode) => {
    if (!teamId) {
      // Fallback to soccer.png in assets
      return require('../../../../assets/soccer.png');
    }
    
    // For dark mode: try 500-dark first, then 500
    // For light mode: try 500 first, then 500-dark
    if (useDarkMode) {
      return `https://a.espncdn.com/i/teamlogos/soccer/500-dark/${teamId}.png`;
    } else {
  ...
  const fetchPlayerTransactions = async (currentPlayerData = null) => {
    try {
      ...
      const transactionResponse = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/athletes/${playerId}/transactions?lang=en&region=us`);
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const basicPlayerData = {
        ...
        team: { 
          id: teamId, 
          displayName: 'Real Madrid', // Better default for esp.1 teams
          name: 'Real Madrid'
        },
      ...
      try {
        ...
        const siteResponse = await fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/athletes/${playerId}`, {
          method: 'GET',
          headers: {
        ...
        if (siteResponse.ok) {
          ...
          if (siteData && siteData.athlete) {
            ...
            // Load league teams for VS Team tab (this can fail silently)
            try {
              await fetchLeagueTeams();
            } catch (teamsError) {
              console.log('Failed to load league teams:', teamsError.message);
            }
      ...
      try {
        const coreResponse = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/athletes/${playerId}?lang=en&region=us`);
        if (coreResponse.ok) {
          ...
          if (coreData.team) {
            ...
            if (typeof coreData.team === 'object' && coreData.team.$ref) {
              try {
                const teamResponse = await fetch(coreData.team.$ref);
                if (teamResponse.ok) {
  ...
  const fetchLeagueTeams = async () => {
    try {
      console.log('Fetching league teams for esp.1');
      
      // Try site API first as it's more reliable
      const siteResponse = await fetch('https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/teams');
      if (siteResponse.ok) {
        const siteData = await siteResponse.json();
        if (siteData && siteData.sports && siteData.sports[0] && siteData.sports[0].leagues && siteData.sports[0].leagues[0] && siteData.sports[0].leagues[0].teams) {
          const teams = siteData.sports[0].leagues[0].teams.map(team => team.team);
          setLeagueTeams(teams);
          console.log('League teams loaded from site API:', teams.length);
          return;
        }
      }
      
      // Fallback to core API
      const coreResponse = await fetch('https://core.api.espn.com/v2/sports/soccer/leagues/esp.1/teams?lang=en&region=us&limit=50');
      if (coreResponse.ok) {
        const coreData = await coreResponse.json();
        
        if (coreData && coreData.items) {
          // For core API, we need to fetch each team individually (but limit to first 10 to avoid too many requests)
          const teamPromises = coreData.items.slice(0, 10).map(async (teamRef) => {
            try {
              ...
              console.error('Error fetching individual team:', error);
              return null;
            }
          });
          
          const teamsData = await Promise.all(teamPromises);
          const validTeams = teamsData.filter(team => team && team.id);
          setLeagueTeams(validTeams);
          console.log('League teams loaded from core API:', validTeams.length);
        }
      } else {
        console.log('Core API teams request failed');
        // Set some default La Liga teams as fallback
        setLeagueTeams([
          { id: '86', displayName: 'Real Madrid', abbreviation: 'RMA' },
          { id: '83', displayName: 'FC Barcelona', abbreviation: 'BAR' },
          { id: '93', displayName: 'Atletico Madrid', abbreviation: 'ATM' },
          { id: '95', displayName: 'Valencia', abbreviation: 'VAL' },
          { id: '244', displayName: 'Sevilla', abbreviation: 'SEV' }
        ]);
      }
    } catch (error) {
  ...
  const fetchGameLog = async () => {
    ...
    try {
      ...
      // Use the exact same eventlog API URL pattern as fantasy.js
      const gameLogUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/seasons/${currentYear}/athletes/${playerId}/eventlog?lang=en&region=us&played=true`;
      ...
      try {
        ...
        if (gameLogResponse.ok) {
          ...
          console.log('Game log API response:', gameLogData);
          ...
          if (gameLogData && gameLogData.events && gameLogData.events.items) {
            ...
            console.log('Found events.items array with', gameLogData.events.count, 'total events');
          ...
          if (eventsToProcess && Array.isArray(eventsToProcess)) {
            ...
            // Process game log exactly like fantasy.js
            const processedGameLog = await processFantasyGameLog(eventsToProcess);
            setGameLog(processedGameLog);
            console.log('Game log loaded:', processedGameLog.length, 'games');
          } else {
            console.log('No valid events array found in game log response, structure:', typeof gameLogData);
            setGameLog([]);
          }
        } else {
          console.log('Game log API failed with status:', gameLogResponse.status);
          setGameLog([]);
        }
      } catch (error) {
  ...
  const processFantasyGameLog = async (events) => {
    ...
    // Process events - we need to fetch the $ref data
    ...
    for (const eventData of events) {
      try {
        ...
        if (eventData && eventData.event && eventData.competition) {
          ...
          // Fetch event details
          const eventResponse = await fetch(`${eventData.event.$ref}?lang=en&region=us`);
          if (!eventResponse.ok) {
          ...
          const competitionResponse = await fetch(`${eventData.competition.$ref}?lang=en&region=us`);
          if (!competitionResponse.ok) {
            console.log('Failed to fetch competition details');
            continue;
          }
          ...
          let stats = {};
          if (eventData.statistics && eventData.statistics.$ref) {
            try {
              const statsResponse = await fetch(`${eventData.statistics.$ref}?lang=en&region=us`);
              if (statsResponse.ok) {
          ...
          const gameEntry = {
            gameId: event.id,
            date: event.date,
            opponent: await getOpponentFromCompetition(competition, eventData.teamId),
    ...
    return gameLogEntries.sort((a, b) => new Date(b.date) - new Date(a.date));
  };
    ...
    if (competition && competition.competitors) {
      ...
      if (opponent && opponent.team && opponent.team.$ref) {
        try {
          ...
          const teamResponse = await fetch(`${opponent.team.$ref}?lang=en&region=us`);
          if (teamResponse.ok) {
            const teamData = await teamResponse.json();
  ...
  const loadPlayerStatsForYear = async (playerId, position, year) => {
    try {
      ...
      if (year !== new Date().getFullYear()) {
        try {
          const playerSeasonResponse = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/${leagueForYear}/seasons/${year}/athletes/${playerId}?lang=en&region=us`);
      ...
      if (teamsForSeason.length === 0) {
        try {
          const espnResponse = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/${leagueForYear}/seasons/${year}/athletes/${playerId}?lang=en&region=us`);
          if (espnResponse.ok) {
      ...
      const careerPromises = yearsToFetch.map(async (year) => {
        try {
          ...
          if (teamsForSeason.length === 0) {
            ...
            const leaguesToTry = ['esp.1', 'fra.1', 'eng.1', 'ger.1', 'ita.1', 'usa.1', 'ksa.1'];
            
            const playerSeasonPromises = leaguesToTry.map(async (league) => {
              try {
                const playerSeasonResponse = await fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/${league}/seasons/${year}/athletes/${playerId}?lang=en&region=us`);
                if (playerSeasonResponse.ok) {
                  const playerSeasonData = await playerSeasonResponse.json();
          ...
          const teamStatsPromises = teamsForSeason.map(async (teamInfo, index) => {
            try {
              ...
              const [statsResponse, teamResponse, competitions] = await Promise.allSettled([
                fetch(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/${teamInfo.league}/seasons/${year}/types/0/athletes/${playerId}/statistics?lang=en&region=us`),
                fetch(`https://site.api.espn.com/apis/site/v2/sports/soccer/${teamInfo.league}/teams/${teamInfo.teamId}`),
  ...
  const fetchCompetitionsForYear = async (playerId, leagueCode, year) => {
    ...
    for (const competition of leagueCompetitions) {
      try {
        const compUrl = `http://sports.core.api.espn.com/v2/sports/soccer/leagues/${competition.code}/seasons/${year}/types/0/athletes/${playerId}/statistics?lang=en&region=us`;
  ...
  const fetchVsTeamStats = async () => {
    // TODO: Implement vs team stats
    console.log('Fetching vs team stats - to be implemented');
  };
          ...
          <View style={styles.playerDetailsRow}>
            {playerData.team && playerData.team.id && (
              <View style={styles.teamContainer}>
                <Image 
                  source={{ uri: getTeamLogoUrl(playerData.team.id) }}
            ...
            gameLog.map((game, index) => (
              ...
              >
                ...
                <View style={styles.mlbPlayerRow}>
                  ...
                  {/* Win Indicator */}
                  <View style={[styles.mlbWinIndicator, { backgroundColor: colors.primary }]}>
                    <Text style={[styles.mlbWinText, { color: 'white' }]}>W</Text>
                  </View>
                </View>
                ...
                <View style={[styles.mlbMatchRow, { borderTopColor: theme.surfaceSecondary }]}>
                  <View style={styles.mlbTeamLogos}>
                    <Image 
                      source={{ uri: getTeamLogoUrl(playerData?.team?.id || teamId, isDarkMode) }}
                    ...
                    <Image 
                      source={{ uri: getTeamLogoUrl(game.opponent?.id || '86', isDarkMode) }}
                      style={styles.mlbTeamLogo}
                    />
                  </View>
      ...
      >
        <View style={styles.careerSeasonHeader}>
          <View style={styles.careerTeamInfo}>
            <Image 
              source={{ uri: getTeamLogoUrl(season.team.id, isDarkMode) }}
              ...
              <View style={styles.modalSeasonTopRow}>
                <View style={styles.modalSeasonInfo}>
                  ...
                  <Text style={[styles.modalLeagueName, { color: theme.textSecondary }]}>
                    {season.league === 'esp.1' ? 'La Liga' : season.league}
                  </Text>
                </View>
  ...
  const renderVsTeamContent = () => {
    return (
      <ScrollView style={[styles.statsContainer, { backgroundColor: theme.background }]} showsVerticalScrollIndicator={false}>
        <View style={styles.statsContent}>
          <Text style={[styles.contentText, { color: theme.textSecondary }]}>
            VS Team content - to be implemented
          </Text>
        </View>
      </ScrollView>
    );
          ...
          <ScrollView style={styles.modalContent}>
            {selectedGameStats && (
              <View style={styles.modalStatsContainer}>
                ...
                {renderGameStatsDetails(selectedGameStats)}
              </View>
            )}
          </ScrollView>
        </View>
