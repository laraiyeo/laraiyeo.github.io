...
const SpainPlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "SpainPlayerPageScreen received - playerId:",
      playerId,
      "playerName:",
      playerName,
      "teamId:",
    ...
    try {
      ...
      try {
        ...
        const siteResponse = await fetch(
          convertToHttps(
            `https://site.api.espn.com/apis/site/v2/sports/soccer/esp.1/athletes/${playerId}`,
          ),
          {
            method: "GET",
            headers: {
      ...
      try {
        const coreResponse = await fetch(
          convertToHttps(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/esp.1/athletes/${playerId}?lang=en&region=us`,
          ),
        );
        if (coreResponse.ok) {
    ...
    try {
      ...
      for (const competition of competitions) {
        try {
          ...
          console.log(
            `Validating ${competition.name} player stats data:`,
            statsData,
          );
          ...
          if (hasValidCategories) {
            ...
            console.log(
              `Successfully fetched ${competition.name} stats with ${statsData.splits.categories.length} categories`,
            );
          } else if (hasValidSplits) {
            console.log(
              `${competition.name} has splits but no statistics data - checking categories structure:`,
              statsData.splits.categories.map((cat) => ({
                name: cat.name,
            ...
            console.log(
              `No valid stats data found for ${competition.name} - missing splits or categories`,
            );
          }
        } catch (error) {
        ...
        if (category.stats) {
          ...
          category.stats.forEach((stat) => {
            ...
            if (
              ...
              console.log(
                `Skipped stat ${stat.name} - no value or empty value:`,
                stat.value,
              );
            }
          });
    ...
    try {
      ...
      const fetchPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(
            `Validating Spain game log data for ${competition}:`,
            gameLogData,
          );
          ...
          if (isValidData) {
            ...
            console.log(`No game log data found for ${competition}`);
          }
        } catch (error) {
  ...
  const fetchCareerData = async () => {
    ...
    try {
      ...
      // Merge transfer pairs that belong to the same league + season
      const mergedCareerStats = [];
      const usedIndices = new Set();

      for (let i = 0; i < careerStats.length; i++) {
        if (usedIndices.has(i)) continue;
        const current = careerStats[i];

        // Only attempt to pair explicit transfer entries (from/to)
        if (
          current.isTransferYear &&
          (current.transferPeriod === "from" || current.transferPeriod === "to")
        ) {
          // Find the matching transfer entry for the same league+season with the opposite period
          console.log(
            `Attempting to pair transfer entry idx=${i} season=${current.displaySeason} league=${current.league} teamId=${current?.team?.id || current?.team?.teamId || "unknown"} transferPeriod=${current.transferPeriod} transferDate=${current.transferDate}`,
          );
          const partnerIndex = careerStats.findIndex((other, idx) => {
            if (idx === i) return false;
            if (usedIndices.has(idx)) return false;
            if (!other || !other.league || !other.season) return false;
            // require opposite transferPeriod and different team ids when available
            if (
              !(
                (current.transferPeriod === "from" &&
                  other.transferPeriod === "to") ||
                (current.transferPeriod === "to" &&
                  other.transferPeriod === "from")
              )
            )
              return false;
            if (
              other.league !== current.league ||
              other.season !== current.season
            )
              return false;
            if (other.team && current.team && other.team.id === current.team.id)
              return false;
            return true;
          });

          if (partnerIndex !== -1) {
            const partner = careerStats[partnerIndex];
            console.log(
              `Found pair: idx=${i} <-> idx=${partnerIndex} season=${current.displaySeason} league=${current.league} fromId=${current.transferPeriod === "from" ? current?.team?.id : partner?.team?.id} toId=${current.transferPeriod === "to" ? current?.team?.id : partner?.team?.id}`,
            );
            // Ensure `fromTeam` is the team with transferPeriod === 'from'
            const fromEntry =
              current.transferPeriod === "from" ? current : partner;
            const toEntry = current.transferPeriod === "to" ? current : partner;

            const merged = {
              ...fromEntry,
              transferPair: true,
              fromTeam: fromEntry.team,
              toTeam: toEntry.team,
              // Keep `team` pointing at the fromTeam to preserve downstream lookups
              team: fromEntry.team,
              // Merge competitions/statistics in a conservative way
              competitions: [
                ...(fromEntry.competitions || []),
                ...(toEntry.competitions || []),
              ],
            };

            mergedCareerStats.push(merged);
            usedIndices.add(i);
            usedIndices.add(partnerIndex);
            continue;
          } else {
            // No partner found — log why by scanning potential same-season/same-league entries
            const reasons = careerStats
              .map((other, idx) => {
                ...
                if (usedIndices.has(idx))
                  return { idx, reason: "already used" };
                const samePeriod =
                  other.transferPeriod === current.transferPeriod;
                const sameTeam =
                  other.team &&
                  current.team &&
                  other.team.id === current.team.id;
                return {
                  idx,
                  teamId: other?.team?.id || other?.team?.teamId || "unknown",
                  transferPeriod: other.transferPeriod,
                  transferDate: other.transferDate,
                  reason: sameTeam
                    ? "same-team"
                    : samePeriod
                      ? "same-transfer-period"
                      : "other-mismatch",
                };
              })
              .filter(Boolean);
            console.log(
              `No partner found for idx=${i} season=${current.displaySeason} league=${current.league}. Candidates:`,
              reasons,
            );
          }
        }

        // No explicit partner found — keep entry as-is
        mergedCareerStats.push(current);
        usedIndices.add(i);
      }

      // As a safeguard, remove exact duplicate entries (same league, season and team id)
      const finalCareer = [];
      const seenKeys = new Set();
      for (const s of mergedCareerStats) {
        const teamId = s.team && s.team.id ? s.team.id : JSON.stringify(s.team);
        const key = `${s.league}__${s.season}__${teamId}`;
        if (!seenKeys.has(key)) {
          finalCareer.push(s);
          seenKeys.add(key);
        }
      ...
      >
        <View style={styles.statsContent}>
          {Object.entries(playerStats).map(
            ([competitionCode, competitionData]) =>
