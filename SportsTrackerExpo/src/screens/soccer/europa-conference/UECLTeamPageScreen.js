...
const UECLTeamPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "UECLTeamPageScreen received - teamId:",
      teamId,
      "teamName:",
      teamName,
    );
  ...
  const fetchTeamRecord = async (teamId) => {
    try {
      ...
      const standingsData =
        await EuropaConferenceLeagueServiceEnhanced.getStandings();
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, "uefa europa conf")) {
              ...
              console.log(
                "UECLTeamPageScreen: team is favorited - updating persisted currentGame:",
                currentGamePayload,
              );
  ...
  const fetchTeamStats = async () => {
    ...
    try {
      ...
      const seasonTypes = [
        ...
        {
          id: "7",
          name: "Europa Conference League Qualifiers",
          leagueCode: "uefa.europa.conf_qual",
        },
      ];
      ...
      const statsPromises = seasonTypes.map(async (seasonType) => {
        try {
          ...
          if (response.ok) {
            ...
            console.log(
              `Validating UECL team stats data for ${seasonType.name}:`,
              statsData,
            );
            const isValid =
              statsData &&
