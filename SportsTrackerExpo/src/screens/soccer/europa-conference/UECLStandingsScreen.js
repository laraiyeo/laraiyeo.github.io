...
const UECLStandingsScreen = ({ navigation, route }) => {
  ...
  const loadStandings = async () => {
    try {
      setLoading(true);
      console.log('Loading UECL standings...');
      const data = await EuropaConferenceLeagueServiceEnhanced.getStandings();
      ...
      if (data.children && data.children.length > 0) {
        ...
        const processedStandings = await Promise.all(
          standingsData.map(async (team) => {
            const logo = await EuropaConferenceLeagueServiceEnhanced.getTeamLogoWithFallback(team.team.id);
            return { ...team, logo };
          })
        );
      ...
      console.error('Error loading UECL standings:', error);
      setLoading(false);
    }
  };
...
export default UECLStandingsScreen;
