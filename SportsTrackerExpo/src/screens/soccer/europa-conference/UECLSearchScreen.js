...
const UECLSearchScreen = ({ route, navigation }) => {
  ...
  const searchTeams = async (query) => {
    try {
      const teamsData =
        await EuropaConferenceLeagueServiceEnhanced.searchTeams(query);
  ...
  const searchPlayers = async (query) => {
    try {
      const playersData =
        await EuropaConferenceLeagueServiceEnhanced.searchPlayers(query);
              ...
              if (teamInfoCache.has(playerData.teamId)) {
                ...
                try {
                  const teamResponse = await fetch(
                    convertToHttps(
                      `https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa_conference/teams/${playerData.teamId}`,
                    ),
                  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.searchHeader, { backgroundColor: theme.surface }]}>
        ...
        >
          Search for {(sport || "Europa Conference League").toUpperCase()} teams
          and players
        </Text>
      </View>
