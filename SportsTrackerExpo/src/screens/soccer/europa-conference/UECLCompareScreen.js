...
const UECLCompareScreen = ({ navigation, route }) => {
  ...
  // All Europa Conference League players cache
  const [allEuropaConferenceLeaguePlayers, setAllEuropaConferenceLeaguePlayers] = useState([]);
  ...
  useEffect(() => {
    fetchAllEuropaConferenceLeaguePlayers();
  }, []);
  ...
  const fetchAllEuropaConferenceLeaguePlayers = async () => {
    try {
      console.log('Fetching all Europa Conference League players...');
      const allPlayers = await EuropaConferenceLeagueServiceEnhanced.searchPlayers('');
      ...
      console.log(`Fetched ${validPlayers.length} valid Europa Conference League players for comparison (${allPlayers.length} total)`);
      setAllEuropaConferenceLeaguePlayers(validPlayers);
    } catch (error) {
      console.error('Error fetching all Europa Conference League players:', error);
    }
  };
  ...
  const searchPlayers = (query) => {
    ...
    const filteredPlayers = allEuropaConferenceLeaguePlayers
      .filter(player => {
  ...
  const fetchPlayerStats = async (playerId, year) => {
    try {
      ...
      // Define competitions like in UECLPlayerPageScreen
      const competitions = [
        { code: 'uefa.europa.conf', name: 'Europa Conference League', seasonType: '0' },
        { code: 'uefa.europa.conf_qual', name: 'Europa Conference League Qualifying', seasonType: '0' }
      ];
      ...
      for (const competition of competitions) {
        try {
          ...
          if (response.ok) {
            ...
            // Process the stats using the same logic as UECLPlayerPageScreen
  ...
  // Process player stats similar to UECLPlayerPageScreen
...
export default UECLCompareScreen;