...
const UECLBracketScreen = ({ navigation, route }) => {
  ...
  const fetchStandings = async () => {
    try {
      const leagueCode = LEAGUES["Europa Conference League"].code;
  ...
  const fetchKnockoutPlayoffs = async () => {
    try {
      const currentYear = getEuropaConferenceLeagueYear();
      const calendarData = await fetchScoreboardOnce(
        LEAGUES["Europa Conference League"].code,
        `${currentYear}0101`,
      );
            ...
            {matchups.map((matchup, index) => {
              ...
              return clickableRight ? (
                <TouchableOpacity
                  ...
                  onPress={() => {
                    console.log(
                      "UECL pairing right press -> r16Matchup",
                      r16Matchup?.homeTeam?.id,
                      r16Matchup?.awayTeam?.id,
                    );
