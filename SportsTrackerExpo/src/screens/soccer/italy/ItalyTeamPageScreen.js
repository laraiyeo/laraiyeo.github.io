...
const ItalyTeamPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "ItalyTeamPageScreen received - teamId:",
      teamId,
      "teamName:",
      teamName,
    );
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      const italianCompetitions = [
        "ita.1",
        "ita.coppa_italia",
        "ita.super_cup",
      ];

      const competitionPromises = italianCompetitions.map(
        async (leagueCode) => {
          try {
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, "serie a")) {
              ...
              console.log(
                "ItalyTeamPageScreen: team is favorited - updating persisted currentGame:",
                currentGamePayload,
              );
    ...
    try {
      ...
      const statsPromises = seasonTypes.map(async (seasonType) => {
        try {
          ...
          console.log(
            `Validating Italy team stats data for ${seasonType.name}:`,
            statsData,
          );
