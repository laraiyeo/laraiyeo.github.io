...
const ItalyTransferScreen = ({ navigation, route }) => {
  ...
  const loadTransferData = async () => {
    try {
      ...
      const response = await fetch(
        convertToHttps(
          "https://sports.core.api.espn.com/v2/sports/soccer/leagues/ita.1/transactions?limit=1000"
        )
      );
          ...
          data.items.map(async (transfer) => {
            try {
              ...
              const athleteResponse = await fetch(
                convertToHttps(transfer.athlete.$ref)
              );
              ...
              if (transfer.from) {
                const fromTeamResponse = await fetch(
                  convertToHttps(transfer.from.$ref)
                );
              ...
              let toTeamData = null;
              if (transfer.to) {
                const toTeamResponse = await fetch(
                  convertToHttps(transfer.to.$ref)
                );
    ...
    if (filterType !== "all") {
      filtered = filtered.filter((transfer) => {
        switch (filterType) {
          ...
          case "permanent":
            return (
              transfer.type.toLowerCase() === "permanent" ||
    ...
    return (
      <View style={[styles.transferItem, { backgroundColor: theme.surface }]}>
        <View style={styles.transferHeader}>
          ...
          <Text
            allowFontScaling={false}
            style={[styles.transferDate, { color: theme.textSecondary }]}
          >
            {formattedDate}
          </Text>
        </View>
        ...
        <View style={styles.transferDetails}>
          <View
            style={[
              styles.transferType,
              { backgroundColor: getTransferTypeColor(item.type) + "20" },
            ]}
          >
            <Text
              allowFontScaling={false}
              style={[
                ...
                { color: getTransferTypeColor(item.type) },
              ]}
            >
              {item.type}
            </Text>
          ...
          {shouldShowAmountBubble(item.amount, item.type) ? (
            <View
              style={[
                styles.transferAmountBubble,
                { backgroundColor: colors.primary + "20" },
              ]}
            >
              <Text
                allowFontScaling={false}
                style={[styles.transferAmountText, { color: colors.primary }]}
              >
            ...
            <Text
              allowFontScaling={false}
              style={[styles.transferAmount, { color: colors.primary }]}
            >
              {getAmountDisplay(item.amount, item.displayAmount, item.type)}
            </Text>
          )}
  ...
  const renderPagination = () => {
    ...
    return (
      <View style={styles.paginationContainer}>
        ...
        <Text
          allowFontScaling={false}
          style={[styles.pageInfo, { color: theme.text }]}
        >
        ...
        <TouchableOpacity
          style={[
            ...
            {
              backgroundColor:
                currentPage < totalPages ? colors.primary : theme.border,
            },
          ]}
          ...
          {[
            ...
            <TouchableOpacity
              ...
              style={[
                ...
                {
                  backgroundColor:
                    filterType === filter.key ? colors.primary : theme.border,
                  borderColor:
                    filterType === filter.key ? colors.primary : theme.border,
                },
              ]}
...
export default ItalyTransferScreen;
