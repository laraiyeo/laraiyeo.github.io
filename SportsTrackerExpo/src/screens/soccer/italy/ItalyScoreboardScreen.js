...
const ItalyScoreboardScreen = ({ navigation, route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      return () => {
        console.log(
          "ItalyScoreboardScreen: Screen unfocused, clearing intervals",
        );
  ...
  useEffect(() => {
    console.log(
      "ItalyScoreboardScreen: Main useEffect triggered for filter:",
      selectedDateFilter,
      "focused:",
      isScreenFocused,
    );
  ...
  useEffect(() => {
    console.log(
      "ItalyScoreboardScreen: Preload useEffect triggered, hasPreloaded:",
  ...
  const loadScoreboard = async (
    ...
    console.log(
      "ItalyScoreboardScreen: loadScoreboard called - silentUpdate:",
      silentUpdate,
      "dateFilter:",
      dateFilter,
    );
    ...
    try {
      ...
      const processedGames = await Promise.all(
        (data.events || []).map(async (game) => {
          // Get team logos
          const awayLogo = await ItalyServiceEnhanced.getTeamLogoWithFallback(
            game.competitions[0]?.competitors[1]?.team?.id,
          );
          const homeLogo = await ItalyServiceEnhanced.getTeamLogoWithFallback(
  ...
  const handleDateFilterChange = (filter) => {
    ...
    if (isCacheValid) {
      console.log(
        "ItalyScoreboardScreen: Using cached data for filter change to:",
        filter,
      );
      setGames(cachedData);
      setLoading(false);
      ...
      console.log(
        "ItalyScoreboardScreen: No valid cache for filter:",
        filter,
  ...
  const handleGamePress = (game) => {
    ...
    try {
      ...
      console.log(
        "ItalyScoreboardScreen: matched live-tracker id ->",
        matchedId,
      );
      ...
      console.warn("ItalyScoreboardScreen: live tracker lookup failed", err);
      navigation.navigate("ItalyGameDetails", {
        gameId: game.id,
        sport: "Italian",
        competition: game.competitionName || "Italy",
