...
const ItalyGameDetailsScreen = ({ route, navigation }) => {
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId
        ? ItalyServiceEnhanced.getTeamLogoWithFallback(homeTeamId)
        : null,
      awayTeamId
        ? ItalyServiceEnhanced.getTeamLogoWithFallback(awayTeamId)
        : null,
    ]);
        ...
        <View style={styles.stickyContent}>
          ...
          <View style={styles.stickyTeamContainer}>
            <View style={styles.stickyTeamInfo}>
              ...
              >
                {isFavorite(homeTeam?.team?.id, "serie a") && "â˜… "}
          ...
          >
            ...
            <View style={styles.stickyTeamInfo}>
              ...
              >
                {isFavorite(awayTeam?.team?.id, "serie a") && "â˜… "}
  ...
  const renderStatsTab = () => {
    ...
    let homeColor =
      ItalyServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) ||
      "#007bff";
    let awayColor =
      ItalyServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) ||
      "#28a745";
