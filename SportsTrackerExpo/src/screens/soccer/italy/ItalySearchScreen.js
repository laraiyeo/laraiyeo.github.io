...
const ItalySearchScreen = ({ route, navigation }) => {
  ...
  const searchTeams = async (query) => {
    try {
      const teamsData = await ItalyServiceEnhanced.searchTeams(query);
      
      if (teamsData && Array.isArray(teamsData)) {
        const processedTeams = await Promise.all(
          teamsData.map(async (teamData) => {
            try {
              const logo = await ItalyServiceEnhanced.getTeamLogoWithFallback(teamData.team.id);
              return {
                id: teamData.team.id,
                type: 'team',
  ...
  const searchPlayers = async (query) => {
    try {
      const playersData = await ItalyServiceEnhanced.searchPlayers(query);
        ...
        const playersWithTeamInfo = await Promise.all(
          playersData.map(async (playerData) => {
            ...
            if (playerData.teamId) {
              try {
                const teamResponse = await fetch(convertToHttps(`https://site.api.espn.com/apis/site/v2/sports/soccer/ita.1/teams/${playerData.teamId}`));
...
export default ItalySearchScreen;
