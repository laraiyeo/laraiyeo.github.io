...
import { ItalyService } from '../../../services/soccer/ItalyService';
...
const ItalyScreen = () => {
  ...
  const leagueId = route.params?.leagueId || 'italy';
  const leagueName = route.params?.leagueName || 'Italy';
  ...
  const loadMatches = async () => {
    try {
      const data = await ItalyService.getScoreboard();
  ...
  const loadStandings = async () => {
    try {
      const data = await ItalyService.getStandings();
