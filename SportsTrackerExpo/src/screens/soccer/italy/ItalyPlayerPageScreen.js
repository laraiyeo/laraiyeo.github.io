...
const ItalyPlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log('ItalyPlayerPageScreen received - playerId:', playerId, 'playerName:', playerName, 'teamId:', teamId);
    fetchPlayerData();
  }, [playerId]);
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const basicPlayerData = {
        ...
        team: { 
          id: teamId, 
          displayName: 'AC Milan', // Better default for ita.1 teams
          name: 'Real Madrid'
        },
      ...
      try {
        ...
        const siteResponse = await fetch(convertToHttps(`https://site.api.espn.com/apis/site/v2/sports/soccer/ita.1/athletes/${playerId}`), {
          method: 'GET',
          headers: {
      ...
      try {
        const coreResponse = await fetch(convertToHttps(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/ita.1/athletes/${playerId}?lang=en&region=us`));
        if (coreResponse.ok) {
    ...
    try {
      ...
      const statsPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(`Validating Italy player stats data for ${competition.name}:`, statsData);
  ...
  const fetchGameLog = async () => {
    ...
    try {
      ...
      // Italy competitions to fetch game logs from
      const competitions = ['ita.1', 'ita.coppa_italia', 'ita.super_cup'];
      
      // Fetch game logs from all Italy competitions with year fallback
      const fetchPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(`Validating Italy game log data for ${competition}:`, gameLogData);
    ...
    const eventPromises = events.map(async (eventData) => {
      try {
        ...
        if (eventData && eventData.event && eventData.competition) {
          ...
          const leagueCode = leagueCodeMatch ? leagueCodeMatch[1] : 'ita.1';
  ...
  const loadPlayerStatsForYear = async (playerId, position, year) => {
    try {
      ...
      let leagueForYear = 'ita.1'; // Default to Serie A
      ...
      const careerPromises = yearsToFetch.map(async (year) => {
        try {
          ...
          if (teamsForSeason.length === 0) {
            ...
            const leaguesToTry = ['ita.1', 'ita.coppa_italia', 'ita.super_cup', 'eng.1', 'esp.1', 'fra.1', 'ger.1', 'usa.1', 'ksa.1'];
...
export default ItalyPlayerPageScreen;
