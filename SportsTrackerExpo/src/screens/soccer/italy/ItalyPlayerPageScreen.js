...
const ItalyPlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "ItalyPlayerPageScreen received - playerId:",
      playerId,
      "playerName:",
      playerName,
      "teamId:",
    ...
    try {
      ...
      try {
        ...
        const siteResponse = await fetch(
          convertToHttps(
            `https://site.api.espn.com/apis/site/v2/sports/soccer/ita.1/athletes/${playerId}`,
          ),
          {
            method: "GET",
            headers: {
      ...
      try {
        const coreResponse = await fetch(
          convertToHttps(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/ita.1/athletes/${playerId}?lang=en&region=us`,
          ),
        );
        if (coreResponse.ok) {
    ...
    try {
      ...
      const statsPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(
            `Validating Italy player stats data for ${competition.name}:`,
            statsData,
          );
    ...
    try {
      ...
      const fetchPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(
            `Validating Italy game log data for ${competition}:`,
            gameLogData,
          );
          ...
          if (isValidData) {
            ...
            console.log(
              `No game log data found for ${competition} player ${playerId} in any year`,
            );
          }
        } catch (error) {
            ...
            >
              <View style={styles.modalSeasonTopRow}>
                ...
                <View style={styles.modalTeamContainer}>
                  <Image
                    source={
                      getTeamLogoUrl(season.team.id, isDarkMode)
