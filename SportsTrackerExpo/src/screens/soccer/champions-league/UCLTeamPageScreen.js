...
const UCLTeamPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "UCLTeamPageScreen received - teamId:",
      teamId,
      "teamName:",
      teamName,
    );
    ...
    try {
      ...
      if (standingsData?.standings?.entries) {
        ...
        if (teamEntry) {
          ...
          const teamPosition =
            standingsData.standings.entries.findIndex(
              (entry) => entry.team.id === teamId,
            ) + 1;
      ...
      const competitionPromises = UCLCompetitions.map(async (leagueCode) => {
        try {
          ...
          const response = await fetch(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${leagueCode}/seasons/${year}/teams/${teamId}/events?lang=en&region=us&limit=100`,
          );
          ...
          if (!hasValidData) {
            console.log(
              `No valid events data found for ${leagueCode} team ${teamId}`,
            );
            return [];
          }

          console.log(
            `Found ${eventsData.items.length} events for team ${teamId} in ${leagueCode}`,
          );
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            const competitionCode =
              ...
              "unknown-competition";
            console.log(
              `UCL currentGame diagnostic -> TeamId: ${teamIdStr}  GameId: ${gameIdStr}  Competition: ${competitionCode}`,
            );
          } catch (diagErr) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, "uefa champions")) {
              ...
              console.log(
                "UCLTeamPageScreen: team is favorited - updating persisted currentGame:",
                currentGamePayload,
              );
  ...
  const fetchRoster = async () => {
    ...
    try {
      ...
      const year = getChampionsLeagueYear();
      const response = await fetch(
        `https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/teams/${teamId}/roster?season=${year}`,
      );
    ...
    try {
      ...
      const seasonTypes = [
        { id: "1", name: "Champions League", leagueCode: "uefa.champions" },
        {
          ...
          leagueCode: "uefa.champions_qual",
        },
      ];
      ...
      const statsPromises = seasonTypes.map(async (seasonType) => {
        try {
          ...
          const year = getChampionsLeagueYear();
          const response = await fetch(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/${seasonType.leagueCode}/seasons/${year}/types/1/teams/${teamId}/statistics/0?lang=en&region=us`,
          );
          ...
          if (statsData) {
            console.log(
              `Found ${seasonType.name} stats data:`,
            ...
            if (
              actualData.splits &&
              actualData.splits.categories &&
              Array.isArray(actualData.splits.categories)
            ) {
              console.log(
                `Found ${seasonType.name} stats with ${actualData.splits.categories.length} categories`,
              );
              return {
              ...
              console.log(`Invalid stats structure for ${seasonType.name}:`, {
                splits: actualData.splits,
                fullData: actualData,
              });
              return null;
            }
            ...
            console.log(
              `No stats data found for ${seasonType.name} team ${teamId} in any year`,
            );
            return null;
          }
        } catch (error) {
