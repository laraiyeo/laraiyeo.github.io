...
const UCLPlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log('UCLPlayerPageScreen received - playerId:', playerId, 'playerName:', playerName, 'teamId:', teamId);
    fetchPlayerData();
  }, [playerId]);
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const basicPlayerData = {
        ...
        team: { 
          id: teamId, 
          displayName: 'Manchester United', // Better default for uefa.champions teams
          name: 'Real Madrid'
        },
      ...
      try {
        ...
        const siteResponse = await fetch(convertToHttps(`https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.champions/athletes/${playerId}`), {
          method: 'GET',
          headers: {
      ...
      try {
        const coreResponse = await fetch(convertToHttps(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.champions/athletes/${playerId}?lang=en&region=us`));
        if (coreResponse.ok) {
  ...
  const fetchPlayerStats = async () => {
    ...
    try {
      ...
      const competitions = [
        ...
        { code: 'uefa.champions_qual', name: 'Champions League Qualifiers', seasonType: '0' },
      ];
    ...
    const eventPromises = events.map(async (eventData) => {
      try {
        ...
        if (eventData && eventData.event && eventData.competition) {
          ...
          const leagueCode = leagueCodeMatch ? leagueCodeMatch[1] : 'uefa.champions';
  ...
  const loadPlayerStatsForYear = async (playerId, position, year) => {
    try {
      ...
      let leagueForYear = 'uefa.champions'; // Default to Champions League
...
export default UCLPlayerPageScreen;
