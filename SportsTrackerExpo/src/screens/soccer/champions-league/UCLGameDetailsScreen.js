...
const UCLGameDetailsScreen = ({ route, navigation }) => {
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl =
      await ChampionsLeagueServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  const updatePlaysDataIncremental = (fetchedPlays) => {
    ...
    setPlaysData((prevPlays) => {
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log(
          "[UCLGameDetails] incremental update: added=",
          addedCount,
          "patched=",
          patchedCount,
        );
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log("[UCLGameDetails] loadGameDetails START", {
        gameId,
        silentUpdate,
      });
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log(
          "[UCLGameDetails] Game data updated - hash changed",
          currentHash,
        );
      ...
      setLoading(false);
      console.log("[UCLGameDetails] loadGameDetails END", {
        gameId,
        silentUpdate,
      });
    } catch (error) {
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId
        ? ChampionsLeagueServiceEnhanced.getTeamLogoWithFallback(homeTeamId)
        : null,
      awayTeamId
        ? ChampionsLeagueServiceEnhanced.getTeamLogoWithFallback(awayTeamId)
        : null,
    ]);
            ...
            <View style={styles.stickyTeamInfo}>
              ...
              <Text
                ...
                style={[
                  ...
                  {
                    color: isFavorite(homeTeam?.team?.id, "uefa champions")
                      ? colors.primary
                      : theme.text,
                  },
                  // Apply loser styling if home team is losing (only for finished games)
              ...
              >
                {isFavorite(homeTeam?.team?.id, "uefa champions") && "â˜… "}
            ...
            <View style={styles.stickyTeamInfo}>
              <Text
                ...
                style={[
                  styles.stickyTeamAbbr,
                  {
                    color: isFavorite(awayTeam?.team?.id, "uefa champions")
                      ? colors.primary
                      : theme.text,
                  },
                  // Apply loser styling if away team is losing (only for finished games)
              ...
              >
                {isFavorite(awayTeam?.team?.id, "uefa champions") && "â˜… "}
  ...
  const renderMatchHeader = () => {
    ...
    const title = competition?.series
      ? competition?.series[0]?.title === "Knockout Round Playoffs"
        ? "KO Playoffs"
        : competition?.series[0]?.title
      : gameData.header.season.name.split(", ")[1];
        ...
        <View style={styles.competitionContainer}>
          ...
          >
            {gameData.header.league.isTournament
              ? `${gameData.competitionName || "Soccer"} - ${title} ${competition.leg ? `- ${competition.leg.displayValue}` : ""}`
    ...
    try {
      ...
      try {
        ...
        if (coreResp.ok) {
          ...
          console.log(
            "[UCLGameDetails] core event resource responded with",
            coreResp.status,
          );
        }
      } catch (coreErr) {
        console.log(
          "[UCLGameDetails] Error fetching core event resource:",
          coreErr,
        );
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log(
              "[UCLGameDetails] statRef fetch failed",
              statRef,
              sResp.status,
            );
          ...
          console.log(
            "[UCLGameDetails] Failed to fetch statRef for competitor",
            comp?.id,
            err,
          );
          return { comp, rawText: null, parsed: null };
      ...
      try {
        teams.forEach((team) => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === "object") {
              ...
              console.log(
                `[UCLGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`,
              );
            }
          }
        });
        ...
        console.log(
          "[UCLGameDetails] Error attaching parsed stats to teams:",
          attachErr,
        );
      }
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (playsFetchingRef.current) {
      console.debug(
        "[UCLGameDetails] plays fetch already in progress - skipping",
      );
      return;
    }
    ...
    console.log("[UCLGameDetails] fetchPlaysData START", {
      gameId,
      lastUpdateHash,
    ...
    try {
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log(
          "[UCLGameDetails] plays changed - applying incremental update",
      ...
      console.log(
        "[UCLGameDetails] fetchPlaysData END - items",
        fetchedPlays.length,
      );
    } catch (error) {
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log(
          "[UCLGameDetails] user stopped scrolling - updates will resume",
        );
      }, 600);
    };
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
                ...
                {currentStreamType && availableStreams[currentStreamType] && (
                  <WebView
                    ...
                    onShouldStartLoadWithRequest={(request) => {
                      console.log(
                        "UCL WebView navigation request:",
                        request.url,
                      );
      ...
      {!!pitchPlayerShareCard &&
        (() => {
          ...
          const passDisplay =
            s.totalPasses > 0
              ? `${s.accuratePasses}-${s.totalPasses}`
              : "\u2014";
