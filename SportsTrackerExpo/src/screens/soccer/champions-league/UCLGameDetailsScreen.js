...
const UCLGameDetailsScreen = ({ route, navigation }) => {
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl = await ChampionsLeagueServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  let patchedCount = 0;
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log('[UCLGameDetails] incremental update: added=', addedCount, 'patched=', patchedCount);
        return updated;
      }
  ...
  const handleGoalLongPress = useCallback((play) => {
    console.log('UCL: Goal long press triggered:', play);
    ...
    // Get team color using ChampionsLeagueServiceEnhanced
    ...
    if (String(playTeamId) === String(homeId)) {
      ...
      teamColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(teamForColor) || '#007bff';
    ...
    console.log('UCL: Team color calculation:', { 
      playTeamId, 
      homeId, 
      awayId, 
      teamColor, 
    ...
    console.log('UCL: Setting share card play with context:', enhancedPlay);
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (showStreamModal) {
          console.log('Stream modal open, skipping UCL game update');
          return;
        }
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('[UCLGameDetails] useFocusEffect triggered');
      loadGameDetails();
  ...
  useEffect(() => {
    console.log('[UCLGameDetails] gameId changed - clearing data states');
    setPlaysData(null);
    setStatsData(null);
  }, [gameId]);
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      const isLive = gameData && gameData.header?.competitions?.[0]?.status?.type?.state === 'in';
      if (isLive) {
        console.log('Stream modal closed, immediately fetching UCL game data');
      ...
      if (scorerData?.id && shareCardPlay.team?.$ref) {
        try {
          ...
          if (teamResponse.ok) {
            ...
            if (gameId) {
              const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.champions/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${scorerData.id}/statistics/0?lang=en&region=us`;
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log('[UCLGameDetails] loadGameDetails START', { gameId, silentUpdate });
      console.debug('[UCLGameDetails] requesting game details for', gameId);
      const data = await ChampionsLeagueServiceEnhanced.getGameDetails(gameId);
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log('[UCLGameDetails] Game data updated - hash changed', currentHash);
        ...
        console.debug('[UCLGameDetails] Game data hash unchanged');
      }

      setLoading(false);
      console.log('[UCLGameDetails] loadGameDetails END', { gameId, silentUpdate });
    } catch (error) {
      console.error('Error loading UCL game details:', error);
      if (!silentUpdate) {
        setLoading(false);
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId ? ChampionsLeagueServiceEnhanced.getTeamLogoWithFallback(homeTeamId) : null,
      awayTeamId ? ChampionsLeagueServiceEnhanced.getTeamLogoWithFallback(awayTeamId) : null
    ]);
  ...
  const fetchPlayerGameStats = async (playerId, teamId) => {
    ...
    try {
      ...
      const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.champions/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${playerId}/statistics/0?lang=en&region=us`;
  ...
  const renderMatchHeader = () => {
    ...
    // Get team colors
    const homeColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team);
    const awayColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team);
        ...
        <View style={styles.matchContainer}>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              {isFavorite(homeTeam?.team?.id, 'uefa champions') ? '★ ' : ''}{homeTeam?.team?.displayName}
            </Text>
          </View>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              {isFavorite(awayTeam?.team?.id, 'uefa champions') ? '★ ' : ''}{awayTeam?.team?.displayName}
            </Text>
          </View>
        </View>
  ...
  const renderStatsTab = () => {
    ...
    const getStat = (team, statName) => {
      try {
        ...
        console.log('[UCLGameDetails] getStat error:', err);
      }
      return 0;
    };
    ...
    let homeColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) || '#007bff';
    let awayColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) || '#28a745';
    ...
    return (
      <View style={[styles.subsBox, { backgroundColor: theme.surface, borderColor: colors.primary }]}>
        <View style={styles.subsHeader}>
          <TeamLogoImage teamId={teamType === 'home' ? gameData?.homeCompetitor?.team?.id : gameData?.awayCompetitor?.team?.id} isDarkMode={isDarkMode} style={styles.subsTeamLogo} />
      ...
      <View style={styles.pitchesWrapper}>
        <View style={styles.pitchContainer}>
          <View style={styles.teamInfo}>
            <TeamLogoImage teamId={gameData?.awayCompetitor?.team?.id} isDarkMode={isDarkMode} style={styles.formTeamLogo} />
        ...
        <View style={styles.pitchContainer}>
          <View style={styles.teamInfo}>
            ...
            <TeamLogoImage teamId={gameData?.homeCompetitor?.team?.id} isDarkMode={isDarkMode} style={styles.formTeamLogo} />
          </View>
          <View style={styles.footballPitch}>
            <View style={styles.centerCircle} />
            <View style={styles.penaltyBox} />
  ...
  const renderSingleTeamPitch = (players, formation, teamLogo, subs, teamType, teamId) => {
    ...
    return (
      <View style={styles.pitchContainer}>
        <View style={styles.teamInfo}>
          <TeamLogoImage teamId={teamType === 'home' ? gameData?.homeCompetitor?.team?.id : gameData?.awayCompetitor?.team?.id} isDarkMode={isDarkMode} style={styles.formTeamLogo} />
  ...
  const mapTeamStats = (flattenedStats = []) => {
    try {
      ...
      console.log('[UCLGameDetails] mapTeamStats error:', err);
      return {};
    }
  };
  ...
  const fetchMatchStats = async () => {
    try {
      ...
      let coreCompetitors = [];
      try {
        const CORE_EVENT_URL = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.champions/events/${gameId}?lang=en&region=us`;
        ...
        if (coreResp.ok) {
          ...
          console.log('[UCLGameDetails] core event resource responded with', coreResp.status);
        }
      } catch (coreErr) {
        console.log('[UCLGameDetails] Error fetching core event resource:', coreErr);
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log('[UCLGameDetails] statRef fetch failed', statRef, sResp.status);
          ...
          console.log('[UCLGameDetails] Failed to fetch statRef for competitor', comp?.id, err);
          return { comp, rawText: null, parsed: null };
        }
      });
        ...
        teams.forEach(team => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === 'object') {
              ...
              console.log(`[UCLGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`);
            }
          }
        });
      } catch (attachErr) {
        console.log('[UCLGameDetails] Error attaching parsed stats to teams:', attachErr);
      }
      ...
      console.error('[UCLGameDetails] fetchMatchStats error:', error);
      return null;
    }
  };
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (isUserScrollingRef.current) {
      console.log('[UCLGameDetails] skipping plays fetch - user is scrolling');
      return;
    }
    ...
    if (playsFetchingRef.current) {
      console.debug('[UCLGameDetails] plays fetch already in progress - skipping');
      return;
    }
    ...
    console.log('[UCLGameDetails] fetchPlaysData START', { gameId, lastUpdateHash, playsDataCount: playsData ? playsData.length : 0, silent });

    try {
      // Fetch plays data from ESPN API exactly like scoreboard.js
      const PLAYS_API_URL = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.champions/events/${gameId}/competitions/${gameId}/plays?lang=en&region=us&limit=1000`;
      ...
      if (!data.items || data.items.length === 0) {
        console.warn('[UCLGameDetails] No plays data available in response');
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log('[UCLGameDetails] plays changed - applying incremental update', { playsHash, prev: lastPlaysHashRef.current });
        updatePlaysDataIncremental(fetchedPlays);
        lastPlaysHashRef.current = playsHash;
      } else {
        console.debug('[UCLGameDetails] plays hash unchanged - skipping merge');
      }

      console.log('[UCLGameDetails] fetchPlaysData END - items', fetchedPlays.length);
    } catch (error) {
      console.error('[UCLGameDetails] Error fetching plays data:', error);
      if (initialLoad) setPlaysData([]);
    } finally {
  ...
  useEffect(() => {
    console.log('[UCLGameDetails] Plays fetch useEffect triggered', {
    ...
    if (gameData && gameId && !playsData) {
      console.log('[UCLGameDetails] Fetching plays data for scorers box');
      fetchPlaysDataInternal({ silent: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log('[UCLGameDetails] user stopped scrolling - updates will resume');
      }, 600);
    };
    ...
    const renderedPlays = visiblePlays.map((play, index) => {
          ...
          if (playTeamId) {
            if (String(playTeamId) === String(awayId)) {
              teamSide = 'away';
              teamColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team || awayTeam) || '#28a745';
            } else if (String(playTeamId) === String(homeId)) {
              teamSide = 'home';
              teamColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team || homeTeam) || '#007bff';
            }
          }
  ...
  const renderMiniField = (coordinate, coordinate2, eventType = 'gen', teamSide = 'home', teamColor = '#007bff') => {
    ...
    const renderPlayerPopup = () => {
      ...
      if (selectedPlayer.teamType === 'home') {
        teamColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeamData?.team) || '#007bff';
      } else if (selectedPlayer.teamType === 'away') {
        teamColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeamData?.team) || '#28a745';
      }
              ...
              {currentStreamType && availableStreams[currentStreamType] && (
                <WebView
                  ...
                  onShouldStartLoadWithRequest={(request) => {
                    console.log('UCL WebView navigation request:', request.url);
                    ...
                    if (requestDomain !== currentDomain || hasPopupKeywords) {
                      console.log('Blocked UCL popup/cross-domain navigation:', request.url);
                      return false;
                    }
                  ...
                  onOpenWindow={(syntheticEvent) => {
                    ...
                    console.log('Blocked UCL popup window:', nativeEvent.targetUrl);
              ...
              >
                ...
                {shareCardPlay && (() => {
                  ...
                  if (!contextTeamColor && scoringTeam) {
                    ...
                    teamColor = ChampionsLeagueServiceEnhanced.getTeamColorWithAlternateLogic(scoringTeam) || teamColor;
...
export default UCLGameDetailsScreen;
