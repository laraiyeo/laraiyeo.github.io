...
import { ChampionsLeagueService } from '../../../services/soccer/ChampionsLeagueService';
...
const ChampionsLeagueScreen = () => {
  ...
  const leagueId = route.params?.leagueId || 'champions-league';
  const leagueName = route.params?.leagueName || 'Champions League';
  ...
  const loadMatches = async () => {
    try {
      const data = await ChampionsLeagueService.getScoreboard();
  ...
  const loadStandings = async () => {
    try {
      const data = await ChampionsLeagueService.getStandings();
