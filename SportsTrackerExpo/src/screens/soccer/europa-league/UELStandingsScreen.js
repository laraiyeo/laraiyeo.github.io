...
const UELStandingsScreen = ({ navigation, route }) => {
  ...
  const loadStandings = async () => {
    try {
      setLoading(true);
      console.log('Loading UEL standings...');
      const data = await EuropaLeagueServiceEnhanced.getStandings();
      ...
      if (data.children && data.children.length > 0) {
        ...
        const processedStandings = await Promise.all(
          standingsData.map(async (team) => {
            const logo = await EuropaLeagueServiceEnhanced.getTeamLogoWithFallback(team.team.id);
            return { ...team, logo };
          })
        );
      ...
      console.error('Error loading UEL standings:', error);
      setLoading(false);
    }
  };
    ...
    return (
      ...
      >
        ...
        <View style={styles.teamContainer}>
          ...
          />
          <Text allowFontScaling={false} style={[styles.teamName, { color: isFavorite(item.team.id, 'uefa europa') ? colors.primary : theme.text }]} numberOfLines={1}>
            {isFavorite(item.team.id, 'uefa europa') ? '★ ' : ''}{item.team.displayName}
          </Text>
        </View>
      ...
      <View style={[styles.legendContainer, { backgroundColor: theme.surface }]}>
        ...
        {Array.from(legend.entries()).map(([color, description]) => (
          <View key={color} style={styles.legendRow}>
            ...
            <Text allowFontScaling={false} style={[styles.legendText, { color: theme.textSecondary }]}>{description}</Text>
          </View>
        ))}
      </View>
    );
...
export default UELStandingsScreen;
