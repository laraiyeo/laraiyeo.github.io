  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      ...
      if (currentGame) {
        ...
        const competition = currentGame.leagueCode || 'uefa.europa' || (currentGame.competitions?.[0]?.league?.id) || null;
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, 'uefa europa')) {
              ...
              console.log('UELTeamPageScreen: team is favorited - updating persisted currentGame:', currentGamePayload);
      ...
      const statsPromises = seasonTypes.map(async (seasonType) => {
        try {
          ...
          if (response.ok) {
            ...
            console.log(`Validating UEL team stats data for ${seasonType.name}:`, statsData);
