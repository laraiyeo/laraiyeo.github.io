...
import { EuropaLeagueService } from '../../../services/soccer/EuropaLeagueService';

const EuropaLeagueScreen = () => {
  ...
  const leagueId = route.params?.leagueId || 'europa-league';
  const leagueName = route.params?.leagueName || 'Europa League';
  ...
  const loadMatches = async () => {
    try {
      const data = await EuropaLeagueService.getScoreboard();
  ...
  const loadStandings = async () => {
    try {
      const data = await EuropaLeagueService.getStandings();
...
export default EuropaLeagueScreen;