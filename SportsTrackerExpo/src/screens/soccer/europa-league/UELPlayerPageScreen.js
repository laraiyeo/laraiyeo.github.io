...
const UELPlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log('UELPlayerPageScreen received - playerId:', playerId, 'playerName:', playerName, 'teamId:', teamId);
    fetchPlayerData();
  }, [playerId]);
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const basicPlayerData = {
        ...
        team: { 
          id: teamId, 
          displayName: 'Manchester United', // Better default for uefa.europa teams
          name: 'Real Madrid'
        },
      ...
      try {
        ...
        const siteResponse = await fetch(convertToHttps(`https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/athletes/${playerId}`), {
          method: 'GET',
          headers: {
      ...
      try {
        const coreResponse = await fetch(convertToHttps(`https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.europa/athletes/${playerId}?lang=en&region=us`));
        if (coreResponse.ok) {
  ...
  const fetchPlayerStats = async () => {
    ...
    try {
      ...
      const competitions = [
        ...
        { code: 'uefa.europa_qual', name: 'Europa League Qualifiers', seasonType: '0' },
      ];
    ...
    const eventPromises = events.map(async (eventData) => {
      try {
        ...
        if (eventData && eventData.event && eventData.competition) {
          ...
          const leagueCode = leagueCodeMatch ? leagueCodeMatch[1] : 'uefa.europa';
  ...
  const loadPlayerStatsForYear = async (playerId, position, year) => {
    try {
      ...
      let leagueForYear = 'uefa.europa'; // Default to Europa League
                    ...
                    if (selectedGameStats.gameId) {
                      ...
                      setTimeout(() => {
                        navigation.navigate('UELGameDetails', {
                          ...
                          sport: 'Europa League',
                          competition: selectedGameStats.competition || 'uefa.europa',
...
export default UELPlayerPageScreen;
