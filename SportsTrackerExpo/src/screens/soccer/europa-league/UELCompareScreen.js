...
const UECLCompareScreen = ({ navigation, route }) => {
  ...
  // All Europa League players cache
  ...
  const fetchAllEuropaLeaguePlayers = async () => {
    try {
      console.log('Fetching all Europa League players...');
      const allPlayers = await EuropaLeagueServiceEnhanced.searchPlayers('');
      ...
      console.log(`Fetched ${validPlayers.length} valid Europa League players for comparison (${allPlayers.length} total)`);
      setAllEuropaLeaguePlayers(validPlayers);
    } catch (error) {
      console.error('Error fetching all Europa League players:', error);
    }
  };
  ...
  const searchPlayers = (query) => {
    ...
    const filteredPlayers = allEuropaLeaguePlayers
      .filter(player => {
  ...
  const fetchPlayerStats = async (playerId, year) => {
    try {
      ...
      const competitions = [
        ...
        { code: 'uefa.europa_qual', name: 'Europa League Qualifying', seasonType: '0' }
      ];
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
            Compare Europa League players side by side
          </Text>
        </View>
