...
const UELScoreboardScreen = ({ navigation, route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('UELScoreboardScreen: Screen focused');
      setIsScreenFocused(true);
      
      return () => {
        console.log('UELScoreboardScreen: Screen unfocused, clearing intervals');
  ...
  useEffect(() => {
    console.log('UELScoreboardScreen: Main useEffect triggered for filter:', selectedDateFilter, 'focused:', isScreenFocused);
  ...
  useEffect(() => {
    console.log('UELScoreboardScreen: Preload useEffect triggered, hasPreloaded:', hasPreloadedRef.current);
    // Only preload if we haven't done it before
    if (hasPreloadedRef.current) {
      console.log('UELScoreboardScreen: Skipping preload, already done');
      return;
    }
    
    // Mark that we're doing preloading
    hasPreloadedRef.current = true;
    console.log('UELScoreboardScreen: Starting preload for other filters');
    
    // Preload the other filters in the background after initial load
    const preloadTimer = setTimeout(() => {
      if (selectedDateFilter !== 'yesterday') {
        console.log('UELScoreboardScreen: Preloading yesterday data');
        loadScoreboard(true, 'yesterday');
      }
      if (selectedDateFilter !== 'upcoming') {
        console.log('UELScoreboardScreen: Preloading upcoming data');
  ...
  const loadScoreboard = async (silentUpdate = false, dateFilter = selectedDateFilter) => {
    console.log('UELScoreboardScreen: loadScoreboard called - silentUpdate:', silentUpdate, 'dateFilter:', dateFilter);
    const now = Date.now();
    ...
    if (isCacheValid && !silentUpdate) {
      console.log('UELScoreboardScreen: Using cached data for', dateFilter);
      setGames(cachedData);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      console.log('UELScoreboardScreen: Fetching fresh data for', dateFilter);
      const data = await EuropaLeagueServiceEnhanced.getScoreboard(dateFilter);
      
      // Process games with enhanced data
      const processedGames = await Promise.all((data.events || []).map(async (game) => {
        // Get team logos
        const awayLogo = await EuropaLeagueServiceEnhanced.getTeamLogoWithFallback(game.competitions[0]?.competitors[1]?.team?.id);
        const homeLogo = await EuropaLeagueServiceEnhanced.getTeamLogoWithFallback(game.competitions[0]?.competitors[0]?.team?.id);
      ...
      if (dateFilter === selectedDateFilter) {
        ...
        if (currentHash !== lastUpdateHash) {
          ...
          console.log('UELScoreboardScreen: Data updated for', dateFilter);
        }
      }

      setLoading(false);
    } catch (error) {
      console.error('UELScoreboardScreen: Error loading scoreboard:', error);
      if (!silentUpdate) {
        setLoading(false);
  ...
  const handleDateFilterChange = (filter) => {
    ...
    console.log('UELScoreboardScreen: Changing filter to:', filter);
    ...
    if (isCacheValid) {
      console.log('UELScoreboardScreen: Using cached data for filter change to:', filter);
      setGames(cachedData);
      setLoading(false);
    } else {
      console.log('UELScoreboardScreen: No valid cache for filter:', filter, '- will fetch fresh data');
    }
  };
  ...
  const handleGamePress = (game) => {
    console.log('UELScoreboardScreen: Game pressed:', game.id);
    navigation.navigate('UELGameDetails', {
      gameId: game.id,
      sport: 'Europa League',
      competition: game.competitionName || 'UEL',
...
export default UELScoreboardScreen;
