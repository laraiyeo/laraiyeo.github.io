...
const UELGameDetailsScreen = ({ route, navigation }) => {
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl = await EuropaLeagueServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  let patchedCount = 0;
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log('[UELGameDetails] incremental update: added=', addedCount, 'patched=', patchedCount);
        return updated;
      }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      try {
        ...
        if (showStreamModal) {
          console.log('Stream modal open, skipping Europa League game update');
          return;
        }
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      const isLive = gameData && gameData.header?.competitions?.[0]?.status?.type?.state === 'in';
      if (isLive) {
        console.log('Stream modal closed, immediately fetching Europa League game data');
      ...
      if (scorerData?.id && shareCardPlay.team?.$ref) {
        try {
          ...
          if (teamResponse.ok) {
            ...
            if (gameId) {
              const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.europa/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${scorerData.id}/statistics/0?lang=en&region=us`;
  ...
  const handleGoalLongPress = useCallback((play) => {
    console.log('UEL: Goal long press triggered:', play);
    ...
    // Get team color using EuropaLeagueServiceEnhanced
    ...
    if (String(playTeamId) === String(homeId)) {
      ...
      teamColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(teamForColor) || '#007bff';
    ...
    console.log('UEL: Team color calculation:', { 
      playTeamId, 
      homeId, 
      awayId, 
      teamColor, 
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log('[UELGameDetails] loadGameDetails START', { gameId, silentUpdate });
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log('[UELGameDetails] Game data updated - hash changed', currentHash);
      ...
      setLoading(false);
      console.log('[UELGameDetails] loadGameDetails END', { gameId, silentUpdate });
    } catch (error) {
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId ? EuropaLeagueServiceEnhanced.getTeamLogoWithFallback(homeTeamId) : null,
      awayTeamId ? EuropaLeagueServiceEnhanced.getTeamLogoWithFallback(awayTeamId) : null
    ]);
  ...
  const renderMatchHeader = () => {
    ...
    // Get team colors
    const homeColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team);
    const awayColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team);
  ...
  const renderStatsTab = () => {
    ...
    let homeColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) || '#007bff';
    let awayColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) || '#28a745';
  ...
  const fetchLineupData = async () => {
    ...
    try {
      console.log('[UELGameDetails] Fetching lineup data for gameId:', gameId);
      ...
      console.log('[UELGameDetails] Lineup API response:', data);
      ...
      console.log('[UELGameDetails] Lineup data extracted:', { 
  ...
  const mapTeamStats = (flattenedStats = []) => {
    try {
      ...
      console.log('[UELGameDetails] mapTeamStats error:', err);
      return {};
    }
  };
  ...
  const fetchMatchStats = async () => {
    try {
      ...
      let coreCompetitors = [];
      try {
        const CORE_EVENT_URL = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.europa/events/${gameId}?lang=en&region=us`;
        ...
        if (coreResp.ok) {
          ...
          console.log('[UELGameDetails] core event resource responded with', coreResp.status);
        }
      } catch (coreErr) {
        console.log('[UELGameDetails] Error fetching core event resource:', coreErr);
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log('[UELGameDetails] statRef fetch failed', statRef, sResp.status);
          ...
          console.log('[UELGameDetails] Failed to fetch statRef for competitor', comp?.id, err);
          return { comp, rawText: null, parsed: null };
        }
      });
        ...
        teams.forEach(team => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === 'object') {
              ...
              console.log(`[UELGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`);
            }
          }
        });
      } catch (attachErr) {
        console.log('[UELGameDetails] Error attaching parsed stats to teams:', attachErr);
      }
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (isUserScrollingRef.current) {
      console.log('[UELGameDetails] skipping plays fetch - user is scrolling');
      return;
    }
    ...
    if (playsFetchingRef.current) {
      console.debug('[UELGameDetails] plays fetch already in progress - skipping');
      return;
    }
    ...
    console.log('[UELGameDetails] fetchPlaysData START', { gameId, lastUpdateHash, playsDataCount: playsData ? playsData.length : 0, silent });

    try {
      // Fetch plays data from ESPN API exactly like scoreboard.js
      const PLAYS_API_URL = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/uefa.europa/events/${gameId}/competitions/${gameId}/plays?lang=en&region=us&limit=1000`;
      ...
      if (!data.items || data.items.length === 0) {
        console.warn('[UELGameDetails] No plays data available in response');
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log('[UELGameDetails] plays changed - applying incremental update', { playsHash, prev: lastPlaysHashRef.current });
        updatePlaysDataIncremental(fetchedPlays);
        lastPlaysHashRef.current = playsHash;
      } else {
        console.debug('[UELGameDetails] plays hash unchanged - skipping merge');
      }

      console.log('[UELGameDetails] fetchPlaysData END - items', fetchedPlays.length);
    } catch (error) {
  ...
  useEffect(() => {
    console.log('[UELGameDetails] Plays fetch useEffect triggered', {
    ...
    if (gameData && gameId && !playsData) {
      console.log('[UELGameDetails] Fetching plays data for scorers box');
      fetchPlaysDataInternal({ silent: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log('[UELGameDetails] user stopped scrolling - updates will resume');
      }, 600);
    };
    ...
    const renderedPlays = visiblePlays.map((play, index) => {
          ...
          if (playTeamId) {
            if (String(playTeamId) === String(awayId)) {
              teamSide = 'away';
              teamColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team || awayTeam) || '#28a745';
            } else if (String(playTeamId) === String(homeId)) {
              teamSide = 'home';
              teamColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team || homeTeam) || '#007bff';
            }
          }
  ...
  const renderMiniField = (coordinate, coordinate2, eventType = 'gen', teamSide = 'home', teamColor = '#007bff') => {
    ...
    const renderPlayerPopup = () => {
      ...
      if (selectedPlayer.teamType === 'home') {
        teamColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(homeTeamData?.team) || '#007bff';
      } else if (selectedPlayer.teamType === 'away') {
        teamColor = uropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(awayTeamData?.team) || '#28a745';
      }
              ...
              {currentStreamType && availableStreams[currentStreamType] && (
                <WebView
                  ...
                  onShouldStartLoadWithRequest={(request) => {
                    console.log('UEL WebView navigation request:', request.url);
                    ...
                    if (requestDomain !== currentDomain || hasPopupKeywords) {
                      console.log('Blocked UEL popup/cross-domain navigation:', request.url);
                      return false;
                    }
                  ...
                  onOpenWindow={(syntheticEvent) => {
                    ...
                    console.log('Blocked UEL popup window:', nativeEvent.targetUrl);
              ...
              >
                ...
                {shareCardPlay && (() => {
                  ...
                  if (!contextTeamColor && scoringTeam) {
                    ...
                    teamColor = EuropaLeagueServiceEnhanced.getTeamColorWithAlternateLogic(scoringTeam) || teamColor;
