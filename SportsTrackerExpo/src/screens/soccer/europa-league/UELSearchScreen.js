...
const UELSearchScreen = ({ route, navigation }) => {
  ...
  const searchPlayers = async (query) => {
    try {
      const playersData =
        await EuropaLeagueServiceEnhanced.searchPlayers(query);
              ...
              if (teamInfoCache.has(playerData.teamId)) {
                ...
                try {
                  const teamResponse = await fetch(
                    convertToHttps(
                      `https://site.api.espn.com/apis/site/v2/sports/soccer/uefa.europa/teams/${playerData.teamId}`,
                    ),
                  );
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <View style={[styles.searchHeader, { backgroundColor: theme.surface }]}>
        ...
        >
          Search for {(sport || "Europa League").toUpperCase()} teams and
          players
        </Text>
      </View>
