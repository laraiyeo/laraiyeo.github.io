...
const EnglandTeamPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log('EnglandTeamPageScreen received - teamId:', teamId, 'teamName:', teamName);
    fetchTeamData();
  ...
  const fetchTeamData = async () => {
    try {
      ...
      const url = `https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/teams/${teamId}`;
  ...
  const fetchTeamRecord = async (teamId) => {
    try {
      ...
      const standingsData = await EnglandServiceEnhanced.getStandings();
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      // Fetch from all England competitions in parallel
      const englishCompetitions = ['eng.1', 'eng.fa', 'eng.league_cup'];
      ...
      if (allEvents.length > 0) {
        ...
        if (foundCurrentGame) {
          ...
          try {
            ...
            if (favId && isFavorite(favId, 'premier league')) {
  ...
  const fetchTeamStats = async () => {
    ...
    try {
      ...
      // Fetch statistics for different England competitions
      const seasonTypes = [
        { id: '1', name: 'Premier League', leagueCode: 'eng.1' },
        { id: '7', name: 'FA Cup', leagueCode: 'eng.fa' },
        { id: '8', name: 'EFL Cup', leagueCode: 'eng.league_cup' }
      ];
      ...
      const statsPromises = seasonTypes.map(async (seasonType) => {
        try {
          ...
          console.log(`Validating England stats data:`, statsData);
  ...
  const renderTeamHeader = () => {
    ...
    const isTeamFavorite = isFavorite(teamData.id, 'premier league');
      ...
      <View style={[styles.teamHeader, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.teamInfo}>
          ...
          <Text allowFontScaling={false} style={[styles.teamDivision, { color: theme.textSecondary }]}>
            {teamData.standingSummary || 'England'}
          </Text>
        ...
        <View style={styles.matchContent}>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[styles.teamAbbreviation, { 
              color: isFavorite(homeTeam.team?.id, 'premier league') ? colors.primary : (homeIsLoser ? '#999' : theme.text)
            }]}>
              {isFavorite(homeTeam.team?.id, 'premier league') ? '★ ' : ''}{homeTeam.team?.abbreviation || homeTeam.team?.shortDisplayName || 'TBD'}
            </Text>
          </View>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[styles.teamAbbreviation, { 
              color: isFavorite(awayTeam.team?.id, 'premier league') ? colors.primary : (awayIsLoser ? '#999' : theme.text)
            }]}>
              {isFavorite(awayTeam.team?.id, 'premier league') ? '★ ' : ''}{awayTeam.team?.abbreviation || awayTeam.team?.shortDisplayName || 'TBD'}
            </Text>
          </View>
        </View>
...
export default EnglandTeamPageScreen;
