...
const EnglandCompareScreen = ({ navigation, route }) => {
  ...
  // All Premier League players cache
  const [allPremierLeaguePlayers, setAllPremierLeaguePlayers] = useState([]);
  ...
  const fetchAllPremierLeaguePlayers = async () => {
    try {
      console.log('Fetching all Premier League players...');
      const allPlayers = await EnglandServiceEnhanced.searchPlayers('');
      ...
      console.log(`Fetched ${validPlayers.length} valid Premier League players for comparison (${allPlayers.length} total)`);
      setAllPremierLeaguePlayers(validPlayers);
    } catch (error) {
      console.error('Error fetching all Premier League players:', error);
    }
  };
  ...
  const searchPlayers = (query) => {
    ...
    const filteredPlayers = allPremierLeaguePlayers
      .filter(player => {
  ...
  const fetchPlayerStats = async (playerId, year) => {
    try {
      ...
      // Define competitions like in EnglandPlayerPageScreen
      const competitions = [
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={[styles.subtitle, { color: theme.textSecondary }]}>
            Compare Premier League players side by side
          </Text>
        </View>
...
export default EnglandCompareScreen;
