...
const EnglandGameDetailsScreen = ({ route, navigation }) => {
      ...
      if (scorerData?.id && shareCardPlay.team?.$ref) {
        try {
          ...
          if (teamResponse.ok) {
            ...
            if (gameId) {
              const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.1/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${scorerData.id}/statistics/0?lang=en&region=us`;
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl = await EnglandServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  let patchedCount = 0;
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log('[EnglandGameDetails] incremental update: added=', addedCount, 'patched=', patchedCount);
        return updated;
      }
  ...
  useFocusEffect(
    React.useCallback(() => {
      console.log('[EnglandGameDetails] useFocusEffect triggered');
      loadGameDetails();
  ...
  useEffect(() => {
    console.log('[EnglandGameDetails] gameId changed - clearing data states');
    setPlaysData(null);
    setStatsData(null);
  }, [gameId]);
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log('[EnglandGameDetails] loadGameDetails START', { gameId, silentUpdate });
      console.debug('[EnglandGameDetails] requesting game details for', gameId);
      const data = await EnglandServiceEnhanced.getGameDetails(gameId);
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log('[EnglandGameDetails] Game data updated - hash changed', currentHash);
        ...
        console.log('[EnglandGameDetails] Lineup data updated:', lineupResult);
        
        // Do not forcibly clear playsData here; the plays effect will compare hashes and merge/refresh
      } else {
        console.debug('[EnglandGameDetails] Game data hash unchanged');
      }

      setLoading(false);
      console.log('[EnglandGameDetails] loadGameDetails END', { gameId, silentUpdate });
    } catch (error) {
      console.error('Error loading England game details:', error);
      if (!silentUpdate) {
        setLoading(false);
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId ? EnglandServiceEnhanced.getTeamLogoWithFallback(homeTeamId) : null,
      awayTeamId ? EnglandServiceEnhanced.getTeamLogoWithFallback(awayTeamId) : null
    ]);
  ...
  const fetchPlayerGameStats = async (playerId, teamId) => {
    ...
    try {
      ...
      const statsUrl = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.1/events/${gameId}/competitions/${gameId}/competitors/${teamId}/roster/${playerId}/statistics/0?lang=en&region=us`;
          ...
          <View style={styles.stickyTeamContainer}>
            <View style={styles.stickyTeamInfo}>
              ...
              <Text allowFontScaling={false} style={[
                ...
                { 
                  color: isFavorite(homeTeam?.team?.id, 'premier league') ? colors.primary : theme.text 
                },
                // Apply loser styling if home team is losing (only for finished games)
                homeIsLoser && {
                  opacity: 0.6
          ...
          <View style={[styles.stickyTeamContainer, { justifyContent: 'flex-end' }]}>
            ...
            <View style={styles.stickyTeamInfo}>
              <Text allowFontScaling={false} style={[
                styles.stickyTeamAbbr, 
                { 
                  color: isFavorite(awayTeam?.team?.id, 'premier league') ? colors.primary : theme.text 
                },
                // Apply loser styling if away team is losing (only for finished games)
                awayIsLoser && {
                  opacity: 0.6
  ...
  const renderMatchHeader = () => {
    ...
    // Get team colors
    const homeColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team);
    const awayColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team);
      ...
      <View style={[styles.headerContainer, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.competitionContainer}>
          <Text allowFontScaling={false} style={[styles.competitionText, { color: theme.textSecondary }]}>
            {gameData.competitionName || 'England'}
          </Text>
        </View>
        ...
        <View style={styles.matchContainer}>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              {isFavorite(homeTeam?.team?.id, 'premier league') ? '★ ' : ''}{homeTeam?.team?.displayName}
            </Text>
          </View>
          ...
          <View style={styles.teamSection}>
            ...
            <Text allowFontScaling={false} style={[
              ...
              {isFavorite(awayTeam?.team?.id, 'premier league') ? '★ ' : ''}{awayTeam?.team?.displayName}
            </Text>
          </View>
        </View>
  ...
  const renderStatsTab = () => {
    ...
    const getStat = (team, statName) => {
      try {
        ...
        console.log('[EnglandGameDetails] getStat error:', err);
      }
      return 0;
    };
    ...
    let homeColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) || '#007bff';
    let awayColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) || '#28a745';
  ...
  const fetchLineupData = async () => {
    ...
    try {
      console.log('[EnglandGameDetails] Fetching lineup data for gameId:', gameId);
      ...
      console.log('[EnglandGameDetails] Lineup API response:', data);
      ...
      console.log('[EnglandGameDetails] Lineup data extracted:', { 
  ...
  const mapTeamStats = (flattenedStats = []) => {
    try {
      ...
      console.log('[EnglandGameDetails] mapTeamStats error:', err);
      return {};
    }
  };
  ...
  const fetchMatchStats = async () => {
    try {
      ...
      let coreCompetitors = [];
      try {
        const CORE_EVENT_URL = `https://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.1/events/${gameId}?lang=en&region=us`;
        ...
        if (coreResp.ok) {
          ...
          console.log('[EnglandGameDetails] core event resource responded with', coreResp.status);
        }
      } catch (coreErr) {
        console.log('[EnglandGameDetails] Error fetching core event resource:', coreErr);
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log('[EnglandGameDetails] statRef fetch failed', statRef, sResp.status);
          ...
          console.log('[EnglandGameDetails] Failed to fetch statRef for competitor', comp?.id, err);
          return { comp, rawText: null, parsed: null };
        }
      });
        ...
        teams.forEach(team => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === 'object') {
              ...
              console.log(`[EnglandGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`);
            }
          }
        });
      } catch (attachErr) {
        console.log('[EnglandGameDetails] Error attaching parsed stats to teams:', attachErr);
      }
      ...
      console.error('[EnglandGameDetails] fetchMatchStats error:', error);
      return null;
    }
  };
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (isUserScrollingRef.current) {
      console.log('[EnglandGameDetails] skipping plays fetch - user is scrolling');
      return;
    }
    ...
    if (playsFetchingRef.current) {
      console.debug('[EnglandGameDetails] plays fetch already in progress - skipping');
      return;
    }
    ...
    console.log('[EnglandGameDetails] fetchPlaysData START', { gameId, lastUpdateHash, playsDataCount: playsData ? playsData.length : 0, silent });
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        console.warn('[EnglandGameDetails] No plays data available in response');
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log('[EnglandGameDetails] plays changed - applying incremental update', { playsHash, prev: lastPlaysHashRef.current });
        updatePlaysDataIncremental(fetchedPlays);
        lastPlaysHashRef.current = playsHash;
      } else {
        console.debug('[EnglandGameDetails] plays hash unchanged - skipping merge');
      }

      console.log('[EnglandGameDetails] fetchPlaysData END - items', fetchedPlays.length);
    } catch (error) {
      console.error('[EnglandGameDetails] Error fetching plays data:', error);
      if (initialLoad) setPlaysData([]);
    } finally {
  ...
  useEffect(() => {
    console.log('[EnglandGameDetails] Plays fetch useEffect triggered', {
    ...
    if (gameData && gameId && !playsData) {
      console.log('[EnglandGameDetails] Fetching plays data for scorers box');
      fetchPlaysDataInternal({ silent: true });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log('[EnglandGameDetails] user stopped scrolling - updates will resume');
      }, 600);
    };
    ...
    const renderedPlays = visiblePlays.map((play, index) => {
          ...
          if (playTeamId) {
            if (String(playTeamId) === String(awayId)) {
              teamSide = 'away';
              teamColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team || awayTeam) || '#28a745';
            } else if (String(playTeamId) === String(homeId)) {
              teamSide = 'home';
              teamColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team || homeTeam) || '#007bff';
            }
          }
  ...
  const renderPlayerPopup = () => {
    ...
    if (selectedPlayer.teamType === 'home') {
      teamColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(homeTeamData?.team) || '#007bff';
    } else if (selectedPlayer.teamType === 'away') {
      teamColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(awayTeamData?.team) || '#28a745';
    }
              ...
              {currentStreamType && availableStreams[currentStreamType] && (
                <WebView
                  ...
                  onShouldStartLoadWithRequest={(request) => {
                    console.log('England WebView navigation request:', request.url);
                    ...
                    if (requestDomain !== currentDomain || hasPopupKeywords) {
                      console.log('Blocked England popup/cross-domain navigation:', request.url);
                      return false;
                    }
                  ...
                  onOpenWindow={(syntheticEvent) => {
                    ...
                    console.log('Blocked England popup window:', nativeEvent.targetUrl);
              ...
              >
                ...
                {shareCardPlay && (() => {
                  ...
                  if (!contextTeamColor && scoringTeam) {
                    ...
                    teamColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(scoringTeam) || teamColor;
