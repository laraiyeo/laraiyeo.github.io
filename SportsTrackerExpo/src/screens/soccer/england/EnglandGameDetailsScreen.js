...
const EnglandGameDetailsScreen = ({ route, navigation }) => {
  ...
  // Pitch / sub player copy-card state
  ...
  const getTeamLogo = async (teamId, isDarkMode) => {
    ...
    const logoUrl =
      await EnglandServiceEnhanced.getTeamLogoWithFallback(teamId);
  ...
  const updatePlaysDataIncremental = (fetchedPlays) => {
    ...
    setPlaysData((prevPlays) => {
      ...
      if (newPlaysBatch.length > 0) {
        ...
        console.log(
          "[EnglandGameDetails] incremental update: added=",
          addedCount,
          "patched=",
          patchedCount,
        );
  ...
  useEffect(() => {
    if (showStreamModal === false && gameData) {
      ...
      if (isLive) {
        console.log(
          "Stream modal closed, immediately fetching England game data",
        );
  ...
  const loadGameDetails = async (silentUpdate = false) => {
    try {
      ...
      console.log("[EnglandGameDetails] loadGameDetails START", {
        gameId,
        silentUpdate,
      });
      ...
      if (currentHash !== lastUpdateHash) {
        ...
        console.log(
          "[EnglandGameDetails] Game data updated - hash changed",
          currentHash,
        );
        ...
        console.log(
          "[EnglandGameDetails] Lineup data updated (from summary):",
          lineupResult,
        );
      ...
      setLoading(false);
      console.log("[EnglandGameDetails] loadGameDetails END", {
        gameId,
        silentUpdate,
      });
    } catch (error) {
  ...
  const processGameData = async (data) => {
    ...
    const [homeLogo, awayLogo] = await Promise.all([
      homeTeamId
        ? EnglandServiceEnhanced.getTeamLogoWithFallback(homeTeamId)
        : null,
      awayTeamId
        ? EnglandServiceEnhanced.getTeamLogoWithFallback(awayTeamId)
        : null,
    ]);
            ...
            <View style={styles.stickyTeamInfo}>
              ...
              <Text
                ...
                style={[
                  ...
                  {
                    color: isFavorite(homeTeam?.team?.id, "premier league")
                      ? colors.primary
                      : theme.text,
                  },
                  // Apply loser styling if home team is losing (only for finished games)
            ...
            <View style={styles.stickyTeamInfo}>
              <Text
                ...
                style={[
                  styles.stickyTeamAbbr,
                  {
                    color: isFavorite(awayTeam?.team?.id, "premier league")
                      ? colors.primary
                      : theme.text,
                  },
                  // Apply loser styling if away team is losing (only for finished games)
  ...
  const renderMatchHeader = () => {
    ...
    // Get team colors
    const homeColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(
      homeTeam?.team,
    );
    const awayColor = EnglandServiceEnhanced.getTeamColorWithAlternateLogic(
      awayTeam?.team,
    );
  ...
  const renderStatsTab = () => {
    ...
    let homeColor =
      EnglandServiceEnhanced.getTeamColorWithAlternateLogic(homeTeam?.team) ||
      "#007bff";
    let awayColor =
      EnglandServiceEnhanced.getTeamColorWithAlternateLogic(awayTeam?.team) ||
      "#28a745";
  ...
  // Fetch lineup data: use summary rosters present in processed/game data only
    ...
    try {
      ...
      try {
        ...
        if (coreResp.ok) {
          ...
          console.log(
            "[EnglandGameDetails] core event resource responded with",
            coreResp.status,
          );
        }
      } catch (coreErr) {
        console.log(
          "[EnglandGameDetails] Error fetching core event resource:",
          coreErr,
        );
      }
      ...
      const statFetchPromises = coreCompetitors.map(async (comp) => {
        try {
          ...
          if (!sResp.ok) {
            console.log(
              "[EnglandGameDetails] statRef fetch failed",
              statRef,
              sResp.status,
            );
          ...
          console.log(
            "[EnglandGameDetails] Failed to fetch statRef for competitor",
            comp?.id,
            err,
          );
          return { comp, rawText: null, parsed: null };
      ...
      try {
        teams.forEach((team) => {
          ...
          if (result && result.parsed) {
            ...
            if (splitsData && typeof splitsData === "object") {
              ...
              console.log(
                `[EnglandGameDetails] Processed ${flattened.length} stats from $ref for ${homeAway} team`,
              );
            }
          }
        });
        ...
        console.log(
          "[EnglandGameDetails] Error attaching parsed stats to teams:",
          attachErr,
        );
      }
  ...
  const fetchPlaysDataInternal = async ({ silent = true } = {}) => {
    ...
    if (isUserScrollingRef.current) {
      console.log(
        "[EnglandGameDetails] skipping plays fetch - user is scrolling",
      );
      return;
    }
    ...
    if (playsFetchingRef.current) {
      console.debug(
        "[EnglandGameDetails] plays fetch already in progress - skipping",
      );
      return;
    }
    ...
    console.log("[EnglandGameDetails] fetchPlaysData START", {
      gameId,
      lastUpdateHash,
    ...
    try {
      ...
      if (!data.items || data.items.length === 0) {
        console.warn(
          "[EnglandGameDetails] No plays data available in response",
        );
      ...
      if (playsHash !== lastPlaysHashRef.current) {
        console.log(
          "[EnglandGameDetails] plays changed - applying incremental update",
        ...
        console.debug(
          "[EnglandGameDetails] plays hash unchanged - skipping merge",
        );
      }

      console.log(
        "[EnglandGameDetails] fetchPlaysData END - items",
        fetchedPlays.length,
      );
    } catch (error) {
  ...
  useEffect(() => {
    const onScroll = (event) => {
      ...
      scrollTimeoutRef.current = setTimeout(() => {
        ...
        console.log(
          "[EnglandGameDetails] user stopped scrolling - updates will resume",
        );
      }, 600);
    };
                ...
                {currentStreamType && availableStreams[currentStreamType] && (
                  <WebView
                    ...
                    onShouldStartLoadWithRequest={(request) => {
                      console.log(
                        "England WebView navigation request:",
                        request.url,
                      );
                      ...
                      if (hasPopupKeywords && !allowIfEmbed) {
                        console.log(
                          "Blocked England popup/cross-domain navigation:",
                          request.url,
                        );
                        return false;
                      }
                  ...
                  >
                    ...
                    <View style={styles.pitchShareCardNameRow}>
                      ...
                      <View style={{ flex: 1, marginLeft: 10 }}>
                        {/* Top-3 summary stats row */}
                        {(() => {
                          const SUMMARY_PRIORITY = isGK
                            ? ["SVS", "GA", "PASS", "CLR", "MIN"]
                            : isDEF
                          ...
                          const ordered = statItems
                            .filter(({ label }) =>
                              SUMMARY_PRIORITY.includes(label),
                            )
                            .sort(
                              (a, b) =>
                                SUMMARY_PRIORITY.indexOf(a.label) -
                                SUMMARY_PRIORITY.indexOf(b.label),
                            );
                          const nonZero = ordered.filter(
                            ({ value }) =>
                              value != null && value !== 0 && value !== "0%",
                          );
                          const fill = ordered.filter(
                            ({ label }) =>
                              !nonZero.find((n) => n.label === label),
                          );
                          const summaryStats = [...nonZero, ...fill].slice(
                            0,
                            3,
                          );
                          return (
                        ...
                        >
                          ...
                          {!!gameData?.header?.competitions?.[0]?.date &&
                            (() => {
                              ...
                              const monthDate = gd.toLocaleDateString("en-US", {
                                month: "short",
                                day: "numeric",
                              });
                              const year = gd.toLocaleDateString("en-US", {
                                year: "numeric",
                              });
                              return (
                                <View
...
const styles = StyleSheet.create({
  ...
  pitchShareCardScoreText: {
    fontSize: 15,
    fontWeight: "700",
  },
  pitchShareCardNameRow: {
    flexDirection: "row",
    alignItems: "center",
  },
  ...
  pitchShareCardTeamLabel: {
    fontSize: 12,
    fontWeight: "500",
  },
  pitchShareSummaryRow: {
    flexDirection: "row",
    gap: 12,
    marginBottom: 2,
  },
  pitchShareSummaryCell: {
    alignItems: "flex-start",
    gap: 1,
  },
  pitchShareSummaryVal: {
    fontSize: 18,
    fontWeight: "800",
    lineHeight: 22,
  },
