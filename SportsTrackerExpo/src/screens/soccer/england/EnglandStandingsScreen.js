...
const EnglandStandingsScreen = ({ navigation, route }) => {
  ...
  const loadStandings = async () => {
    try {
      setLoading(true);
      console.log('Loading England standings...');
      const data = await EnglandServiceEnhanced.getStandings();
      ...
      if (data.children && data.children.length > 0) {
        ...
        const processedStandings = await Promise.all(
          standingsData.map(async (team) => {
            const logo = await EnglandServiceEnhanced.getTeamLogoWithFallback(team.team.id);
            return { ...team, logo };
          })
        );
      ...
      console.error('Error loading England standings:', error);
      setLoading(false);
    }
  };
...
export default EnglandStandingsScreen;
