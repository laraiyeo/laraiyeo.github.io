...
const EnglandScoreboardScreen = ({ navigation, route }) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      return () => {
        console.log(
          "EnglandScoreboardScreen: Screen unfocused, clearing intervals"
        );
  ...
  useEffect(() => {
    console.log(
      "EnglandScoreboardScreen: Main useEffect triggered for filter:",
      selectedDateFilter,
      "focused:",
      isScreenFocused
    );
  ...
  useEffect(() => {
    console.log(
      "EnglandScoreboardScreen: Preload useEffect triggered, hasPreloaded:",
  ...
  const loadScoreboard = async (
    ...
    console.log(
      "EnglandScoreboardScreen: loadScoreboard called - silentUpdate:",
      silentUpdate,
      "dateFilter:",
      dateFilter
    );
    ...
    try {
      ...
      console.log(
        "EnglandScoreboardScreen: Fetching fresh data for",
        dateFilter
      );
      ...
      const processedGames = await Promise.all(
        (data.events || []).map(async (game) => {
          // Get team logos
          const awayLogo = await EnglandServiceEnhanced.getTeamLogoWithFallback(
            game.competitions[0]?.competitors[1]?.team?.id
          );
          const homeLogo = await EnglandServiceEnhanced.getTeamLogoWithFallback(
      ...
      console.error(
        "EnglandScoreboardScreen: Error loading scoreboard:",
        error
      );
      if (!silentUpdate) {
        setLoading(false);
  ...
  const handleDateFilterChange = (filter) => {
    ...
    if (isCacheValid) {
      console.log(
        "EnglandScoreboardScreen: Using cached data for filter change to:",
        filter
      );
      setGames(cachedData);
      setLoading(false);
      ...
      console.log(
        "EnglandScoreboardScreen: No valid cache for filter:",
        filter,
