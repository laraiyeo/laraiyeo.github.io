...
const EnglandPlayerPageScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    console.log(
      "EnglandPlayerPageScreen received - playerId:",
      playerId,
      "playerName:",
      playerName,
      "teamId:",
    ...
    try {
      ...
      try {
        ...
        const siteResponse = await fetch(
          convertToHttps(
            `https://site.api.espn.com/apis/site/v2/sports/soccer/eng.1/athletes/${playerId}`,
          ),
          {
            method: "GET",
            headers: {
      ...
      try {
        const coreResponse = await fetch(
          convertToHttps(
            `https://sports.core.api.espn.com/v2/sports/soccer/leagues/eng.1/athletes/${playerId}?lang=en&region=us`,
          ),
        );
        if (coreResponse.ok) {
    ...
    try {
      ...
      const statsPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(`Validating England player stats data:`, statsData);
  ...
  const fetchGameLog = async () => {
    ...
    try {
      ...
      // Fetch game logs from all England competitions with year fallback
      const fetchPromises = competitions.map(async (competition) => {
        try {
          ...
          console.log(
            `Validating England game log data for ${competition}:`,
            gameLogData,
          );
  ...
  const getPlayerTeamsForSeason = (transactions, season) => {
    ...
    // Keep both 'from' and 'to' teams for a transfer within the same league
    // so the UI can show two logos side-by-side. Only remove exact duplicate
    // team entries (same teamId + league).
    const deduped = [];
    for (const t of teams) {
      const exists = deduped.some(
        (m) => m.teamId === t.teamId && m.league === t.league,
      );
      if (!exists) deduped.push(t);
    }
  ...
  const fetchCareerData = async () => {
    ...
    try {
      ...
      // Merge transfer 'from'/'to' pairs for the same league+season into a single
      // season object so the UI can render one combined card (from -> to).
      const mergedCareerStats = [];
      const seenSeasonKeys = new Set();

      for (let i = 0; i < careerStats.length; i++) {
        const s = careerStats[i];
        const key = `${s.league}__${s.season}`;
        if (seenSeasonKeys.has(key)) continue;

        if (s.isTransferYear) {
          // Try to find the matching opposite transfer entry for same league+season
          const otherIndex = careerStats.findIndex(
            (o, idx) =>
              idx !== i &&
              o.league === s.league &&
              o.season === s.season &&
              ((s.transferPeriod === "from" && o.transferPeriod === "to") ||
                (s.transferPeriod === "to" && o.transferPeriod === "from")),
          );

          if (otherIndex !== -1) {
            const other = careerStats[otherIndex];
            // Determine which is from and which is to
            const fromTeam = s.transferPeriod === "from" ? s.team : other.team;
            const toTeam = s.transferPeriod === "to" ? s.team : other.team;

            const merged = {
              // Keep the main fields from one of the entries (use 's')
              ...s,
              transferPair: true,
              fromTeam,
              toTeam,
              // Use 'from' team as the primary 'team' to preserve existing lookups
              team: fromTeam,
            };
        ...
        mergedCareerStats.push(s);
        seenSeasonKeys.add(key);
      }

      setCareerData({ seasons: mergedCareerStats });
    } catch (error) {
