...
// (you want the other team to have as few runs/hits/HR/BB/AVG as possible)
...
// Build rows from the stat object returned by the /wbc/team endpoint.
// Each stat entry has the shape { value, rank, min, max }.
// For lower-is-better stats the bar is inverted so full = best.
...
const MatchCard = ({
  ...
  const awayColor = WBCService.getTeamColor(awayId) || colors.primary;
  const homeColor = WBCService.getTeamColor(homeId) || colors.secondary;
  const awayWinner =
    isFinished &&
    awayScore != null &&
    homeScore != null &&
...
const TeamPageScreen = ({ route, navigation }) => {
  ...
  const teamColor = WBCService.getTeamColor(resolvedId) || colors.primary;
  const teamLogo = WBCService.getTeamLogo(resolvedId, isDarkMode);
  ...
  const loadTeam = useCallback(
    async (isRefresh = false) => {
      ...
      try {
        const res = await WBCService.getTeam(resolvedId);
  ...
  // ── Stats: pick whichever year key exists (prefer 2026, fall back to 2025,
