...
const TeamPageScreen = ({ route, navigation }) => {
  ...
  const fetchTeamData = async () => {
    try {
      ...
      if (data.teams && data.teams.length > 0) {
        ...
        // Fetch team record and streak using MLB team ID (abbreviation not in standings)
        await fetchTeamRecord(data.teams[0].abbreviation, mlbApiId);
  ...
  const fetchTeamRecord = async (teamAbbreviation, mlbTeamId) => {
    try {
      console.log(
        "Fetching team record for abbreviation:",
        teamAbbreviation,
        "mlbId:",
        mlbTeamId,
      );
      // Use cached MLBService.getStandings() — shared with StandingsScreen
      const res = await MLBService.getStandings();
      const records = res?.data?.records ?? res?.records ?? [];

      for (const divRecord of records) {
        const teamEntry = (divRecord.teamRecords ?? []).find((tr) => {
          // Prefer matching by MLB team ID (abbreviation not in standings response)
          if (mlbTeamId && tr.team?.id?.toString() === mlbTeamId.toString())
            return true;
          // Fallback: abbreviation match if API happens to include it
          const abbr = tr.team?.abbreviation ?? "";
          return (
            (abbr && abbr === teamAbbreviation) ||
            (teamAbbreviation === "ARI" && abbr === "AZ") ||
            (teamAbbreviation === "AZ" && abbr === "ARI")
          );
        });

        if (teamEntry) {
          console.log(
            "Found team entry in MLB standings:",
            teamEntry.team?.name,
          );
          const wins = teamEntry.wins?.toString() ?? "0";
          const losses = teamEntry.losses?.toString() ?? "0";
          const streak = teamEntry.streak?.streakCode ?? "N/A";
          // last-ten from splitRecords if available
          const lastTenRecord = teamEntry.records?.splitRecords?.find(
            (r) => r.type === "lastTen",
          );
          const lastTen = lastTenRecord
            ? `${lastTenRecord.wins}-${lastTenRecord.losses}`
            : "0-0";
          const clincher = teamEntry.clinchIndicator
            ? `${teamEntry.clinchIndicator.toUpperCase()} - `
            : "";
      ...
      console.log(
        "Team not found in standings for abbreviation:",
        teamAbbreviation,
      );
    } catch (error) {
  ...
  const fetchCurrentGame = async () => {
    try {
      ...
      console.log(
        "TeamPage fetchCurrentGame: teamId =",
        teamId,
        "mlbApiId =",
        mlbApiId,
      );
      ...
      if (
        todayData.dates &&
        todayData.dates.length > 0 &&
        todayData.dates[0].games.length > 0
      ) {
        ...
        if (liveGame) {
          ...
          try {
            if (isFavorite(teamId, sport)) {
              await updateTeamCurrentGame(teamId, {
                ...
                eventLink:
                  liveGame.link ||
                  `/api/v1.1/game/${liveGame.gamePk}/feed/live`,
            ...
            console.log(
              "TeamPage: failed to persist live currentGame to favorites",
              e?.message || e,
            );
          }
          ...
          try {
            setLastStatusString(
              `Live: true Scheduled: false Finished: false Start Time: ${new Date(liveGame.gameDate).toLocaleString()} Direct Link: ${liveGame.link || `/api/v1.1/game/${liveGame.gamePk}/feed/live`}`,
            );
          } catch (e) {}
          return;
        }
        ...
        if (scheduledGame) {
          __orig_console_log(
            "Found scheduled game for today:",
          ...
          try {
            if (isFavorite(teamId, sport)) {
              await updateTeamCurrentGame(teamId, {
                ...
                eventLink:
                  scheduledGame.link ||
                  `/api/v1.1/game/${scheduledGame.gamePk}/feed/live`,
            ...
            console.log(
              "TeamPage: failed to persist scheduled currentGame to favorites",
              e?.message || e,
            );
          }
          ...
          try {
            setLastStatusString(
              `Live: false Scheduled: true Finished: false Start Time: ${new Date(scheduledGame.gameDate).toLocaleString()} Direct Link: ${scheduledGame.link || `/api/v1.1/game/${scheduledGame.gamePk}/feed/live`}`,
            );
          } catch (e) {}
          return;
        }
        ...
        try {
          if (isFavorite(teamId, sport)) {
            await updateTeamCurrentGame(teamId, {
              ...
              eventLink:
                games[0].link || `/api/v1.1/game/${games[0].gamePk}/feed/live`,
          ...
          console.log(
            "TeamPage: failed to persist completed currentGame to favorites",
            e?.message || e,
          );
        }
        ...
        try {
          setLastStatusString(
            `Live: false Scheduled: false Finished: true Start Time: ${new Date(games[0].gameDate).toLocaleString()} Direct Link: ${games[0].link || `/api/v1.1/game/${games[0].gamePk}/feed/live`}`,
          );
        } catch (e) {}
      } else {
        ...
        console.log(
          "Searching for upcoming games from:",
          startSearchDate,
          "to:",
          end,
        );
          ...
          for (const date of upcomingData.dates) {
            if (date.games && date.games.length > 0) {
              const nextGame = date.games.find((game) =>
                ["Scheduled", "Pre-Game", "Warmup"].includes(
                ...
                try {
                  if (isFavorite(teamId, sport)) {
                    ...
                    await updateTeamCurrentGame(teamId, {
                      ...
                      eventLink:
                        nextGame.link ||
                        `/api/v1.1/game/${nextGame.gamePk}/feed/live`,
                  ...
                  console.log(
                    "TeamPage: failed to persist upcoming currentGame to favorites",
                    e?.message || e,
                  );
                }
  ...
  const fetchAllMatches = async () => {
    try {
      ...
      if (data.dates && data.dates.length > 0) {
        ...
        const yesterdayMLBString =
          yesterdayMLB.getFullYear() +
          "-" +
        ...
        const lastMatchesData = allGames
          .filter((game) => {
            ...
            const gameEasternDateString =
              gameEastern.getFullYear() +
              "-" +
            ...
            return (
              ...
              gameEasternDateString <= yesterdayMLBString
            );
          })
          .reverse();
        ...
        const nextMatchesData = allGames.filter((game) => {
          ...
          const todayMLBString =
            todayMLB.getFullYear() +
            "-" +
  ...
  const fetchRoster = async () => {
    ...
    try {
      ...
      const mlbApiId = getAPITeamId(teamId, sport);
      const response = await fetch(
        `https://statsapi.mlb.com/api/v1/teams/${mlbApiId}/roster?fields=roster,person,id,fullName,jerseyNumber,position,name,abbreviation,status,code,description&rosterType=40Man`,
      );
      const data = await response.json();
  ...
  const fetchTeamStats = async () => {
    ...
    try {
      ...
      const [hittingResponse, pitchingResponse] = await Promise.all([
        fetch(
          `https://statsapi.mlb.com/api/v1/teams/${mlbApiId}/stats?stats=season&group=hitting&season=${currentYear}`,
        ),
        fetch(
          `https://statsapi.mlb.com/api/v1/teams/${mlbApiId}/stats?stats=season&group=pitching&season=${currentYear}`,
        ),
      ]);
  ...
  useEffect(() => {
    if (currentGame) {
      ...
      const mlbCoded =
        currentGame.mlbGameData?.status?.codedGameState ||
        currentGame.liveData?.status?.codedGameState;
      const statusType =
        currentGame.status ||
        currentGame.header?.competitions?.[0]?.status ||
        null;
      const isFinishedFlag = Boolean(
        mlbCoded === "F" ||
        mlbCoded === "O" ||
        (statusType &&
          (statusType.abstractGameState === "Final" ||
            statusType.type?.state === "post")),
      );
      ...
      const startDateRaw =
        ...
        (currentGame.header &&
          currentGame.header.competitions &&
          currentGame.header.competitions[0] &&
          currentGame.header.competitions[0].date);
      const startTimeStr = startDateRaw
        ? new Date(startDateRaw).toLocaleString("en-US", {
            month: "short",
            day: "numeric",
            hour: "numeric",
            minute: "2-digit",
      ...
      const directLink =
        ...
        (currentGame.gamePk
          ? `/api/v1.1/game/${currentGame.gamePk}/feed/live`
          : currentGame.eventId
            ? `/api/v1.1/game/${currentGame.eventId}/feed/live`
            : "none");
      ...
      try {
        setLastStatusString(statusString);
      } catch (e) {}
    }
  ...
  const getTeamLogoStyle = (game, isAwayTeam) => {
    ...
    return isLosing
      ? [styles.gameTeamLogo, { opacity: 0.5 }]
      : styles.gameTeamLogo;
  };
  ...
  const renderTeamHeader = () => {
    ...
    const handleToggleFavorite = async () => {
      ...
      try {
        ...
        console.log(
          "handleToggleFavorite: fetchCurrentGame failed or was unavailable:",
          e?.message || e,
        );
      }
      ...
      if (currentGameData) {
        console.log(
          "MLB TeamPage: favoriting with currentGameData (sourceGame chosen)",
          currentGameData,
        );
      } else {
        console.log(
          "MLB TeamPage: favoriting without a currentGame (no candidate found)",
        );
      }
      ...
      <View style={[styles.teamHeader, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.teamInfo}>
          <Text
            ...
            style={[
              styles.teamName,
              { color: getTeamColor(teamData.abbreviation) },
            ]}
          >
            {teamData.name}
          </Text>
          ...
          >
            {teamRecord ? teamRecord.clincher || "" : ""}
            {teamData.division?.name || "N/A"}
          </Text>
            ...
            <View style={styles.recordContainer}>
              <View style={styles.recordRow}>
                ...
                <Text
                  ...
                  style={[styles.recordValue, { color: theme.text }]}
                >
                  {teamRecord.lastTen || "0-0"}
                </Text>
                <Text
                  allowFontScaling={false}
                  style={[
                    styles.recordValue,
                    { color: getStreakColor(teamRecord.streak) },
                  ]}
                >
        ...
        >
          <Text
            ...
            style={[
              styles.favoriteIcon,
              { color: isTeamFavorite ? colors.primary : theme.textSecondary },
            ]}
          >
            {isTeamFavorite ? "★" : "☆"}
          </Text>
      ...
      <View style={styles.matchesSection}>
        ...
        >
          ...
          {currentGame.description && (
            <View
              style={[
                styles.gameDescriptionBanner,
                { backgroundColor: colors.secondary },
              ]}
            >
              <Text allowFontScaling={false} style={styles.gameDescriptionText}>
                {currentGame.description}
              </Text>
            </View>
          )}
          <View style={styles.gameTeams}>
            <View style={styles.teamContainer}>
              <View style={styles.teamLogoContainer}>
                ...
                />
                {(isGameLive(currentGame) ||
                  currentGame.status.abstractGameState === "Final") &&
                  away.score !== undefined && (
                    <Text
                      allowFontScaling={false}
                      style={[
                        getTeamScoreStyle(currentGame, true),
                        { color: getScoreColor(currentGame, true) },
                      ]}
                    >
                      {away.score}
                    </Text>
              ...
              <Text
                allowFontScaling={false}
                style={[
                  getTeamNameStyle(currentGame, true),
                  { color: getTeamNameColor(currentGame, true) },
                ]}
              >
                {(() => {
            ...
            <View style={styles.gameInfo}>
              <Text
                allowFontScaling={false}
                style={[styles.gameStatus, { color: colors.accent }]}
              >
                {getGameStatus()}
              </Text>
              <Text
              ...
              <Text
                allowFontScaling={false}
                style={[styles.versus, { color: theme.textSecondary }]}
              >
                vs
              </Text>
            </View>
            ...
            <View style={styles.teamContainer}>
                ...
                {(isGameLive(currentGame) ||
                  ...
                  home.score !== undefined && (
                    <Text
                      allowFontScaling={false}
                      style={[
                        getTeamScoreStyle(currentGame, false),
                        { color: getScoreColor(currentGame, false) },
                      ]}
                    >
                      {home.score}
                    </Text>
              ...
              <Text
                allowFontScaling={false}
                style={[
                  getTeamNameStyle(currentGame, false),
                  { color: getTeamNameColor(currentGame, false) },
                ]}
              >
                {(() => {
        ...
        <View style={styles.gameTeams}>
          <View style={styles.teamContainer}>
            <View style={styles.teamLogoContainer}>
              ...
              {isCompleted && away.score !== undefined && (
                <Text
                  allowFontScaling={false}
                  style={[
                    getTeamScoreStyle(game, true),
                    { color: getScoreColor(game, true) },
                  ]}
                >
                  {away.score}
                </Text>
            ...
            <Text
              allowFontScaling={false}
              style={[
                getTeamNameStyle(game, true),
                { color: getTeamNameColor(game, true) },
              ]}
            >
              {(() => {
                const mlbId = away.team.id?.toString();
          ...
          <View style={styles.teamContainer}>
            <View style={styles.teamLogoContainer}>
              {isCompleted && home.score !== undefined && (
                <Text
                  allowFontScaling={false}
                  style={[
                    getTeamScoreStyle(game, false),
                    { color: getScoreColor(game, false) },
                  ]}
                >
                  {home.score}
                </Text>
            ...
            <Text
              allowFontScaling={false}
              style={[
                getTeamNameStyle(game, false),
                { color: getTeamNameColor(game, false) },
              ]}
            >
              {(() => {
                const mlbId = home.team.id?.toString();
  ...
  const renderStatsContent = () => {
    ...
    const perGameStats = teamStats.hitting
      ? {
          ...
          ha: teamStats.pitching
            ? (teamStats.pitching.hits / gamesPlayed).toFixed(1)
            : "--",
          // BBA = Walks Allowed (from pitching stats)
          bba: teamStats.pitching
            ? (teamStats.pitching.baseOnBalls / gamesPlayed).toFixed(1)
            : "--",
          // HRA = Home Runs Allowed (from pitching stats)
          hra: teamStats.pitching
            ? (teamStats.pitching.homeRuns / gamesPlayed).toFixed(1)
            : "--",
        }
      : null;
              ...
              {lastMatches.length > 0 ? (
                <View>
                  {(lastMatchesCollapsed
                    ...
                    <View key={`last-${game.gamePk}-${index}`}>
              ...
              {nextMatches.length > 0 ? (
                <View>
                  {(nextMatchesCollapsed
                    ...
                    <View key={`next-${game.gamePk}-${index}`}>
                      {renderMatchCard(game)}
                    </View>
                  ))}
                </View>
