      ...
      for (const teamId of teams) {
        try {
          const response = await fetch(
            `https://statsapi.mlb.com/api/v1/teams/${teamId}/roster?rosterType=active&season=${currentYear}`,
          );
          ...
          if (data.roster) {
            ...
            const teamPlayers = data.roster.map((player) => ({
              ...
              isTwoWayPlayer:
                player.position.abbreviation === "TWP" ||
  ...
  const loadComparison = async () => {
    ...
    try {
      ...
      if (!canCompare) {
        Alert.alert(
          "Invalid Comparison",
          "Can only compare pitchers with pitchers, hitters with hitters, or Two-Way Players with either.",
        );
        setComparisonStats(null);
        setLoading(false);
        return;
      ...
      const comparisonData = statsToCompare.map((statDef) => {
        const defaultValue =
          statDef.key.includes("avg") ||
          statDef.key.includes("obp") ||
          statDef.key.includes("slg") ||
          statDef.key.includes("ops") ||
          statDef.key.includes("era") ||
          statDef.key.includes("whip")
            ? "0.000"
            : "0";
              ...
              {yearOptions.map((year) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    {
                      backgroundColor:
                        year === player1Year ? colors.primary : "transparent",
                    },
                  ]}
                  onPress={() => {
                    setPlayer1Year(year);
              ...
              {yearOptions.map((year) => (
                <TouchableOpacity
                  ...
                  style={[
                    ...
                    {
                      backgroundColor:
                        year === player2Year ? colors.primary : "transparent",
                    },
                  ]}
                  onPress={() => {
                    setPlayer2Year(year);
