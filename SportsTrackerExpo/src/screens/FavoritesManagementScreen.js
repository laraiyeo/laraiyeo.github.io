  ...
  const confirmRemoveFavorite = (item) => {
    Alert.alert(
      ...
      `Are you sure you want to remove ${
        item.displayName || item.teamName || "this team"
      } from your favorites?`,
      [
        {
          text: "Cancel",
      ...
      try {
        ...
        if (sport === "f1") {
          ...
          if (teamId && typeof teamId === "string" && teamId.includes("_")) {
            ...
            console.log(
              "Stripped soccer teamId for navigation:",
              stripped,
              "from",
              teamId
            );
          ...
          if (teamId && typeof teamId === "string" && teamId.includes("_")) {
            const stripped = teamId.split("_")[0];
            console.log(
              "Stripped teamId for navigation:",
              stripped,
              "from",
              teamId
            );
    ...
    return (
      ...
      >
        <View style={styles.teamContent}>
          <Image
            source={{
              uri:
                sport === "f1"
                  ? getF1ConstructorLogo(item.teamName || item.displayName)
                  : getTeamLogoUrl(sport, abbreviation || item.teamId),
            }}
        ...
        <TouchableOpacity
          style={[
            styles.removeButton,
            { backgroundColor: theme.error || "#ff4d4f" },
          ]}
      ...
      >
        ...
        {favoriteTeams.length > 0 ? (
          <View style={styles.favoritesList}>
            {favoriteTeams.map((item, index) =>
              renderFavoriteTeam(item, index)
            )}
          </View>
        ) : (
          <View style={styles.emptyState}>
            ...
            >
              Add teams to your favorites by tapping the star icon on team pages
              or game details.
            </Text>
          </View>
        )}
