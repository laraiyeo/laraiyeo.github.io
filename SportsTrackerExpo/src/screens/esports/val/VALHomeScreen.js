...
const VALHomeScreen = ({ navigation, route }) => {
  ...
  const loadFilteredData = async (filter) => {
    try {
      ...
      setUpcomingSeries(
        list.filter((s) => s && !s.live && !s.completed && inDay(s.startDate)),
      );
    } catch (error) {
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                navigation.navigate("VALEvent", {
                  eventId: event.parentEventId || event.eventId,
                })
              }
            >
              ...
              {event.series.map((match, index) => (
                <View key={match.id}>
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      navigation.navigate("VALSeries", { seriesId: match.id })
                    }
                  >
                    {/* Time on left */}
  ...
  const CompletedSeriesCard = ({ series }) => (
    ...
    >
      {/* Event Child Label */}
      <Text
        style={[styles.completedEventLabel, { color: theme.text }]}
        numberOfLines={1}
      >
