...
const VALTeamPageScreen = ({ route }) => {
  ...
  const fetchTeamStats = async () => {
    ...
    try {
      ...
      const [mapStatsResponse, winConditionsResponse] = await Promise.all([
        ...
        ribApiCall(
          `/x/teams/win-conditions?startDate=${startDate}&teamId=${teamData.id}`,
        ),
      ]);
  ...
  const fetchTransactionHistory = async () => {
    ...
    try {
      ...
      if (!earningsData) {
        console.log(
          "No cached team earnings data found - trying to fetch it...",
        );
      ...
      if (!earningsData || !earningsData.data) {
        console.log(
          "❌ No team earnings data available - skipping transaction history",
        );
        setTransactionHistory([]);
        return;
      }
      ...
      console.log(
        "✅ Found cached earnings data with",
      ...
      console.log(
        ...
        JSON.stringify(earningsData.data[0], null, 2),
      );
      ...
      if (!foundTeam) {
        ...
        foundTeam = earningsData.data.find((item) => {
          ...
          return (
            lowerTeamName.includes(searchTerm) ||
            searchTerm.includes(lowerTeamName)
          );
        });
      }
      ...
      if (foundTeam) {
        ...
        const teamName =
          foundTeam.team?.name || foundTeam.name || foundTeam.team_name;
        console.log(
          "✅ Team found in earnings data - ID:",
          bo3TeamId,
          "Name:",
          teamName,
        );
        console.log(
          "Full found team object:",
          JSON.stringify(foundTeam, null, 2),
        );
      } else {
        ...
        first50Teams.forEach((item, index) => {
          const teamName =
            item.team?.name || item.name || item.team_name || "NO_NAME";
  ...
  const getMatchHistory = () => {
    ...
    leagues.forEach((league) => {
      if (teamData.ratingHistory[league]) {
        console.log(
          ...
          teamData.ratingHistory[league].length,
        );
    ...
    const sortedMatches = filteredMatches.sort(
      (a, b) => new Date(b.series_date) - new Date(a.series_date),
    );
              ...
              <View style={styles.teamContainer}>
                {transaction.team_from ? (
                  <>
                    ...
                    <Text
                      style={[styles.transferTeamName, { color: theme.text }]}
                      numberOfLines={2}
                    >
  ...
  const renderMapStatsCard = (mapStat) => {
    ...
    const mapWinConditions = winConditions.filter(
      (wc) => wc.mapId === mapStat.mapId,
    );
    ...
    const conditionGroups = {
      ...
      "kills (pre-plant)": mapWinConditions.filter(
        (wc) => wc.condition === "kills (pre-plant)",
      )[0],
      "kills (post-plant)": mapWinConditions.filter(
        (wc) => wc.condition === "kills (post-plant)",
      )[0],
    };
    ...
    return (
      <View
        key={mapStat.mapId}
        style={[styles.mapStatsCard, { backgroundColor: theme.surface }]}
      >
        {/* Background Image */}
        <Image
          source={{
        ...
        <View style={styles.mapStatsContent}>
          ...
          <View style={styles.mapHeader}>
            ...
            <Text style={styles.mapWinLoss}>
              {mapStat.matchWins}W {losses}L • {mapStat.rounds} Rounds
            </Text>
          </View>
      ...
      <SafeAreaView style={styles.stickyHeaderSafeArea}>
        <View style={styles.stickyHeaderContent}>
          <View style={styles.stickyHeaderLeft}>
            ...
            />
            <Text style={[styles.stickyHeaderTitle, { color: theme.text }]}>
              {teamData?.name || teamName}
            </Text>
          </View>
          <View style={styles.stickyHeaderRight}>
            <Text
              style={[styles.stickyHeaderRank, { color: theme.textSecondary }]}
            >
              Rank: #{getTeamRank()}
            </Text>
          </View>
