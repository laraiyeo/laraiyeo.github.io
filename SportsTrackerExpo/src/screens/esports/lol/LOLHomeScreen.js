...
const LOLHomeScreen = ({ navigation, route }) => {
    ...
    try {
      ...
      if (fetchedMatches && fetchedMatches.length > 0) {
        ...
        fetchedMatches.forEach((match, index) => {
          ...
          const easternString = `${
            easternParts.find((p) => p.type === "year").value
          }-${easternParts.find((p) => p.type === "month").value}-${
            easternParts.find((p) => p.type === "day").value
          } ${easternParts.find((p) => p.type === "hour").value}:${
            easternParts.find((p) => p.type === "minute").value
          }:${easternParts.find((p) => p.type === "second").value}`;

          console.log(
            `${index + 1}. ${match.match?.teams?.[0]?.code || "Unknown"} vs ${
              match.match?.teams?.[1]?.code || "Unknown"
            }`
          );
        ...
        const liveData = fetchedMatches.filter((event) => {
          ...
          if (event.state === "unstarted" && event.match && event.match.teams) {
            const hasGameWins = event.match.teams.some(
              (team) => team.result && team.result.gameWins > 0
            );
            if (hasGameWins) {
              console.log(
                "Including unstarted match with gameWins as live:",
                event.match.teams.map((t) => t.code),
                "Wins:",
                event.match.teams.map((t) => t.result?.gameWins)
              );
              return true;
            }
          }
        ...
        liveData.forEach((match, index) => {
          ...
          console.log(
            `${index + 1}. State: ${match.state}, Teams: ${teams.length}`
          );
          if (teams.length >= 2) {
            console.log(
              `   ${teams[0]?.code || "Unknown"} (${
                teams[0]?.result?.gameWins || 0
              }) vs ${teams[1]?.code || "Unknown"} (${
  ...
  const filterCompletedMatches = (matches, filter) => {
    ...
    const yesterdayEastern = new Date(
      todayEastern.getTime() - 24 * 60 * 60 * 1000
    );
    const tomorrowEastern = new Date(
      todayEastern.getTime() + 24 * 60 * 60 * 1000
    );
    ...
    if (filter === "yesterday") {
      filteredData = matches.filter((event) => {
        ...
        const eventDayEastern = new Date(
          `${eventEasternParts.find((p) => p.type === "year").value}-${
            eventEasternParts.find((p) => p.type === "month").value
          }-${eventEasternParts.find((p) => p.type === "day").value}`
        );

        // Debug log for each match
        console.log(
          `Match: ${event.match?.teams?.[0]?.code || "Unknown"} vs ${
            event.match?.teams?.[1]?.code || "Unknown"
          }`
        );
        console.log(`  State: ${event.state}`);
        console.log(
          `  GameWins: ${event.match?.teams?.map(
            (t) => t.result?.gameWins || 0
          )}`
        );
        ...
        console.log(
          `  Is Yesterday: ${
            eventDayEastern.getTime() === yesterdayEastern.getTime()
          }`
        );
      ...
      filteredData = matches.filter((event) => {
        ...
        console.log(`  Eastern Day: ${eventDayEastern.toDateString()}`);
        console.log(
          `  Is Today: ${eventDayEastern.getTime() === todayEastern.getTime()}`
        );
        console.log(`  Is Upcoming (no gameWins): ${isUpcoming(event)}`);
      ...
      filteredData = matches.filter((event) => {
        ...
        console.log(
          `  Is Tomorrow: ${
            eventDayEastern.getTime() === tomorrowEastern.getTime()
          }`
        );
        console.log(`  Is Upcoming (no gameWins): ${isUpcoming(event)}`);
    ...
    console.log(
      `Filtered ${filteredData.length} upcoming matches for ${filter}`
    );
    ...
    console.log(
      `Filtered ${completedFilteredData.length} completed matches for ${filter}`
    );
  ...
  const getTournamentLogo = (tournamentName) => {
    ...
    const matchingLeague = leagues.find((league) => {
      ...
      return (
        leagueName.includes(tournamentLower) ||
        tournamentLower.includes(leagueName)
      );
    });
  ...
  const LiveSeriesCard = ({ match }) => {
    ...
    if (teams.length < 2) {
      console.log(
        "LiveSeriesCard: Skipping render due to insufficient teams:",
        teams.length
      );
      return null;
    }
    ...
    return (
      ...
      >
        ...
        {/* Event info */}
        <Text
          style={[styles.liveEventChildLabel, { color: theme.textTertiary }]}
        >
          {formattedMatch.blockName?.toUpperCase() || "MATCH"}
        </Text>
        <Text
          style={[styles.liveEventName, { color: theme.text }]}
          numberOfLines={2}
        >
          {eventName}
        </Text>
              ...
              {teams[0]?.image ? (
                ...
                >
                  <Text style={styles.teamLogoText}>
                    {teams[0]?.code?.substring(0, 2).toUpperCase() ||
                      teams[0]?.name?.substring(0, 2).toUpperCase() ||
                      "??"}
                  </Text>
                </View>
              )}
          ...
          <View style={styles.liveTeamSide}>
            <View style={styles.liveTeamLogo}>
              {teams[1]?.image ? (
                ...
                >
                  <Text style={styles.teamLogoText}>
                    {teams[1]?.code?.substring(0, 2).toUpperCase() ||
                      teams[1]?.name?.substring(0, 2).toUpperCase() ||
                      "??"}
                  </Text>
                </View>
              )}
            ...
            >
              <View style={styles.eventLogoContainer}>
                {getTournamentLogo(event.eventName) ? (
                  <Image
                    source={{
                      uri: getSafeImageUri(getTournamentLogo(event.eventName)),
                    }}
                ...
                <View style={styles.completedTeamWithLogo}>
                  <View style={styles.completedTeamLogo}>
                    {team.image ? (
                      ...
                      <View
                        style={[
                          styles.completedTeamLogoPlaceholder,
                          { backgroundColor: theme.border },
                        ]}
                      >
  ...
  const getMatchStatus = (match) => {
    switch (match.state) {
      case "inProgress":
        return { text: "LIVE", color: theme.error, bg: `${theme.error}20` };
      case "completed":
        return {
          text: "FINAL",
          color: theme.textSecondary,
      ...
      default:
        return {
          text: match.state?.toUpperCase() || "TBD",
          color: theme.textSecondary,
          bg: "transparent",
        };
    }
      ...
      >
        <View style={styles.matchHeader}>
          <View style={styles.tournamentInfo}>
            <Text
              style={[styles.tournamentName, { color: theme.textSecondary }]}
              numberOfLines={1}
            >
              {formattedMatch.tournament}
            </Text>
            <Text
              style={[styles.seriesInfo, { color: theme.textTertiary }]}
              numberOfLines={1}
            >
        ...
        <View style={styles.teamsContainer}>
          {teams.length >= 2 ? (
            ...
            <View style={styles.noTeamsContainer}>
              <Text
                style={[styles.noTeamsText, { color: theme.textSecondary }]}
              >
                {formattedMatch.name}
              </Text>
            </View>
          ...
          <View style={styles.section}>
            ...
            >
              {completedSeries.map((match, index) => (
                <CompletedSeriesCard
                  key={`completed-${match.match?.id || index}`}
                  match={match}
                />
              ))}
            </ScrollView>
