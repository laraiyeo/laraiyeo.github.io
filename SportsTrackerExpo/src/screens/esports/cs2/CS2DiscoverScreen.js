...
const CS2DiscoverScreen = ({ navigation }) => {
  ...
  const sortEventsByStatusPrizeAndDate = (events) => {
    return events.sort((a, b) => {
      // First priority: sort by status (current/live > upcoming > completed)
      const getStatusPriority = (event) => {
        if (event.status === "current") return 0; // Current/live events first
  ...
  const loadData = async () => {
    try {
      ...
      const [currentTournaments, upcomingTournaments, recentTournaments] =
        await Promise.all([
      ...
      const currentEvents = (currentTournaments.edges || []).map(
        (tournament) => ({
      ...
      const upcomingEvents = (upcomingTournaments.edges || []).map(
        (tournament) => ({
          id: tournament.node.id,
          slug: tournament.node.slug,
          name: tournament.node.name,
      ...
      const recentEvents = (recentTournaments.edges || []).map(
        (tournament) => ({
          id: tournament.node.id,
          slug: tournament.node.slug,
          name: tournament.node.name,
      ...
      const currentList = currentEvents.filter(
        (event) => event.status === "current"
      );
      const upcomingList = upcomingEvents;
      const completedList = recentEvents;

      // Sort current tournaments by status/prize/date, then take top 5 for featured section
      const featured = sortEventsByStatusPrizeAndDate(currentList).slice(0, 5);
    ...
    return (
      ...
      >
        ...
        <View style={styles.trendingContent}>
          ...
          <Text
            style={[styles.trendingSubtitle, { color: theme.textSecondary }]}
            numberOfLines={1}
          >
