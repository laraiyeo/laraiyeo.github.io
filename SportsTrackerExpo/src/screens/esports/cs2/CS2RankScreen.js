...
import { getTeamRankings } from '../../../services/cs2Service';

// Region mapping for API calls
const regionMapping = {
  'WRLD': null, // No filter for worldwide
  'EURO': 'Europe',
  'AMER': 'Americas',
  'ASIA': 'Asia'
};
...
const CS2RankScreen = () => {
  ...
  const [selectedRegion, setSelectedRegion] = useState('WRLD');
  ...
  const fetchRankings = async () => {
    ...
    try {
      ...
      const data = await getTeamRankings(apiRegion);
      setRankings(data || []);
    } catch (error) {
    ...
    return (
      ...
      >
        ...
        <View style={styles.teamInfo}>
          ...
          <Text allowFontScaling={false} style={[styles.countryText, { color: theme.textSecondary }]}>
            {team.country}
          </Text>
        </View>
        ...
        <View style={styles.teamStats}>
          <Text allowFontScaling={false} style={[styles.statText, { color: theme.textSecondary }]}>
            {team.score || Math.floor(Math.random() * 9000) + 1000}
          </Text>
        </View>
      </TouchableOpacity>
    );
      ...
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        <View style={styles.rankingsContainer}>
          <View style={styles.regionButtonsContainer}>
            {['WRLD', 'EURO', 'AMER', 'ASIA'].map((region) => (
              <TouchableOpacity
                key={region}
                style={[
                  styles.regionButton,
