...
// OpenF1 request cache (module-level, TTL: 1 minute)
const openF1RequestCache = new Map();
const OPENF1_CACHE_TTL = 60 * 1000;
...
const _setOpenF1Cache = (key, data) => {
  openF1RequestCache.set(key, { data, expiry: Date.now() + OPENF1_CACHE_TTL });
};
...
// ─── F1 Session Share Card ────────────────────────────────────────────────────

const buildF1HeadshotUrl = (athleteId) =>
  `https://a.espncdn.com/combiner/i?img=/i/headshots/rpm/players/full/${athleteId}.png&w=200`;
...
const F1SessionShareCard = ({
  ...
  const top3 = [...(competition.competitors || [])]
  ...
  // Mirrors the results-tab elimination / qual-time logic
  const hasAnyQual2Ms = (competition.competitors || []).some(
    (c) => c?.qual2Ms && Number(c.qual2Ms) > 0,
  );
  const hasAnyQual3Ms = (competition.competitors || []).some(
  ...
  const getTime = (driver, idx) => {
    ...
    if (driver.liveStats?.splits?.categories) {
      const categories = driver.liveStats.splits.categories;
      ...
      if (gapToLeaderCategory?.stats) {
        ...
        if (positionStat && positionStat.value === 1) {
          return { primary: driver.totalTime || "Leader", secondary: null };
        }
        const gapStat = gapToLeaderCategory.stats.find(
        ...
        if (gapStat) {
          ...
          const gap = gapStat.displayValue || String(gapStat.value);
          return driver.totalTime
            ? { primary: driver.totalTime, secondary: gap }
            : { primary: gap, secondary: null };
        }
      }
      ...
      if (generalCategory?.stats) {
        ...
        if (behindLapsStat?.value != null) {
          const lapsStr = `+${behindLapsStat.value} Laps`;
          return driver.totalTime
            ? { primary: driver.totalTime, secondary: lapsStr }
            : { primary: lapsStr, secondary: null };
        }
      }
    }
    ...
    const behind =
      driver.behindTime ||
      (driver.behindLaps != null ? `+${driver.behindLaps} Laps` : null);
    if (driver.totalTime && behind) {
      return { primary: driver.totalTime, secondary: behind };
    }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      console.warn("F1 share failed", e);
    } finally {
      setSharing(false);
    }
  };
      ...
      <View style={f1CardStyles.overlay}>
        ...
        >
          <View
            style={[
              f1CardStyles.card,
              { backgroundColor: theme.surface, width: CARD_SIZE },
            ]}
          >
            {/* ── Header ── */}
            <View
            ...
            >
              ...
              {/* Flag + venue • session time */}
              {venueLine ? (
                <View style={f1CardStyles.venueRow}>
                  {circuitInfo?.countryFlag ? (
                    <Image
                      source={{ uri: circuitInfo.countryFlag }}
            ...
            <View style={f1CardStyles.podiumRow}>
              {top3.map((driver, idx) => {
                const teamColor = driver.teamColor || colors.primary;
                const badgeTextColor = getF1TextOnColor(teamColor);
                const time = getTime(driver, idx);
                const pos = driver.order ?? idx + 1;
                const lastName =
                ...
                return (
                  <View key={driver.id || idx} style={f1CardStyles.driverCol}>
                    {/* Headshot + position badge */}
                    <View style={f1CardStyles.headshotWrap}>
                      ...
                      >
                        <F1CardDriverPhoto
                          url={buildF1HeadshotUrl(driver.id)}
                          name={driver.name}
                          teamColor={teamColor}
                        />
                      </View>
                    ...
                    {/* Constructor */}
                    <Text
                      style={[f1CardStyles.driverTeam, { color: teamColor }]}
                      numberOfLines={1}
                    >
                      {driver.manufacturer || ""}
                    </Text>
            ...
            <View
              style={[
                f1CardStyles.cardFooter,
                { borderTopColor: theme.border },
              ]}
            >
              <Text style={[f1CardStyles.brandText, { color: theme.text }]}>
                SportsHeart{" "}
...
const F1RacerShareCard = ({
  ...
  // Session label (same logic as F1SessionShareCard)
  const sessionLabelPre =
    competition?.type?.displayName || competition?.name || "Session";
  const abbr = (competition?.type?.abbreviation || "").toLowerCase();
  const isFP = abbr.startsWith("fp");
  ...
  for (const d of driverEntries) {
    ...
    const broadcastName = (d.broadcast_name || "").toLowerCase();
    if (
      (lastName && normalizedName.includes(lastName)) ||
      fullName === normalizedName ||
      broadcastName === normalizedName
    ) {
      matchedDriver = d;
      break;
    }
  }
  const driverNumber = matchedDriver?.driver_number ?? null;
  ...
  const behindText =
    competitor?.behindLaps != null
      ...
      : competitor?.behindTime || null;

  // For live races, extract gap/time from liveStats categories
  const liveTimeDisplay = (() => {
    const categories = competitor?.liveStats?.splits?.categories;
    if (!categories) return null;
    const gapCat = categories.find((cat) => cat.name === "gapToLeader");
    if (gapCat?.stats) {
    ...
    if (genCat?.stats) {
      const btStat = genCat.stats.find((s) => s.name === "behindTime");
      if (btStat?.displayValue) return btStat.displayValue;
      const blStat = genCat.stats.find((s) => s.name === "behindLaps");
      if (blStat?.value != null) return `+${blStat.value} Laps`;
    }
    return null;
  })();
  ...
  const driverLaps =
    driverNumber != null
      ? (laps || []).filter((l) => Number(l.driver_number) === driverNumber)
      : [];

  // Top speed
  const topSpeed = driverLaps.reduce((best, l) => {
    const spd = Number(l.st_speed);
    return !isNaN(spd) && spd > best ? spd : best;
  }, 0);
  const topSpeedDisplay = topSpeed > 0 ? `${Math.round(topSpeed)}` : "-";

  // Overtakes count
  const overtakeCount =
    driverNumber != null
      ? (overtakes || []).filter(
          (o) => Number(o.overtaking_driver_number) === driverNumber,
        ).length
      : null;
  ...
  const formatLapSec = (sec) => {
    if (sec == null) return "-";
    const mins = Math.floor(sec / 60);
    const remainder = sec - mins * 60;
    return `${mins}:${remainder.toFixed(3).padStart(6, "0")}`;
  };
  const fastestLapDisplay = formatLapSec(fastestLapSec);

  // Best sector — always computed
  // Uses segments_sector arrays: 2051 = purple/fastest mini-sector
  let bestSectorLabel = "BEST SEC";
  let bestSectorVal = "-";
  ...
  if (driverLaps.length > 0) {
    ...
    for (const l of driverLaps) {
      ...
      if (Array.isArray(l.segments_sector_2))
        countS2 += l.segments_sector_2.filter((v) => v === 2051).length;
      if (Array.isArray(l.segments_sector_3))
        countS3 += l.segments_sector_3.filter((v) => v === 2051).length;
    }
    // Dominant sector = most 2051 occurrences
    const sectorMeta = [
      { n: "S1", count: countS1, durKey: "duration_sector_1" },
      { n: "S2", count: countS2, durKey: "duration_sector_2" },
      { n: "S3", count: countS3, durKey: "duration_sector_3" },
    ].sort((a, b) => b.count - a.count);
    const dominant = sectorMeta[0];
    // Find best time for that sector and the lap it occurred on
  ...
  // Team color
  const rawTeamColor = competitor?.teamColor;
  const teamColor = rawTeamColor
    ? rawTeamColor.startsWith("#")
      ? rawTeamColor
      : `#${rawTeamColor}`
    : colors.primary;
  ...
  const teamDisplayName =
    competitor?.manufacturer || matchedDriver?.team_name || "";
  const teamLogoName =
    F1_NAME_MAP[teamDisplayName] ||
    teamDisplayName.toLowerCase().replace(/\s+/g, "");
  const _currentYear = new Date().getFullYear();
  const teamLogoUrl = teamDisplayName
    ? `https://media.formula1.com/image/upload/c_fit,h_1080/q_auto/v1740000000/common/f1/${_currentYear}/${teamLogoName}/${_currentYear}${teamLogoName}logowhite.webp`
    : null;
  const carUrl = teamDisplayName
    ? `https://media.formula1.com/image/upload/c_lfill,w_3392/q_auto/v1740000000/common/f1/${_currentYear}/${teamLogoName}/${_currentYear}${teamLogoName}carright.webp`
    : null;

  // Headshot
  const headshotUrl = competitor?.id
    ? `https://a.espncdn.com/combiner/i?img=/i/headshots/rpm/players/full/${competitor.id}.png&w=200`
    : null;
  ...
  // Summary stats (3 shown next to headshot)
  const timeShort =
    timeDisplay.length > 10 ? timeDisplay.slice(0, 10) : timeDisplay;
  const summaryStats = [
    { label: "POS", val: String(position) },
    { label: "TIME", val: timeShort },
  ...
  const flagUri = circuitInfo?.countryFlag || null;
    ...
    try {
      ...
      if (Platform.OS === "ios") {
        await Sharing.shareAsync(uri, {
          mimeType: "image/png",
          UTI: "public.png",
          dialogTitle: "Share Driver Stats",
        });
      } else {
      ...
      console.error("Error sharing racer card:", e);
      Alert.alert("Error", "Failed to share driver card");
    } finally {
      setSharing(false);
    }
  };
    ...
    >
      <View style={f1CardStyles.overlay}>
        {loading ? (
          <View
            style={[
              f1RacerCardStyles.card,
              f1RacerCardStyles.loadingOverlay,
              { backgroundColor: theme.surface, width: CARD_SIZE },
            ]}
          >
          ...
          >
            ...
            >
              ...
              >
                {/* Top row: driver badge + race flag & name */}
                  ...
                  <View style={f1RacerCardStyles.nameBlock}>
                    <View style={f1RacerCardStyles.summaryRow}>
                      {summaryStats.map(({ label, val }) => (
                        <View key={label} style={f1RacerCardStyles.summaryCell}>
                          <Text
                            style={[
              ...
              <View style={f1RacerCardStyles.statGrid}>
                {sixStats.map(({ label, val, topRight }, i) => (
                  <View
                    key={`${label}-${i}`}
                    style={[
...
const RaceDetailsScreen = ({ route }) => {
  ...
  // Session share card state
  const [sessionCardVisible, setSessionCardVisible] = useState(false);
  const [sessionCardComp, setSessionCardComp] = useState(null);

  // Racer share card state
  const [racerCardVisible, setRacerCardVisible] = useState(false);
  const [racerCardLoading, setRacerCardLoading] = useState(false);
  const [racerCardData, setRacerCardData] = useState(null); // { competitor, overtakes, laps }
  ...
  // Transient live-state guards to avoid flipping off quickly
  const liveFalseStreakRef = useRef(0);
  const lastLiveSeenRef = useRef(0);
  // Store the most recently fetched status synchronously for immediate checks
  const lastFetchedRaceStatusRef = useRef(null);
  // Guard against overlapping live-update runs and allow aborting
  const isUpdatingRef = useRef(false);
  const liveUpdateAbortControllerRef = useRef(null);
  const lastLiveStartRef = useRef(0);
  ...
  const computeShouldRunNow = () => {
    ...
    const isLiveNow = (() => {
      try {
        return checkIfRaceIsLive();
      } catch (e) {
        return false;
      }
    })();

    const compResultsNow = competitionResultsRef.current || {};
    const hasAnyLiveCompetitionNow =
      raceData.competitions?.some((comp) => {
        const competition = compResultsNow[comp.id];
    ...
    const recentlyLiveNow =
      lastLiveSeenRef.current && Date.now() - lastLiveSeenRef.current < 15000;
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      // Update ref immediately to avoid races with the interval callback
      if (typeof isScreenFocusedRef !== "undefined") {
  ...
  const getLiveGapToLeader = (competitor) => {
    // First priority: Check for live gapToLeader data
    if (competitor.liveStats?.splits?.categories) {
      const categories = competitor.liveStats.splits.categories;
      ...
      // If no gapToLeader category found, check general category for live data
      const generalCategory = categories.find((cat) => cat.name === "general");
    ...
    // If no live stats available, fall back to completed race data
    if (competitor.behindTime) {
      return competitor.behindTime;
    }
  ...
  useEffect(() => {
    ...
    const loadStatus = async () => {
      // If live updates are currently running, or the stream modal is open,
      // avoid clearing/fetching the status here to prevent a race condition
      // where status is briefly null and causes check logic to flip to not-live.
  ...
  }, [selectedCompetitionId, competitionResults, liveUpdateInterval]);
  ...
  const fetchCompetitionResultsForEvent = async (eventData) => {
        ...
        try {
          ...
          if (statusRef) {
            try {
              ...
              if (statusResponse.ok) {
                ...
                console.log(
                  `[fetchCompetitionResultsForEvent] Fetched status for ${compName}:`,
                  {
                    compId,
          ...
          const competitorPromises = competitors.map(async (c) => {
            ...
            return {
              ...
              raw: c,
              isEliminatedFromQual: false, // Will be set to true for bottom 5 in Q1/Q2
            };
          });
          ...
          // Check if this is a qualifying competition and mark eliminated drivers
          const compTypeStr = (
            (compType.abbreviation || "") +
            " " +
            (compType.text || "") +
          ...
          const isQualOrSS =
            compTypeStr.includes("qual") || compTypeStr.includes("ss");

          if (isQualOrSS && resolved.length > 0) {
            // Sort by qualifying times to find bottom 5
          ...
          results[compId] = {
            ...
            status: statusData, // Include fetched status
            date: comp.date,
    ...
    console.log("[fetchCompetitionResultsForEvent] Final results:", {
      ...
      order,
      resultsWithStatus: Object.entries(results).map(([id, r]) => ({
        id,
        name: r.name,
        hasStatus: !!r.status,
  ...
  const fetchOpenF1Meetings = async (year = new Date().getFullYear()) => {
    ...
    const cached = _getFromOpenF1Cache(cacheKey);
  ...
  const fetchOpenF1Stints = async (sessionKey) => {
    const cacheKey = `stints_${sessionKey}`;
    const cached = _getFromOpenF1Cache(cacheKey);
    if (cached) return cached;
    try {
      ...
      _setOpenF1Cache(cacheKey, stints);
      return stints;
    } catch (error) {
  ...
  const fetchOpenF1Overtakes = async (sessionKey) => {
    const cacheKey = `overtakes_${sessionKey}`;
    const cached = _getFromOpenF1Cache(cacheKey);
    if (cached) return cached;
    try {
      const response = await fetch(
        `https://api.openf1.org/v1/overtakes?session_key=${sessionKey}`,
      );
      if (!response.ok)
        throw new Error(`Failed to fetch overtakes: ${response.status}`);
      const data = await response.json();
      _setOpenF1Cache(cacheKey, data);
      return data;
    } catch (error) {
      console.error("Error fetching OpenF1 overtakes:", error);
      return [];
    }
  };

  const fetchOpenF1Laps = async (sessionKey) => {
    const cacheKey = `laps_${sessionKey}`;
    const cached = _getFromOpenF1Cache(cacheKey);
    if (cached) return cached;
    try {
      const response = await fetch(
      ...
      if (!response.ok)
        throw new Error(`Failed to fetch laps: ${response.status}`);
      const data = await response.json();
      _setOpenF1Cache(cacheKey, data);
      return data;
    } catch (error) {
  ...
  const openRacerShareCard = async (competitor) => {
    const competition = competitionResults[selectedCompetitionId] || null;
    setRacerCardLoading(true);
    setRacerCardData({ competitor, overtakes: [], laps: [], competition });
    setRacerCardVisible(true);
    const sessionKey = openF1Data.selectedSessionKey;
    if (sessionKey) {
      try {
        const [overtakesData, lapsData] = await Promise.all([
  ...
  const checkIfRaceIsLive = () => {
    ...
    // Primary check: Use status from competitionResults (fetched during initial load)
    if (selectedCompetitionId && competitionResults[selectedCompetitionId]) {
      const competition = competitionResults[selectedCompetitionId];

      // Use the status fetched in fetchCompetitionResultsForEvent
      if (competition.status?.type) {
        ...
        if (
          ...
          statusState === "post" ||
          statusState === "final" ||
          statusDescription === "End of Session"
        ) {
          console.log(
        ...
        // If status is "pre" or scheduled, not live yet
    ...
    // Secondary check: if we have race status (from separate fetch), use it as fallback
    const effectiveRaceStatus = lastFetchedRaceStatusRef.current || raceStatus;
    if (effectiveRaceStatus && effectiveRaceStatus.type) {
      const statusState = effectiveRaceStatus.type.state;
      const completed = effectiveRaceStatus.type.completed;
  ...
  }, [isScreenFocused, streamModalVisible]);
  ...
  const updateLiveRaceStatus = async () => {
    ...
    // Respect UI state: don't fetch while modal open or screen not focused
    // Use refs here to avoid stale closures from the interval callback.
    ...
    liveUpdateAbortControllerRef.current = new AbortController();
    const signal = liveUpdateAbortControllerRef.current.signal;
    ...
    try {
      ...
      // Stabilize transient flips: require two consecutive 'not live' checks
      if (isLive) {
        liveFalseStreakRef.current = 0;
        lastLiveSeenRef.current = Date.now();
        setIsLiveRace(true);
      } else {
        liveFalseStreakRef.current = (liveFalseStreakRef.current || 0) + 1;
              ...
              try {
                ...
                if (competitor.statistics && competitor.statistics.$ref) {
                    ...
                    for (const category of categories) {
                      if (category.stats) {
                        for (const stat of category.stats) {
                          ...
                          // Prefer stat-provided laps when present
                          if (
                          ...
                          // Prefer stat-provided totalTime when present
                          if (
                          ...
                          // Position (used for ordering/delta) — capture numeric position if present
                          if (
                            statName === "position" ||
                            statName.includes("position")
                          ) {
                            const n = Number(statValue);
                            if (!Number.isNaN(n)) liveUpdatedPosition = n;
                          }
                        }
                      }
                    }
                  ...
                  const updatedCompetitor = {
                    ...competitor,
                    liveStats: statsData,
                    // Use live-updated values when available (fall back to previous values)
                    laps:
                      liveUpdatedLaps != null
                        ? liveUpdatedLaps
                        : competitor.laps,
                    totalTime:
                      liveUpdatedTotalTime != null
                        ? liveUpdatedTotalTime
                        : competitor.totalTime,
                    // Update order if the live feed provides a position value
                    order:
                      liveUpdatedPosition != null
                        ? liveUpdatedPosition
                        : competitor.order,
                  };
  ...
  const startLiveUpdates = () => {
    ...
    const interval = setInterval(() => {
      // Check UI state (use refs to avoid stale closure values)
      if (!isScreenFocusedRef.current || streamModalVisibleRef.current) {
        console.log(
      ...
      // Update live race status
      console.log("[liveUpdateInterval] Running scheduled update");
      updateLiveRaceStatus();
    }, 5000); // Update every 5 seconds

    setLiveUpdateInterval(interval);
    lastLiveStartRef.current = Date.now();
  };

  const stopLiveUpdates = () => {
    // Before stopping, double-check whether we actually should stop now.
    if (computeShouldRunNow()) {
      console.log("[stopLiveUpdates] Skipping stop — computeShouldRunNow true");
      return;
    }
    ...
    // Abort any in-flight fetches initiated by updateLiveRaceStatus
    if (liveUpdateAbortControllerRef.current) {
      try {
        liveUpdateAbortControllerRef.current.abort();
      } catch (e) {}
      liveUpdateAbortControllerRef.current = null;
    }
    // Clear running flag
  ...
  useEffect(() => {
    // Decide whether we should be running live updates before mutating interval state.
    const shouldRun = (() => {
      if (!(isScreenFocused && !streamModalVisible && raceData)) return false;
      const isLive = checkIfRaceIsLive();
      ...
      // Also allow a short grace window after we last observed live activity
      const recentlyLive =
        lastLiveSeenRef.current && Date.now() - lastLiveSeenRef.current < 15000;
    ...
    // Diagnostic log to understand why we start/stop
    try {
      // Also compute more detailed diagnostics
      const isLiveDiagnostic = checkIfRaceIsLive();
      const compStatuses = (raceData?.competitions || []).map((comp) => {
        const cr = competitionResults?.[comp.id] || {};
        return {
          id: comp.id,
          statusState: cr?.status?.type?.state || null,
          completed: cr?.status?.type?.completed || false,
          hasResults: !!(cr && cr.competitors && cr.competitors.length),
        };
      });

      console.log("[useEffect] live-run-check", {
        at: new Date().toISOString(),
        isScreenFocused,
        streamModalVisible,
        hasRaceData: !!raceData,
        liveUpdateIntervalExists: !!liveUpdateInterval,
        shouldRun,
        isLiveDiagnostic,
        compStatuses,
        lastLiveSeen: lastLiveSeenRef.current,
        lastLiveStart: lastLiveStartRef.current,
      });
    } catch (e) {}

    if (shouldRun) {
      if (!liveUpdateInterval) {
        console.log("[useEffect] Conditions met — starting live updates");
        startLiveUpdates();
      } else {
        // already running
      }
      ...
      if (liveUpdateInterval) {
        console.log("[useEffect] Conditions not met — stopping live updates");
        stopLiveUpdates();
      } else {
        // already stopped
      }
              ...
              {(competitionOrder && competitionOrder.length
                ...
                return (
                  <TouchableOpacity
                    ...
                    onLongPress={() => {
                      setSessionCardComp(competitionResults[comp.id] || null);
                      ...
                      <View style={styles.racerRight}>
                        ...
                        {isQual ? (
                          <View style={{ alignItems: "flex-end" }}>
                            {isLiveRace && !r.isEliminatedFromQual ? (
                              <>
                                {/* Live qualifying: show position and live gap for non-eliminated drivers */}
                                <Text
                                  allowFontScaling={false}
                                  style={[
                                    styles.totalTime,
                              ...
                              <>
                                {/* Completed qualifying OR eliminated drivers: show Q1/Q2/Q3 times */}
                                {hasAnyQual2Ms &&
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── Racer Share Card ── */}
      <F1RacerShareCard
        visible={racerCardVisible}
        onClose={() => {
          setRacerCardVisible(false);
          setRacerCardData(null);
        }}
        competitor={racerCardData?.competitor || null}
        overtakes={racerCardData?.overtakes || []}
        laps={racerCardData?.laps || []}
        openF1Drivers={openF1Data.drivers}
        raceName={raceData?.name || raceName}
        circuitInfo={circuitInfo}
        theme={theme}
        colors={colors}
        loading={racerCardLoading}
        competition={racerCardData?.competition || null}
      />
    </View>
  );
};
