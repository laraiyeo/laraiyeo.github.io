...
const NHLPlayerPageScreen = ({ route, navigation }) => {
  ...
  const [selectedGameStatsDetails, setSelectedGameStatsDetails] =
    useState(null);
  const [loadingSelectedGameStats, setLoadingSelectedGameStats] =
    useState(false);
  ...
  const getHeadshotSource = (p) => {
    try {
      if (headshotFailed) return require("../../../assets/nhl.png");
      if (!p) return require("../../../assets/nhl.png");
      // Prefer provided headshot fields (could be string or object)
      if (p.headshot)
        return typeof p.headshot === "string"
          ? { uri: convertToHttps(p.headshot) }
          : {
              uri: convertToHttps(
                p.headshot.href || p.headshot.uri || p.headshot.url
              ),
            };
      if (p.athlete && p.athlete.headshot)
        return typeof p.athlete.headshot === "string"
          ? { uri: convertToHttps(p.athlete.headshot) }
          : {
              uri: convertToHttps(
                p.athlete.headshot.href ||
                  p.athlete.headshot.uri ||
                  p.athlete.headshot.url
              ),
            };

      // Try common alternate locations
      if (p.profile && p.profile.headshot)
        return { uri: convertToHttps(p.profile.headshot) };
      if (p.profile && p.profile.image)
        return { uri: convertToHttps(p.profile.image) };

      // Build combiner 88x88 ESPN headshot by id (same as TeamPage helper)
      const id =
        p.id ||
        p.athlete?.id ||
        (typeof route?.params?.playerId !== "undefined"
          ? route.params.playerId
          : null);
      if (id)
        return {
          uri: convertToHttps(
            `https://a.espncdn.com/combiner/i?img=/i/headshots/nhl/players/${id}.png&w=88&h=88`
          ),
        };
    } catch (e) {
      // fall through
    }
    return require("../../../assets/nhl.png");
  };

  // Convert ESPN numeric team id to NHL abbreviation (same mapping used in ScoreboardScreen)
  ...
  // Convert ESPN numeric team id to full NHL team name
  ...
  const getTeamLogoUrl = (teamParam, useDarkMode = isDarkMode) => {
    ...
    if (failedLogos.has(primaryKey)) {
      return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nhl/${
        useDarkMode ? "500" : "500-dark"
      }/${normalized}.png&w=200&h=200`;
    }
    return `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nhl/${
  ...
  const getTeamLogoFallbackUrl = (teamParam, useDarkMode = isDarkMode) => {
    if (!teamParam) return require("../../../assets/nhl.png");
    let token = null;
    if (typeof teamParam === "object") {
    ...
    if (!token) return require("../../../assets/nhl.png");
    let normalizedToken = token;
    if (/^\d+$/.test(String(token))) {
      const abbr = convertTeamIdToAbbr(token);
      if (abbr) normalizedToken = abbr;
    ...
    const normalized = String(normalizedToken).toLowerCase();
    return {
      uri: `https://a.espncdn.com/combiner/i?img=/i/teamlogos/nhl/500/${normalized}.png&w=200&h=200`,
    };
  };
  ...
  // Map various position display names to a simplified category for hockey
  const positionCategoryFromDisplay = (posDisplay) => {
    if (!posDisplay) return null;
    const p = String(posDisplay).toLowerCase();
    // Goalie positions
    if (p.includes("goalie") || p.includes("goaltender") || p === "g")
      return "G";
    // Defenseman positions
    if (p.includes("defense") || p.includes("defens") || p === "d") return "D";
    // Forward positions - Center, Left Wing, Right Wing
    if (p.includes("center") || p === "c" || p.includes("centre")) return "F";
    if (
      p.includes("wing") ||
      ...
      return "F";
    if (p.includes("forward") || p === "f") return "F";
    return null;
  };
  ...
  const getGameCardStatsString = (game) => {
    try {
      ...
      if (!statsArr || !Array.isArray(statsArr) || statsArr.length === 0)
        return "";
      const posCat = positionCategoryFromDisplay(
        playerData?.position?.displayName || playerData?.position?.name || ""
      );
      // Helper to safely read index
      const s = (i) =>
        typeof statsArr[i] !== "undefined" && statsArr[i] !== null
          ? statsArr[i]
          : "";
      switch (posCat) {
        case "F": // Forwards (Center, Left Wing, Right Wing)
          // G | A | +/- (Goals, Assists, Plus/Minus)
          return `${s(0)} G • ${s(1)} A • ${s(3)} +/-`;
        case "D": // Defensemen
          // G | A | +/- (same as forwards)
          return `${s(0)} G • ${s(1)} A • ${s(3)} +/-`;
        case "G": // Goalies
          // GA | SA | SV% (Goals Against, Shots Against, Save Percentage)
          return `${s(6)} GA • ${s(8)} SA • ${s(10)} SV%`;
        default:
          return "";
      }
    } catch (e) {
  ...
  const getTileStatsForSeason = (item) => {
    try {
      ...
      if (
        !statsResp ||
        !statsResp.splits ||
        !Array.isArray(statsResp.splits.categories)
      )
        return [];
      ...
      const findStatInCategories = (
        categoryNames = [],
        candidateNames = []
      ) => {
        const cats = statsResp.splits.categories || [];
        const loweredCategoryNames = categoryNames.map((cn) =>
        ...
        for (const cat of cats) {
          ...
          if (!loweredCategoryNames.some((cn) => catName.includes(cn)))
            continue;
      ...
      const makeStatEntry = (s) => {
        if (!s) return null;
        const label =
          s.displayName || s.name || s.shortDisplayName || s.abbreviation || "";
        const value =
          s.displayValue != null
            ? s.displayValue
            : s.value != null
      ...
      switch (posCat) {
        case "F": {
          ...
          const g = findStatByNames(["goals"]);
          const a = findStatByNames(["assists", "assist"]);
          const pts = findStatByNames(["points", "point"]);
          tileStats = [
            makeStatEntry(g),
            makeStatEntry(a),
            makeStatEntry(pts),
        ...
        case "G": {
          // Goalies
          const w = findStatByNames(["wins", "win"]);
          const gaa = findStatByNames(["goalsAgainst"]);
          const svp = findStatByNames(["SV"]);
          tileStats = [
            makeStatEntry(w),
            makeStatEntry(gaa),
            makeStatEntry(svp),
        ...
        default: {
          ...
          if (Array.isArray(firstCat.stats) && firstCat.stats.length > 0) {
            tileStats = firstCat.stats
              .slice(0, 3)
              .map((s) => makeStatEntry(s))
              .filter(Boolean);
          }
        }
      }
  ...
  const playerTeamFallbackUrl = useMemo(() => {
    if (!playerData?.team?.id) return require("../../../assets/nhl.png");
  ...
  const getStableTeamLogoUrl = useCallback(
    (teamParam) => {
      if (!teamParam) return null;
  ...
  const getStableTeamFallbackUrl = useCallback(
    (teamParam) => {
      if (!teamParam) return require("../../../assets/nhl.png");
    ...
    try {
      ...
      await Promise.all(
        seasons.map(async (season) => {
          try {
            const url = `https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/seasons/${season}/athletes/${playerId}/eventlog?lang=en&region=us&limit=1`;
            ...
            if (json && json.teams && typeof json.teams === "object") {
              const teamList = Object.values(json.teams).map((t) => ({
                id: t.id,
                abbreviation: convertTeamIdToAbbr(t.id) || String(t.id),
              }));
  ...
  const loadCareerData = async () => {
    try {
      ...
      if (
        gameLogData &&
        Array.isArray(gameLogData.seasonTypes) &&
        gameLogData.seasonTypes.length > 0
      ) {
        seasons = gameLogData.seasonTypes
          .map((s) => s.season || s.seasonYear || s.name || s.label)
          .filter(Boolean);
      }
      ...
      const fetches = seasons.map((season) => {
        const url = `https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/seasons/${season}/types/2/athletes/${playerId}/statistics?lang=en&region=us`;
      ...
      const seasonResults = settled.map((s, idx) => ({
        season: seasons[idx],
      ...
      const careerItems = seasonResults.map((sr) => {
        ...
        if (statsResp) {
          if (Array.isArray(statsResp.teams) && statsResp.teams.length > 0) {
            ...
            let id =
              t && (t["$ref"] || t.href)
                ? (String(t["$ref"] || t.href).match(/teams\/(\d+)/) || [])[1]
                : t && t.id
                ? t.id
                : null;
            const abbr =
              t && (t.abbreviation || t.abbr || t.alias)
                ? t.abbreviation || t.abbr || t.alias
                : null;
            const name =
              t && (t.displayName || t.name) ? t.displayName || t.name : null;
          ...
          teams = [
            {
              ...
              abbreviation:
                playerData.team.abbreviation || playerData.team.abbr || null,
              displayName:
                playerData.team.displayName || playerData.team.name || null,
            },
          ];
        }
        ...
        if (
          ...
          statsResp.splits &&
          Array.isArray(statsResp.splits.categories) &&
          statsResp.splits.categories.length > 0
        ) {
          ...
          switch (posCat) {
            case "F": {
              // Forwards (Centers, Wings)
              const g = findStatByNames(["goals", "goal"]);
              const a = findStatByNames(["assists", "assist"]);
              const pts = findStatByNames(["points", "point"]);
              tileStats = [
                makeStatEntry(g),
            ...
            case "G": {
              // Goalies
              const w = findStatByNames(["wins", "win"]);
              const gaa = findStatByNames(["goalsAgainstAverage", "gaa"]);
              const svp = findStatByNames([
                "savePercentage",
                "savePct",
                "saves",
              ]);
  ...
  const fetchPlayerData = async () => {
    try {
      ...
      const maybeAbbr =
        typeof teamId === "string" && /^[A-Za-z]{2,5}$/.test(teamId)
          ? teamId
          : null;
      const basicPlayerData = {
        ...
        headshot: {
          href: `https://a.espncdn.com/combiner/i?img=/i/headshots/nhl/players/full/${playerId}.png`,
        },
      };
      ...
      const statsUrl = `https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/seasons/${seasonYear}/types/2/athletes/${playerId}/statistics?lang=en&region=us`;
      const gamelogUrl = `https://site.web.api.espn.com/apis/common/v3/sports/hockey/nhl/athletes/${playerId}/gamelog`;
      const coreUrl = `https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/seasons/${seasonYear}/athletes/${playerId}`;

      const [siteRes, splitsRes, glRes, coreRes, statsRes] =
      ...
      let splitsData = null;
      if (
        splitsRes.status === "fulfilled" &&
        splitsRes.value &&
        splitsRes.value.ok
      ) {
        try {
          splitsData = await splitsRes.value.json();
        } catch (e) {
          splitsData = null;
        }
      }
      ...
      if (glRes.status === "fulfilled" && glRes.value && glRes.value.ok) {
        try {
          gamelogData = await glRes.value.json();
        } catch (e) {
          gamelogData = null;
        }
      }
      ...
      if (coreRes.status === "fulfilled" && coreRes.value && coreRes.value.ok) {
        try {
          coreData = await coreRes.value.json();
        } catch (e) {
          coreData = null;
        }
      }
      ...
      if (siteData && siteData.athlete) {
        ...
        const position = athlete.position ||
          athlete.positions?.[0] || { displayName: "N/A" };
        ...
        if (teamFromCore && typeof teamFromCore === "object") {
          ...
          coreData.teams &&
          Array.isArray(coreData.teams) &&
          coreData.teams.length > 0
        ) {
      ...
      if (
        ...
        try {
          // Priority lists per category for NHL stats.
          ...
          const chooseStatsForCategory = (cat) => {
            ...
            const prefList =
              preferredByCategory[cat.name] ||
              preferredByCategory[cat.abbreviation] ||
              preferredByCategory.default;

            // Helper to push a stat object into chosen array transformed to { label, value, rank }
            const pushStat = (s) => {
              const label =
                s.displayName || s.name || s.label || s.statName || "";
              const value =
                s.displayValue != null
                  ? s.displayValue
                  : s.perGameDisplayValue != null
                  ? s.perGameDisplayValue
                  : s.value != null
                  ? s.value
                  : "0";
              const rank = s.rankDisplayValue || null;
            ...
            if (prefList && prefList.length > 0) {
              for (const pname of prefList) {
                ...
                const found = allStats.find(
                  (s) =>
                    (s.name && s.name.toLowerCase() === pname.toLowerCase()) ||
                    (s.abbreviation &&
                      s.abbreviation.toLowerCase() === pname.toLowerCase())
                );
            ...
            if (chosen.length < 6) {
              for (const s of allStats) {
                ...
                const lname = s.name.toLowerCase();
                if (
                  lname.includes("yards") ||
                  lname.includes("att") ||
                  lname.includes("td") ||
                  lname.includes("comp") ||
                  lname.includes("rec") ||
                  lname.includes("int") ||
                  lname.includes("tackle") ||
                  lname.includes("sack") ||
                  lname.includes("points") ||
                  lname.includes("games")
                ) {
                  pushStat(s);
      ...
      console.error("Error in fetchPlayerData (NHL):", error);
      ...
      <View style={[styles.playerHeader, { backgroundColor: theme.surface }]}>
        ...
        <View style={styles.playerInfo}>
          ...
          >
            <Text
              ...
              style={[
                ...
                { color: theme.textSecondary, marginRight: 8 },
              ]}
            >
              #{playerData.jersey || ""}
            </Text>
          ...
          <View style={{ flexDirection: "row", alignItems: "center" }}>
            {playerData.team &&
              (playerData.team.abbreviation || playerData.team.id) && (
  ...
  const renderStatsContent = () => {
    ...
    const renderStatBox = (stat, idx) => (
      <View
        key={`${stat.label}-${idx}`}
        style={[styles.statBoxSmall, { backgroundColor: theme.surface }]}
      >
        <Text
          allowFontScaling={false}
          style={[styles.statBoxValueSmall, { color: theme.text }]}
        >
          {stat.value}
        </Text>
        <Text
          allowFontScaling={false}
          style={[styles.statBoxLabelSmall, { color: theme.textSecondary }]}
        >
          {stat.label}
        </Text>
        {stat.rank ? (
  ...
  const renderSplitsContent = () => {
              ...
              .map((cat, idx) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() => openSplitCategory(cat)}
                  >
                    <View style={styles.careerTileHeader}>
                      <Text
                        allowFontScaling={false}
                        style={[
                          styles.careerSeasonLabel,
                          { color: theme.text },
                        ]}
                      >
                        {capitalizedName}
                      ...
                      <Text
                        allowFontScaling={false}
                        style={[
                          styles.careerTeamsText,
                          { color: theme.textSecondary },
                        ]}
                      >
                        {cat.splits.length} items
                      </Text>
                    </View>
                    {cat.description ? (
                      <Text
          ...
          <View style={styles.statsContent}>
            ...
            >
              <View style={styles.careerTileHeader}>
                ...
                <Text
                  allowFontScaling={false}
                  style={[styles.careerSeasonLabel, { color: theme.text }]}
                >
                ...
                >
                  <View style={styles.careerTileHeader}>
                    <Text
                      ...
                      style={[styles.careerSeasonLabel, { color: theme.text }]}
                    >
                      {it.displayName || it.abbreviation || `Split ${idx + 1}`}
                    </Text>
                    <Text
                      allowFontScaling={false}
                      style={[
                    ...
                    >
                      {Array.isArray(it.stats)
                        ? `${it.stats.length} stats`
                        : ""}
                    </Text>
                  </View>
                </TouchableOpacity>
    ...
    if (selectedSplitDetail) {
      ...
      const labels = Array.isArray(playerSplits.labels)
        ? playerSplits.labels
        : [];
          ...
          <View style={styles.statsContent}>
            ...
            >
              <View style={styles.careerTileHeader}>
                ...
                <Text
                  ...
                  style={[styles.careerSeasonLabel, { color: theme.text }]}
                >
                  {detail.displayName || detail.abbreviation || "Split Detail"}
                </Text>
              </View>
            </View>
            ...
            {stats.length === 0 ? (
              <Text
                ...
                style={[styles.contentText, { color: theme.textSecondary }]}
              >
                No stats available for this split.
              </Text>
            ) : (
              <View style={styles.statsGridRows}>
                {stats.map((statValue, idx) => (
                  ...
                  >
                    ...
                    <Text
                      allowFontScaling={false}
                      style={[
                        styles.statBoxLabelSmall,
                        { color: theme.textSecondary },
                      ]}
                    >
                      {labels[idx] || `Stat ${idx + 1}`}
                    </Text>
                  </View>
                ))}
              </View>
              ...
              >
                ...
                >
                  <View style={styles.mlbTeamLogos}>
                    <Image
                      key={`game-player-team-${game.id}-${
                        playerData?.team?.id || game.team?.id
                      }`}
                      source={
                        getStableTeamLogoUrl(playerData?.team || game.team)
                          ? {
                              uri: getStableTeamLogoUrl(
                                playerData?.team || game.team
                              ),
                            }
                          : require("../../../assets/nhl.png")
                      }
                      style={styles.mlbTeamLogo}
                      defaultSource={getStableTeamFallbackUrl(
                        playerData?.team || game.team
                      )}
                      onError={() =>
                        handleLogoError(
                    ...
                    <Image
                      ...
                      source={
                        getStableTeamLogoUrl(game.opponent)
  ...
  const openGameModal = async (game) => {
    try {
      ...
      // For NHL, use the stats from game log data instead of service call
      if (game && game.id) {
        try {
          const statsArr = findEventStatsByEventId(game.id);
          if (statsArr && Array.isArray(statsArr)) {
            // Format the game log stats to match the expected structure
            const posCat = positionCategoryFromDisplay(
              playerData?.position?.displayName ||
                playerData?.position?.name ||
                ""
            ...
            if (posCat === "G") {
              // Goalie stats: GA, SA, SV%, Saves, Minutes
              formattedStats = {
                splits: {
                  categories: [
                    {
                      name: "goaltending",
                      displayName: "Goaltending",
                      stats: [
                        {
                          name: "goalsAgainst",
                          displayName: "Goals Against",
                          value: statsArr[6] || 0,
                          displayValue: (statsArr[6] || 0).toString(),
                        },
                        {
                          name: "shotsAgainst",
                          displayName: "Shots Against",
                          value: statsArr[8] || 0,
                          displayValue: (statsArr[8] || 0).toString(),
                        },
                        {
                          name: "savePercentage",
                          displayName: "Save %",
                          value: statsArr[10] || "0.000",
                          displayValue: (statsArr[10] || "0.000").toString(),
                        },
                        {
                          name: "saves",
                          displayName: "Saves",
                          value: statsArr[9] || 0,
                          displayValue: (statsArr[9] || 0).toString(),
                        },
                        {
                          name: "timeOnIce",
                          displayName: "Time on Ice",
                          value: statsArr[1] || "00:00",
                          displayValue: (statsArr[1] || "00:00").toString(),
                        },
                      ],
                    },
                  ],
              ...
              // Skater stats: G, A, TOI, Shots, Hits, Blocked Shots, Plus/Minus
              formattedStats = {
                splits: {
                  categories: [
                    {
                      name: "skaters",
                      displayName: "Skater Stats",
                      stats: [
                        {
                          name: "goals",
                          displayName: "Goals",
                          value: statsArr[0] || 0,
                          displayValue: (statsArr[0] || 0).toString(),
                        },
                        {
                          name: "assists",
                          displayName: "Assists",
                          value: statsArr[1] || 0,
                          displayValue: (statsArr[1] || 0).toString(),
                        },
                        {
                          name: "points",
                          displayName: "Points",
                          value: statsArr[2] || 0,
                          displayValue: (statsArr[2] || 0).toString(),
                        },
                        {
                          name: "plusMinus",
                          displayName: "Plus/Minus",
                          value: statsArr[3] || 0,
                          displayValue: (statsArr[3] || 0).toString(),
                        },
                        {
                          name: "penaltyMinutes",
                          displayName: "Penalty Minutes",
                          value: statsArr[4] || 0,
                          displayValue: (statsArr[4] || 0).toString(),
                        },
                        {
                          name: "shootingPct",
                          displayName: "Shooting %",
                          value: statsArr[5] || 0,
                          displayValue: (statsArr[5] || 0).toString(),
                        },
                        {
                          name: "powerPlayGoals",
                        ...
                        },
                        {
                          ...
                          displayName: "Time on Ice",
                          value: statsArr[12] || 0,
                          displayValue: (statsArr[12] || 0).toString(),
                        },
                        {
                          name: "production",
                          displayName: "Production",
                          value: statsArr[13] || 0,
                          displayValue: (statsArr[13] || 0).toString(),
                        },
                      ],
                    },
                  ],
            ...
            setSelectedGameStatsDetails(formattedStats);
          } else {
            setSelectedGameStatsDetails(null);
          }
        } catch (e) {
          console.warn("Unable to format game log stats:", e?.message || e);
          setSelectedGameStatsDetails(null);
        }
      }
    } finally {
          ...
          >
            ...
            >
              <View style={styles.modalSeasonTopRow}>
                ...
                >
                  ...
                  >
                    {g.leagueName || g.leagueShortName || "NHL"}
                  </Text>
                </View>
              </View>
            </View>
            ...
            <View style={styles.modalStatsSection}>
              ...
              {loadingSelectedGameStats ? (
                ...
                details.splits &&
                Array.isArray(details.splits.categories) ? (
                      ...
                      {Array.isArray(cat.stats) && cat.stats.length > 0 ? (
                        (() => {
                          ...
                          return cat.stats.map((s, i) => (
                            <View
                              key={`${s.displayName || s.name}-${i}`}
                              style={[
                                styles.modalStatCard,
                                isSingle && styles.modalStatCardFull,
                                { backgroundColor: theme.surface },
                              ]}
                            >
                              <Text
                                allowFontScaling={false}
                                style={[
                                  styles.modalStatCardValue,
                                  { color: theme.text },
                                ]}
                              >
                                {typeof s.displayValue !== "undefined" &&
                                s.displayValue !== null
                                  ? s.displayValue
                                  : typeof s.display !== "undefined" &&
                                    s.display !== null
                                  ? s.display
                ...
                >
                  No player game statistics available
                </Text>
              )}
            </View>
                ...
                setTimeout(
                  () =>
                    navigation.navigate("GameDetails", {
                      gameId: g.id || g.eventId || g.gameId,
                      sport: "nhl",
                    }),
                  150
                );
              ...
              >
                ...
                >
                  <View style={styles.modalTeamContainer}>
                    <View style={styles.mlbTeamLogos}>
                      <Image
                        key={`modal-game-player-team-${g.id}-${
                          playerData?.team?.id || g.team?.id
                        }`}
                        source={
                          getStableTeamLogoUrl(playerData?.team || g.team)
                            ? {
                                uri: getStableTeamLogoUrl(
                                  playerData?.team || g.team
                                ),
                              }
                            : require("../../../assets/nhl.png")
                        }
                      ...
                      <Image
                        ...
                        source={
                          getStableTeamLogoUrl(g.opponent)
                    ...
                    >
                      {playerData?.team?.abbreviation || "Team"}{" "}
                      {g.atVs === "@" ? "@" : "vs"}{" "}
                      {g.opponent?.abbreviation || "Opponent"}
                    </Text>
                  </View>
                </View>
              </View>
        ...
        <View style={styles.careerContainer}>
          ...
          {careerData.map((item, idx) => {
            ...
            teams = teams.filter((t) => {
              ...
              return (
                teamId &&
                (convertTeamIdToAbbr(teamId) || convertTeamIdToFullName(teamId))
              );
            });
            ...
            const displayLabel =
              lastFirst.length === 1
                ? lastFirst[0].displayName ||
                  convertTeamIdToFullName(lastFirst[0].id) ||
                  lastFirst[0].abbreviation ||
                  convertTeamIdToAbbr(lastFirst[0].id) ||
                  ""
                : lastFirst
                    .map(
                      (t) => t.abbreviation || convertTeamIdToAbbr(t.id) || ""
                    )
                    .filter(Boolean)
                    .join(" / ");
            const logos = lastFirst
              .map((t) => t.abbreviation || t.id)
              .slice(0, 3);
            return (
              <TouchableOpacity
                key={`career-${item.season}-${idx}`}
                style={[styles.careerTile, { backgroundColor: theme.surface }]}
                onPress={() => {
              ...
              >
                <View style={styles.careerTileHeader}>
                  <View style={styles.careerLogosRow}>
                    {logos.map((l, li) => (
                      <Image
                        key={`career-logo-${item.season}-${li}-${l?.id || l}`}
                        source={
                          getStableTeamLogoUrl(l)
                            ? { uri: getStableTeamLogoUrl(l) }
                            : require("../../../assets/nhl.png")
                        }
                        style={styles.careerLogo}
                        defaultSource={getStableTeamFallbackUrl(l)}
                        onError={() => handleLogoError(l, isDarkMode)}
                      />
                    ))}
                  </View>
                  <Text
                  ...
                  >
                    ...
                    {(() => {
                      ...
                      return Array.from({ length: 3 }).map((_, i) => {
                        const s =
                          sourceStats && sourceStats[i] ? sourceStats[i] : null;
                        return (
                          <View
                            key={`statcol-${i}`}
        ...
        >
              ...
              >
                ...
                <View style={styles.modalTeamContainer}>
                  ...
                  >
                    {teams.map((teamObj, index) => {
                      const token =
                        teamObj.abbreviation ||
                        teamObj.id ||
                        convertTeamIdToAbbr(teamObj.id);
                      return (
                        <Image
                          key={`modal-season-team-logo-${selectedSeasonStats?.season}-${index}-${token}`}
                          source={
                            getStableTeamLogoUrl(token)
                              ? { uri: getStableTeamLogoUrl(token) }
                              : require("../../../assets/nhl.png")
                          }
                          style={[
                            styles.modalSeasonTeamLogo,
                            { width: 40, height: 40, marginHorizontal: 4 },
                          ]}
                  ...
                  >
                    {teams.length === 1
                      ? teams[0].displayName ||
                        convertTeamIdToFullName(teams[0].id) ||
          ...
          >
            <View style={{ padding: 8 }}>
              ...
              >
                {item.season} Statistics
              </Text>
              {details &&
              details.splits &&
              Array.isArray(details.splits.categories) ? (
                details.splits.categories.map((cat, cidx) => (
...
export default NHLPlayerPageScreen;
