...
const NHLPlayerPageScreen = ({ route, navigation }) => {
  ...
  const getHeadshotSource = (p) => {
    try {
      ...
      if (id) return { uri: convertToHttps(`https://a.espncdn.com/combiner/i?img=/i/headshots/nhl/players/${id}.png&w=88&h=88`) };
    } catch (e) {
      // fall through
    }
  ...
  const convertTeamIdToAbbr = (teamId) => {
    ...
    const teamMapping = {
      ...
      '1': 'BOS', '13': 'NYR', '15': 'PHI', '16': 'PIT', '20': 'TB', '7': 'CAR',
      '4': 'CHI', '5': 'DET', '27': 'NSH', '19': 'STL', '23': 'WSH',
      '25': 'ANA', '8': 'LA', '18': 'SJ', '29': 'CBJ', '30': 'MIN', '14': 'OTT',
  ...
  const convertTeamIdToFullName = (teamId) => {
    ...
    const idStr = String(teamId);
    const nameMap = {
      '21': 'Toronto Maple Leafs', '10': 'Montreal Canadiens', '3': 'Calgary Flames', '6': 'Edmonton Oilers', '22': 'Vancouver Canucks', '28': 'Winnipeg Jets',
      '1': 'Boston Bruins', '13': 'New York Rangers', '15': 'Philadelphia Flyers', '16': 'Pittsburgh Penguins', '20': 'Tampa Bay Lightning', '7': 'Carolina Hurricanes',
      '4': 'Chicago Blackhawks', '5': 'Detroit Red Wings', '27': 'Nashville Predators', '19': 'St. Louis Blues', '23': 'Washington Capitals',
      '25': 'Anaheim Ducks', '8': 'Los Angeles Kings', '18': 'San Jose Sharks', '29': 'Columbus Blue Jackets', '30': 'Minnesota Wild', '14': 'Ottawa Senators',
      '26': 'Florida Panthers', '2': 'Buffalo Sabres', '11': 'New Jersey Devils', '12': 'New York Islanders', '9': 'Dallas Stars', '17': 'Colorado Avalanche',
      '129764': 'Utah Mammoth', '124292': 'Seattle Kraken', '37': 'Vegas Golden Knights'
    };
  ...
  const positionCategoryFromDisplay = (posDisplay) => {
    ...
    // Goalie positions
    if (p.includes('goalie') || p.includes('goaltender') || p === 'g') return 'G';
    ...
    if (p.includes('wing') || p.includes('left wing') || p.includes('right wing') || p === 'lw' || p === 'rw') return 'F';
    ...
    try {
      ...
      switch (posCat) {
        ...
        case 'G': { // Goalies
          ...
          tileStats = [makeStatEntry(w), makeStatEntry(gaa), makeStatEntry(svp)].filter(Boolean);
          break;
        }
        default: {
        ...
        if (statsResp && statsResp.splits && Array.isArray(statsResp.splits.categories) && statsResp.splits.categories.length > 0) {
          ...
          switch (posCat) {
            ...
            case 'G': { // Goalies
              ...
              const svp = findStatByNames(['savePercentage', 'savePct', 'saves']);
              tileStats = [makeStatEntry(w), makeStatEntry(gaa), makeStatEntry(svp)].filter(Boolean);
              break;
            }
            default: {
      ...
      if (statsData && statsData.splits && Array.isArray(statsData.splits.categories)) {
        try {
          ...
          const preferredByCategory = {
            general: [
              'games', 'gameStarted', 'timeOnIcePerGame', 'wins', 'losses', 'ties'
            ],
            offensive: [
              'goals', 'assists', 'shotsTotal', 'points', 'powerPlayGoals', 'shootingPct'
            ],
            defensive: [
              'goalsAgainst', 'shotsAgainst', 'saves', 'blockedShots', 'hits', 'shutouts'
            ],
            penalties: [
              'penaltyMinutes', 'majorPenalties', 'minorPenalties', 'fightingPenalties', 'misconducts', 'gameMisconducts'
            ],
            ...
            {playerData.team && (playerData.team.abbreviation || playerData.team.id) && (
              <View style={[styles.teamContainer, { marginRight: 8 }]}>
                <Image 
                  ...
                  source={playerTeamLogoUrl ? { uri: playerTeamLogoUrl } : require('../../../assets/nhl.png')}
                ...
                {items.map((it, idx) => {
                  ...
                  return (
                    <TouchableOpacity key={`opp-${idx}`} style={[styles.teamCard, { backgroundColor: theme.surface }]} onPress={() => openSplitDetail(it)}>
                      <Image 
                        source={getTeamLogoUrl({ abbreviation: teamAbbr }, isDarkMode) ? { uri: getTeamLogoUrl({ abbreviation: teamAbbr }, isDarkMode) } : require('../../../assets/nhl.png')} 
              ...
              formattedStats = {
                splits: {
                  categories: [{
                    ...
                    stats: [
                      { name: 'goalsAgainst', displayName: 'Goals Against', value: statsArr[6] || 0, displayValue: (statsArr[6] || 0).toString() },
                      { name: 'shotsAgainst', displayName: 'Shots Against', value: statsArr[8] || 0, displayValue: (statsArr[8] || 0).toString() },
                      { name: 'savePercentage', displayName: 'Save %', value: statsArr[10] || '0.000', displayValue: (statsArr[10] || '0.000').toString() },
                      { name: 'saves', displayName: 'Saves', value: statsArr[9] || 0, displayValue: (statsArr[9] || 0).toString() },
                      { name: 'timeOnIce', displayName: 'Time on Ice', value: statsArr[1] || '00:00', displayValue: (statsArr[1] || '00:00').toString() }
                    ]
                  }]
                }
              };
              ...
              formattedStats = {
                splits: {
                  categories: [{
                    ...
                    stats: [
                      { name: 'goals', displayName: 'Goals', value: statsArr[0] || 0, displayValue: (statsArr[0] || 0).toString() },
                      { name: 'assists', displayName: 'Assists', value: statsArr[1] || 0, displayValue: (statsArr[1] || 0).toString() },
                      { name: 'points', displayName: 'Points', value: statsArr[2] || 0, displayValue: (statsArr[2] || 0).toString() },
                      { name: 'plusMinus', displayName: 'Plus/Minus', value: statsArr[3] || 0, displayValue: (statsArr[3] || 0).toString() },
                      { name: 'penaltyMinutes', displayName: 'Penalty Minutes', value: statsArr[4] || 0, displayValue: (statsArr[4] || 0).toString() },
                      { name: 'shootingPct', displayName: 'Shooting %', value: statsArr[5] || 0, displayValue: (statsArr[5] || 0).toString() },
                      { name: 'powerPlayGoals', displayName: 'Power Play Goals', value: statsArr[6] || 0, displayValue: (statsArr[6] || 0).toString() },
                      { name: 'timeOnIce', displayName: 'Time on Ice', value: statsArr[12] || 0, displayValue: (statsArr[12] || 0).toString() },
                      { name: 'production', displayName: 'Production', value: statsArr[13] || 0, displayValue: (statsArr[13] || 0).toString() }
                    ]
                  }]
                }
              };
        ...
        <View style={[styles.modalContainer, { backgroundColor: theme.background }]}>
          ...
          <ScrollView style={styles.modalContent} showsVerticalScrollIndicator={false}>
            ...
            <TouchableOpacity onPress={() => {
              ...
              setTimeout(() => navigation.navigate('GameDetails', { gameId: g.id || g.eventId || g.gameId, sport: 'nhl' }), 150);
            }}>
                ...
                <View style={[styles.modalSeasonTopRow, { justifyContent: 'center' }]}>
                  <View style={styles.modalTeamContainer}>
                    ...
                    <Text allowFontScaling={false} style={[styles.modalTeamName, { color: theme.text, marginTop: 8 }]}>
                      {playerData?.team?.abbreviation || 'Team'} {g.atVs === '@' ? '@' : 'vs'} {g.opponent?.abbreviation || 'Opponent'}
                    </Text>
                  </View>
                </View>
              </View>
    ...
    return (
      <ScrollView style={[styles.statsContainer, { backgroundColor: theme.background }]} showsVerticalScrollIndicator={false}>
        <View style={styles.careerContainer}>
          <Text allowFontScaling={false} style={[styles.careerSectionTitle, { color: colors.primary }]}>NHL Career</Text>
              ...
              <TouchableOpacity key={`career-${item.season}-${idx}`} style={[styles.careerTile, { backgroundColor: theme.surface }]} onPress={() => { setSelectedSeasonStats(item); setShowSeasonModal(true); }}>
                <View style={styles.careerTileHeader}>
                  <View style={styles.careerLogosRow}>
                    {logos.map((l, li) => (
                      <Image key={`career-logo-${item.season}-${li}-${l?.id || l}`} source={getStableTeamLogoUrl(l) ? { uri: getStableTeamLogoUrl(l) } : require('../../../assets/nhl.png')} style={styles.careerLogo} defaultSource={getStableTeamFallbackUrl(l)} onError={() => handleLogoError(l, isDarkMode)} />
                    ))}
                  </View>
            ...
            return (
              <View style={[styles.modalSeasonHeader, { backgroundColor: theme.surface, marginHorizontal: 15, marginTop: 10 }]}>
                <View style={styles.modalSeasonInfo}>
                  ...
                  <Text allowFontScaling={false} style={[styles.modalLeagueName, { color: theme.textSecondary }]}>NHL</Text>
                </View>
                <View style={styles.modalTeamContainer}>
                    ...
                    {teams.map((teamObj, index) => {
                      ...
                      return (
                        <Image 
                          ...
                          source={getStableTeamLogoUrl(token) ? { uri: getStableTeamLogoUrl(token) } : require('../../../assets/nhl.png')} 
