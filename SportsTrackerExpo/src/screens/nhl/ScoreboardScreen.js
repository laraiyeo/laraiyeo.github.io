...
const NHLScoreboardScreen = ({ navigation }) => {
    ...
    try {
      ...
      if (!data || !data.events || data.events.length === 0) {
        ...
        const grouped = groupGamesByDate(
          data.events
            .map((e) => NHLService.formatGameForMobile(e))
            .filter(Boolean)
        );
        processed = [];
        Object.keys(grouped)
      ...
      if (!silent) {
        Alert.alert("Error", "Failed to load NHL games");
        console.error(e);
      }
    } finally {
      if (!silent) {
  ...
  const isLiveGame = (item) => {
    ...
    return (
      !item.isCompleted && (isEndOf || hasMeaningfulClock(item.displayClock))
    );
  };
  ...
  const renderDateHeader = (date) => (
    ...
    >
      <Text
        ...
        style={[styles.dateHeaderText, { color: "#fff" }]}
      >
        {date}
      </Text>
    </View>
  ...
  const renderGameCard = ({ item }) => {
    ...
    const periodText =
      item.situation?.period ||
      item.situation?.periodName ||
      item.period ||
      item.periodName ||
      item.statusDetail ||
      null;
      ...
      >
        ...
        <View style={styles.teamsContainer}>
          ...
          <View style={styles.teamRow}>
            <TouchableOpacity
              onPress={() => navigateToTeam(item.awayTeam)}
              ...
              <Image
                source={{
                  uri:
                    getTeamLogoUrl(
                      "nhl",
                      normalizeAbbreviation(item.awayTeam.abbreviation)
                    ) || item.awayTeam.logo,
                }}
                style={[
                  styles.teamLogo,
                  isTeamLosing(item, true) ? styles.losingTeamLogo : null,
                ]}
              />
            </TouchableOpacity>
              ...
              <View style={styles.teamNameRow}>
                ...
                <Text
                  ...
                  style={[
                    ...
                    {
                      color: isFavorite(getNHLTeamId(item.awayTeam), "nhl")
                        ? colors.primary
            ...
            >
              {isLive || item.isCompleted ? item.awayTeam.score ?? "" : ""}
            </Text>
          </View>
          ...
          <View style={styles.teamRow}>
            <TouchableOpacity
              onPress={() => navigateToTeam(item.homeTeam)}
              style={styles.teamLogoContainer}
            >
              <Image
                source={{
                  uri:
                    getTeamLogoUrl(
                      "nhl",
                      normalizeAbbreviation(item.homeTeam.abbreviation)
                    ) || item.homeTeam.logo,
                }}
                style={[
                  styles.teamLogo,
                  isTeamLosing(item, false) ? styles.losingTeamLogo : null,
                ]}
              />
            </TouchableOpacity>
            <View style={styles.teamInfo}>
              <View style={styles.teamNameRow}>
                ...
                <Text
                  ...
                  style={[
                    ...
                    {
                      color: isFavorite(getNHLTeamId(item.homeTeam), "nhl")
                        ? colors.primary
            ...
            >
              {isLive || item.isCompleted ? item.homeTeam.score ?? "" : ""}
            </Text>
          </View>
        </View>
        ...
        <View style={[styles.gameFooter, { borderTopColor: theme.border }]}>
          <View style={styles.gameFooterLeft}>
            ...
            {periodText && isLive && (
              ...
              >
                {getOrdinalSuffix(periodText)} {periodText < 4 ? "Period" : ""}
              </Text>
            )}
          </View>
