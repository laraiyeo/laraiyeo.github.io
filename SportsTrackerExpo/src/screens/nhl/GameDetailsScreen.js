...
const getSmartTeamColors = (awayTeam, homeTeam) => {
  ...
  const awayAlternate = awayTeam.alternateColor
    ? `#${awayTeam.alternateColor}`
    : awayPrimary;
  const homeAlternate = homeTeam.alternateColor
  ...
  console.log(
    ...
    } (${awayPrimary}, alt: ${awayAlternate}) vs ${
      homeTeam.displayName
    } (${homePrimary}, alt: ${homeAlternate}), similarity: ${similarity.toFixed(
      3,
    )}`,
  );
  ...
  if (similarity > conflictThreshold) {
    console.log(
      `Color conflict detected: Using away alternate (${awayAlternate}) vs home primary (${homePrimary})`,
    );
    return {
  ...
  console.log(
    `No color conflict detected: Using away primary (${awayPrimary}) vs home primary (${homePrimary})`,
  );
  return {
...
const TeamLogo = ({
  ...
  const resolvedUri = logoUri || getTeamLogoUrl("nhl", teamAbbreviation);
...
const NHLGameDetailsScreen = ({ route }) => {
    ...
    try {
      ...
      const matches = allMatches.filter((match) => {
        ...
        return (
          ...
          matchSport === "nhl" ||
          (match.title &&
            (match.title.toLowerCase().includes("nhl") ||
              match.title.toLowerCase().includes("hockey")))
        );
      });
  ...
  const findNHLMatchStreams = async (homeTeamName, awayTeamName) => {
    try {
      console.log(
        `Finding NHL streams for: ${awayTeamName} vs ${homeTeamName}`,
      );
      ...
      console.log(
        `Normalized NHL team names: {homeNormalized: '${homeNormalized}', awayNormalized: '${awayNormalized}'}`,
      );
      ...
      if (!bestMatch || bestScore < 0.25) {
        console.log(
          `No good matching NHL live match found (best score: ${bestScore})`,
        );
        return {};
      }

      console.log(
        `Found matching NHL match: ${
      ...
      for (const source of bestMatch.sources) {
        try {
          ...
          if (sourceStreams && sourceStreams.length > 0) {
            ...
            console.log(
              `Added NHL stream for ${source.source}:`,
              allStreams[sourceKey],
            );
          }
        } catch (error) {
          console.error(
            `Error fetching NHL streams for ${source.source}:`,
            error,
          );
        }
      }
  ...
  const openStreamModal = async () => {
    try {
      ...
      if (streamKeys.length > 0) {
        ...
        initialUrl = generateNHLStreamUrl(
          awayName,
          homeName,
          initialStreamType,
        );
  ...
  useEffect(() => {
    if (!streamModalVisible && gameId && details) {
      ...
      if (wasModalOpen) {
        ...
        NHLService.getGameDetails(gameId)
          .then(setDetails)
          .catch((e) =>
            console.error(
              "Failed to fetch NHL game details after stream modal close",
              e,
            ),
          );
      }
  ...
  useEffect(() => {
    ...
    const processPlays = async () => {
      try {
        ...
        if (
          competitionObjLocal &&
          Array.isArray(competitionObjLocal.competitors)
        ) {
          const comps = competitionObjLocal.competitors;
          awayLocal =
            comps.find((c) => c.homeAway === "away") || comps[0] || null;
          homeLocal =
            comps.find((c) => c.homeAway === "home") || comps[1] || null;
        } else {
          const teams = details?.boxscore?.teams
            ? Object.values(details.boxscore.teams)
            : [];
          awayLocal =
            teams.find((t) => t.team?.homeAway === "away") || teams[0] || null;
          homeLocal =
            teams.find((t) => t.team?.homeAway === "home") || teams[1] || null;
        }

        const awayScoreNumLocal =
          parseInt(awayLocal?.score ?? awayLocal?.team?.score ?? "0", 10) || 0;
        const homeScoreNumLocal =
          parseInt(homeLocal?.score ?? homeLocal?.team?.score ?? "0", 10) || 0;
        ...
        const getLogoUriForTeam = (teamObj, preferDark = false) => {
          ...
          if (preferDark) {
            ...
            if (Array.isArray(teamObj?.team?.logos)) {
              const dark = teamObj.team.logos.find((l) => {
                ...
                else if (Array.isArray(l?.rel))
                  relVal = l.rel.join(" ").toLowerCase();
                let nameVal = "";
          ...
          if (teamObj?.team?.logo || teamObj?.logo)
            return teamObj.team?.logo || teamObj.logo;
        ...
        for (let i = 0; i < playsArray.length; i += chunkSize) {
          ...
          const processedChunk = chunk.map((play) => {
            ...
            const awayScore =
              typeof play?.awayScore !== "undefined"
                ? play.awayScore
                : awayScoreNumLocal;
            const homeScore =
              typeof play?.homeScore !== "undefined"
                ? play.homeScore
                : homeScoreNumLocal;
            const playTeamId = getTeamIdFromPlayTeam(play.team);
            let playTeamSummary = null;
            if (playTeamId && competitionObjLocal?.competitors) {
              playTeamSummary = competitionObjLocal.competitors.find(
                (c) =>
                  String(c.team?.id || c.id) === String(playTeamId) ||
                  String(c.id) === String(playTeamId),
              );
            }
            ...
            if (playTeamSummary?.team) {
              ...
              if (playTeamAbbrev === awayTeamAbbrev) {
                ...
                const playTeamColorRaw =
                  playTeamSummary?.team?.color ||
                  playTeamSummary?.color ||
                  null;
                playTeamColor = playTeamColorRaw
                  ? playTeamColorRaw.startsWith("#")
                    ? playTeamColorRaw
                    : `#${playTeamColorRaw}`
                  : null;
              }
            ...
            const coordX =
              typeof coordXRaw === "number"
                ? coordXRaw
                : coordXRaw
                  ? parseFloat(coordXRaw)
                  : null;
            const coordY =
              typeof coordYRaw === "number"
                ? coordYRaw
                : coordYRaw
                  ? parseFloat(coordYRaw)
                  : null;
            const textColor = theme.text;
            const borderLeftWidth = playTeamColor ? 6 : 0;
            const borderLeftColor = playTeamColor || "transparent";
            const awayLogoUri =
              getLogoUriForTeam(awayLocal, isScoring) ||
              getTeamLogoUrl(
                "nhl",
                awayLocal?.team?.abbreviation || awayLocal?.abbreviation,
              );
            const homeLogoUri =
              getLogoUriForTeam(homeLocal, isScoring) ||
              getTeamLogoUrl(
                "nhl",
                homeLocal?.team?.abbreviation || homeLocal?.abbreviation,
              );
        ...
        try {
          ...
          const getShortNameFromParticipant = (part) => {
            ...
            const candidate =
              part.player || part.athlete || part.person || part;
            return (
              (candidate &&
                (candidate.shortName ||
          ...
          const findScorerName = (play) => {
            ...
            let scorer = parts.find((p) => {
              const role = (p?.type || p?.role || p?.participantType || "")
                .toString()
                .toLowerCase();
          ...
          for (const rawPlay of playsArray) {
            ...
            const periodNum = getPeriodNumber(
              rawPlay?.period ||
                rawPlay?.periodNumber ||
                rawPlay?.period?.number,
            );
            ...
            const teamId = (function () {
              ...
              if (!t) return null;
              if (typeof t === "string")
                return (t.match(/(\d+)/) || [null, null])[1];
              if (typeof t === "number") return String(t);
              if (t.id) return String(t.id);
              if (t.$ref)
                return (t.$ref.match(/teams\/(\d+)/) || [null, null])[1];
              return null;
            })();

            const shortName =
              findScorerName(rawPlay) ||
              rawPlay?.scorer ||
              rawPlay?.scorers?.[0]?.athlete?.shortName ||
              rawPlay?.scorers?.[0]?.player?.shortName ||
              null;
            if (!shortName) continue;

            const periodDisplay =
              rawPlay?.period?.displayValue ||
              (rawPlay?.period ? String(rawPlay.period) : "");
            const clockDisplay =
              rawPlay?.clock?.displayValue ||
              rawPlay?.clock ||
              rawPlay?.time ||
              "";
          ...
          const awayList = Object.keys(foundAway).map((name) => {
            const stamps = Array.isArray(foundAway[name])
              ? sortTimestamps(foundAway[name].slice())
              : [];
            return `${name} ${stamps.join(", ")}`;
          });
          const homeList = Object.keys(foundHome).map((name) => {
            const stamps = Array.isArray(foundHome[name])
              ? sortTimestamps(foundHome[name].slice())
              : [];
            return `${name} ${stamps.join(", ")}`;
          });
  ...
  const renderFaceoffCircle = () => {
    ...
    const awayFaceoffPct = parseFloat(
      awayTeam?.statistics?.find((s) => s.name === "faceoffPercent")
        ?.displayValue || "0",
    );
    const homeFaceoffPct = parseFloat(
      homeTeam?.statistics?.find((s) => s.name === "faceoffPercent")
        ?.displayValue || "0",
    );
    ...
    return (
      <View style={styles.faceoffContainer}>
        <Text style={[styles.faceoffTitle, { color: theme.text }]}>
          Faceoff Win %
        </Text>
        <View style={styles.faceoffSection}>
            ...
            <View style={styles.faceoffCircle}>
              <Svg width={120} height={120} style={styles.faceoffSvg}>
                ...
                <Circle
                  ...
                  fill="transparent"
                  strokeDasharray={`${
                    (awayFaceoffPct / 100) * 314.159
                  } 314.159`}
                  strokeDashoffset="0"
                  transform={`rotate(${
                    (homeFaceoffPct / 100) * 360 - 90
                  } 60 60)`}
                />
              </Svg>
          ...
          <View style={styles.faceoffValues}>
            <View style={styles.faceoffTeam}>
              <View
                style={[styles.faceoffColor, { backgroundColor: awayColor }]}
              />
            ...
            <View style={styles.faceoffTeam}>
              ...
              <View
                style={[styles.faceoffColor, { backgroundColor: homeColor }]}
              />
            </View>
          </View>
        </View>
          ...
          <View style={[styles.statsBar, { backgroundColor: theme.border }]}>
            <View
              style={[
                ...
                {
                  width: `${awayPercent}%`,
                  backgroundColor: ensureHexColor(awayColor),
                },
              ]}
            />
            <View
              style={[
                ...
                {
                  width: `${homePercent}%`,
                  backgroundColor: ensureHexColor(homeColor),
                },
              ]}
            />
          </View>
            ...
            <Image
              source={{
                uri:
                  ...
                  getTeamLogoUrl("nhl", awayTeam?.team?.abbreviation),
              }}
            ...
            <Image
              source={{
                uri:
                  ...
                  getTeamLogoUrl("nhl", homeTeam?.team?.abbreviation),
              }}
          ...
          <View key={teamIdx} style={styles.teamLeadersContainer}>
            <View style={styles.teamLeadersHeader}>
              <Image
                source={{
                  uri:
                    teamLeaders.team?.logos?.[isDarkMode ? 1 : 0].href ||
                    getTeamLogoUrl("nhl", teamLeaders.team?.abbreviation),
                }}
  ...
  const renderRosterSection = (teamType) => {
    ...
    const competitionForStatus =
      details?.header?.competitions?.[0] ||
      details?.boxscore?.game ||
      details?.game ||
      null;
    const statusTypeForRoster =
      competitionForStatus?.status?.type || details?.game?.status?.type || {};
    ...
    const isScheduledForRoster = !!(
      ...
      String(statusTypeForRoster?.description || "")
        .toLowerCase()
        .includes("scheduled") ||
      gameStatus.isPre
    );
    ...
    if (isScheduledForRoster) {
      ...
      let goalies = [];
      if (details.goalies) {
        const goalieTeamData =
          teamType === "home"
            ? details.goalies.homeTeam
            : details.goalies.awayTeam;
        // Check that the team ID matches to ensure we get the right goalies
        if (
          goalieTeamData &&
          String(goalieTeamData.teamId) === String(team.team.id)
        ) {
      ...
      if (goalies.length === 0) {
        ...
        const teamBoxForInj =
          playersBoxAll.find(
            (pb) =>
              String(pb.team?.id) === String(team.team.id) ||
              pb.team?.abbreviation === team.team.abbreviation,
          ) || null;
        const groups = teamBoxForInj?.statistics || [];
        const goaliesGroup = groups.find((g) =>
          (g.name || "").toLowerCase().includes("goalies"),
        );
        goalies = (goaliesGroup?.athletes || []).map((a) => ({
          ...a,
          position: "goalies",
        }));
      }
      ...
      goalies.forEach((goalie) => {
        console.log(
          `Team Name: ${team.team.displayName} | Name: ${
            goalie.athlete?.displayName || "Unknown"
          }`,
        );
      });
            ...
            <Image
              source={{
                uri:
                  ...
                  getTeamLogoUrl("nhl", team.team?.abbreviation),
              }}
          ...
          >
            <View style={styles.modernSectionHeader}>
              ...
              <Text style={[styles.modernSectionTitle, { color: theme.text }]}>
                Projected Goalies ({goalies.length})
              </Text>
            </View>
            {goalies.length === 0 ? (
              <Text
                style={[
                  styles.contentText,
                  { color: theme.textSecondary, padding: 12 },
                ]}
              >
                No goalies listed
              </Text>
              ...
              <>
                ...
                {goalies.map((g, idx) =>
                  renderPlayerRow(g, idx, "proj-goalie", [0, 4, 2]),
                )}
              </>
            )}
          </View>
              ...
              <View style={{ padding: 12 }}>
                {injuriesList.map((inj, ii) => (
                  ...
                  >
                    ...
                    >
                      <Text
                        style={[styles.injuryPlayerName, { color: theme.text }]}
                      >
                  ...
                  >
                    ...
                    >
                      <View style={styles.miniGameHeader}>
                        <Text
                          style={[
                            styles.miniGameResult,
                            {
                              color:
                                game.gameResult === "W" ? "#4CAF50" : "#F44336",
                            },
                          ]}
                        >
                          {game.gameResult || "L"}
                        ...
                        <Text
                          style={[styles.miniGameScore, { color: theme.text }]}
                        >
                          {game.score ||
    ...
    const hasOnIceData =
      !isGameFinished &&
      details?.onIce &&
      Array.isArray(details.onIce) &&
      details.onIce.length > 0;

    console.log(
      `Has onIce data: ${hasOnIceData}, onIce length: ${
        details?.onIce?.length || 0
      }`,
    );

    // Get players on ice for this team (from onIce array) if available and game not finished
    const onIceData = hasOnIceData
      ? details.onIce.find((ice) => ice.teamId === team.team.id)
      : null;
      ...
      if (positionGroup.athletes) {
        positionGroup.athletes.forEach((athlete) => {
          ...
          allPlayers.push({
            ...
            isOnIce: hasOnIceData
              ? onIcePlayers.some(
                  (onIce) =>
                    String(onIce.athleteid) === String(athlete.athlete?.id),
                )
              : false,
          });
        });
        ...
        <View style={styles.rosterContainer}>
          ...
          >
            ...
            {skaters.map((player, idx) =>
              renderPlayerRow(player, idx, "all-skater", [9, 11, 4]),
            )}

            {skaters.length > 0 && goalies.length > 0 && renderDivider()}

            {goalies.length > 0 &&
              renderStatHeaders(["GA", "SA", "SV%"], false)}
            {goalies.map((player, idx) =>
              renderPlayerRow(player, idx, "all-goalie", [0, 1, 5]),
            )}
          </View>
        </View>
      );
    ...
    const skatersOnBench = playersOnBench.filter(
      (p) => p.position !== "goalies",
    );
    const goaliesOnBench = playersOnBench.filter(
      (p) => p.position === "goalies",
    );
      ...
      return (
        <View style={styles.statHeaderRow}>
          <View style={styles.playerInfo}>
            <Text
              style={[styles.statHeaderText, { color: theme.textSecondary }]}
            >
              {isSkater ? "Player" : "Goalie"}
            </Text>
          </View>
    ...
    function renderPlayerRow(player, idx, keyPrefix, statIndices = [9, 11, 4]) {
      ...
      const position =
        player.position === "goalies"
          ? "G"
          : player.position === "forwards"
            ? "F"
            : player.position === "defenses"
              ? "D"
              : "";
      ...
      function statToString(s) {
        ...
        if (typeof s === "object")
          return s.displayValue ?? s.value ?? JSON.stringify(s);
        return String(s);
      }
          ...
          >
            <View style={styles.playerInfo}>
              <View style={styles.playerNameSection}>
                <Text
                  style={[styles.modernPlayerName, { color: theme.text }]}
                  numberOfLines={1}
                >
                  {displayName}
                </Text>
      ...
      <View style={styles.rosterContainer}>
        ...
        {totalOnIce > 0 && (
          ...
          >
            ...
            {skatersOnIce.map((player, idx) =>
              renderPlayerRow(player, idx, "onice-skater", [9, 11, 4]),
            )}

            {skatersOnIce.length > 0 &&
              goaliesOnIce.length > 0 &&
              renderDivider()}

            {/* Goalies use different stat indices and header */}
            {goaliesOnIce.length > 0 && renderStatHeaders(["GA", "SA", "SV%"])}
            {goaliesOnIce.map((player, idx) =>
              renderPlayerRow(player, idx, "onice-goalie", [0, 1, 5]),
            )}
          </View>
        )}
        ...
        {totalOnBench > 0 && (
          ...
          >
            ...
            {skatersOnBench.map((player, idx) =>
              renderPlayerRow(player, idx, "bench-skater", [9, 11, 4]),
            )}

            {skatersOnBench.length > 0 &&
              goaliesOnBench.length > 0 &&
              renderDivider()}

            {goaliesOnBench.length > 0 &&
              renderStatHeaders(["GA", "SA", "SV%"])}
            {goaliesOnBench.map((player, idx) =>
              renderPlayerRow(player, idx, "bench-goalie", [0, 1, 5]),
            )}
          </View>
        )}
      </View>
          ...
          <Image
            source={{
              uri:
                away?.team?.logo ||
                away?.logo ||
                getTeamLogoUrl("nhl", away?.team?.abbreviation),
            }}
          ...
          <Image
            source={{
              uri:
                home?.team?.logo ||
                home?.logo ||
                getTeamLogoUrl("nhl", home?.team?.abbreviation),
            }}
  ...
  const getNHLTeamId = (team) => {
    ...
    console.log(
      `[NHL GameDetails] getNHLTeamId: team=${JSON.stringify(
        team?.team?.abbreviation || team?.abbreviation,
      )}, id=${teamId}`,
    );
    return teamId;
  };
  ...
  const handleFavoriteToggle = async (team) => {
    ...
    if (!teamId) {
      console.warn(
        "NHL GameDetails handleFavoriteToggle: Invalid team ID",
        team,
      );
      return;
    }
          ...
          <View style={styles.soccerMainRow}>
            ...
            <View style={styles.soccerTeamSection}>
              ...
              >
                {away?.team?.logo || away?.logo ? (
                  <Image
                    source={{ uri: away.team?.logo || away.logo }}
                    style={[
            ...
            <View style={styles.soccerScoreSection}>
              <View style={styles.soccerScoreRow}>
                ...
                >
                  <Text style={styles.soccerStatusText}>
                    {getGameStatus().text}
                  </Text>
                  <Text
                    style={[
                      styles.soccerFullTimeText,
            ...
            <View style={styles.soccerTeamSection}>
              ...
              >
                {home?.team?.logo || home?.logo ? (
                  <Image
                    source={{ uri: home.team?.logo || home.logo }}
                    style={[
                      styles.soccerTeamLogo,
            ...
            >
              {selectedPlayer &&
                (() => {
                  ...
                  const isProjectedGoalie =
                    player?.meta && player.meta.groupName === "goalies";
                  ...
                  if (isProjectedGoalie) {
                    ...
                    const teams = details.boxscore.teams;
                    if (
                      details.goalies?.homeTeam?.athletes?.some(
                        (a) => String(a.id) === String(athlete?.id),
                      )
                    ) {
                      team = teams.find((t) => t.homeAway === "home")?.team;
                    } else if (
                      details.goalies?.awayTeam?.athletes?.some(
                        (a) => String(a.id) === String(athlete?.id),
                      )
                    ) {
                    ...
                    if (foundPlayer && foundPlayer.team) {
                      teamLogo =
                        ...
                        (foundPlayer.team.abbreviation
                          ? getTeamLogoUrl("nhl", foundPlayer.team.abbreviation)
                          : null);
