...
const StatsScreen = ({ route }) => {
  ...
  const fetchPlayerStats = async () => {
    try {
      const response = await fetch(`https://sports.core.api.espn.com/v2/sports/hockey/leagues/nhl/seasons/${YearFallbackUtils.getPreferredYear()}/types/2/leaders?limit=10`);
  ...
  const fetchTeamStats = async () => {
    try {
      const response = await fetch('https://site.web.api.espn.com/apis/site/v3/sports/hockey/nhl/teamleaders');
      const data = await response.json();
  ...
  const getNHLTeamAbbreviation = (team) => {
    ...
    return team?.displayName?.substring(0, 3)?.toUpperCase() || 'NHL';
  };
      ...
      return (
        <View key={index} style={[styles.firstLeaderRow, { borderBottomColor: theme.border }]}>
          {selectedType === 'ATHLETES' ? (
            ...
            <Image
              source={{ uri: getTeamLogoUrl('nhl', teamAbbr) }}
