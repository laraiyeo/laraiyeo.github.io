...
const BetLeadersScreen = () => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const { data: rows, error } = await supabase
          .from("leaderboard")
          .select("user_id,multiplier,total_bets,username,first_bet,credits_start,credits_current,updated_at")
          ...
          if (uid) {
            ...
            if (!found) {
              ...
              const { data: myRow, error: myErr } = await supabase
                .from("leaderboard")
                .select("user_id,multiplier,total_bets,username,first_bet,credits_start,credits_current")
                .eq("user_id", uid)
                .maybeSingle();
  ...
  function scrollToUserAndFlash() {
    ...
    const targetIndex = idx >= 0 ? idx : data.findIndex((d) => d.rank && d.rank === 1);
  ...
  const renderItem = ({ item, index }) => {
    ...
    const background = flashIndex === index ? flashAnim.interpolate({
      inputRange: [0, 1],
      outputRange: [theme.background, flashColor]
    }) : theme.background;

    return (
      <Animated.View style={[styles.itemContainer, { backgroundColor: background, borderColor: theme.border || '#eee' }]}>
        <View style={styles.rankBadgeContainer} pointerEvents="none">
          <View style={styles.rankBadge}>
            <Text style={styles.rankBadgeText}><Text style={ styles.superscript }>#</Text>{item.rank}</Text>
          </View>
        </View>

        <View style={styles.leftRow}>
          {showAvatar ? (
            <View style={[styles.avatarWrap, isTopThree ? { borderColor: rankColors[item.rank - 1] || 'transparent' } : {}]}>
              <View style={[styles.avatar, isTopThree ? { backgroundColor: (item.rank===1? '#e6c200': item.rank===2? '#9ea0a0' : '#a8651f') } : { backgroundColor: colors.primary || '#666' }]}>
                <Text style={styles.avatarText}>{initialsFor(item.username)}</Text>
              </View>
              {isTopThree && (
                <View style={[styles.trophyWrap, { backgroundColor: theme.surfaceSecondary, borderColor: rankColors[item.rank - 1] || '#000' || '#eee', borderWidth: 1 }]}> 
                  <Ionicons name="trophy" size={15} color={rankColors[item.rank - 1] || '#000'} />
                </View>
              )}
            </View>
          ) : (
          ...
          <View style={styles.nameBlock}>
            <Text style={[styles.usernameText, isYou ? { fontWeight: '800', color: theme.text } : {fontWeight: '400', color: theme.text}]}>{item.username}</Text>
            <Text style={[styles.subText, { color: theme.textSecondary }]}>First Bet: {formatDateShort(item.first_bet_date)}</Text>
          </View>
        </View>

        <View style={styles.rightRow}>
          <Text style={[styles.multiplierText, { color: theme.text }]}>{item.multiplier.toFixed ? item.multiplier.toFixed(2) : String(item.multiplier)}</Text>
          <Text style={[styles.smallText, { color: theme.textSecondary }]}>{item.total_bets} bets</Text>
        </View>
      </Animated.View>
    );
  };
  ...
  function formatWeekStartLabel(weekStart) {
    const weekday = weekStart.toLocaleDateString(undefined, { weekday: 'short' });
    const rest = weekStart.toLocaleDateString(undefined, { month: 'short', day: 'numeric' });
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}> 
      <Text style={[styles.headerTitle, { color: theme.text }]}>Weekly Leaderboard</Text>
      <FlatList
        ref={flatRef}
        data={data}
        keyExtractor={(i) => i.id}
      ...
      <View style={[styles.footer, { borderTopColor: theme.border, backgroundColor: theme.background || '#eee' }]}> 
        <Text style={[styles.footerText, { color: theme.textSecondary }]}>Week start: {formatWeekStartLabel(weekStart)}</Text>
        <TouchableOpacity style={[styles.showButton, { backgroundColor: theme.primary || '#c62828' }]} onPress={scrollToUserAndFlash}>
...
const styles = StyleSheet.create({
  ...
  avatar: { width: 56, height: 56, borderRadius: 12, alignItems: 'center', justifyContent: 'center' },
  avatarText: { color: '#fff', fontWeight: '800', fontSize: 40 },
  trophyWrap: { position: 'absolute', right: -6, bottom: -6, width: 28, height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center', elevation: 2 },
  ...
  footer: { position: 'absolute', left: 0, right: 0, bottom: 0, height: 72, paddingHorizontal: 12, paddingVertical: 12, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', borderTopWidth: 1, backgroundColor: 'transparent' },
  footerText: { fontSize: 14 },
  showButton: { paddingHorizontal: 16, paddingVertical: 10, borderRadius: 8 },
  showButtonText: { color: '#fff', fontWeight: '800' },
  rankBadgeContainer: { position: 'absolute', right: -10, top: -20, zIndex: 10 },
  rankBadge: { backgroundColor: '#222', width: 34, height: 34, borderRadius: 18, alignItems: 'center', justifyContent: 'center' },
