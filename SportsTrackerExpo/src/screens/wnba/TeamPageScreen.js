...
const TeamPageScreen = ({ route, navigation }) => {
  ...
  const statsConfiguration = {
    ...
    'Offensive': { 
      limit: 6, 
      statNames: ['avgPoints', 'avgAssists', 'scoringEfficiency', 'assists', 'fieldGoalPct', 'turnovers'] 
    },
    'Defensive': { 
      limit: 6, 
      statNames: ['avgBlocks', 'avgDefensiveRebounds', 'avgSteals', 'blocks', 'defensiveRebounds', 'steals'] 
    },
    'General': { 
      limit: 6, 
      statNames: ['NBARating', 'avgFouls', 'assistTurnoverRatio', 'gamesPlayed', 'totalRebounds', 'plusMinus']
    }
  ...
  const getTeamLogoUrls = (teamParam, isDarkMode) => {
    ...
    const primaryUrl = isDarkMode
      ? `https://a.espncdn.com/combiner/i?img=/i/teamlogos/wnba/500-dark/${normalized}.png&w=200&h=200`
      : `https://a.espncdn.com/combiner/i?img=/i/teamlogos/wnba/500/${normalized}.png&w=200&h=200`;
  ...
  const renderMatchCard = (game) => {
    ...
    const header = game.competitions[0]?.notes[0]?.headline || `WNBA - ${game.seasonType.name}` || 'WNBA';
  ...
  const renderRosterTab = () => {
    ...
    const guards = roster.filter(player => {
      ...
      return pos.includes('PG') || pos.includes('SG') || pos.includes('G') || pos.includes('GUARD');
    });
    const forwards = roster.filter(player => {
      const pos = (player.position || '').toString().toUpperCase();
      return pos.includes('SF') || pos.includes('PF') || pos.includes('F') || pos.includes('FORWARD');
    });
    ...
    const others = roster.filter(player => {
      const pos = (player.position || '').toString().toUpperCase();
      return !guards.includes(player) && !forwards.includes(player) && !centers.includes(player);
    });
