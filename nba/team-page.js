...
async function showPlayerDetails(playerId, firstName, lastName, jerseyNumber, position, headshotUrl) {
  try {
    ...
    closeButton.addEventListener('mouseenter', () => {
      closeButton.style.backgroundColor = currentTeam && currentTeam.color ? `#${currentTeam.color}` : '#f0f0f0';
    ...
    sliderBackground.style.cssText = `
      ...
      width: 51.75%;
      height: 100%;
      background-color: ${teamColor}; /* Dynamic team color */
      border-radius: 25px;
    ...
    const result = await fetchAthleteStats('basketball', 'nba', seasonYear, playerId);
...
async function showOverallStats() {
  ...
  const seasonYear = await getValidSeasonYear('basketball', 'nba', selectedPlayer.id);
  const result = await fetchAthleteStats('basketball', 'nba', seasonYear, selectedPlayer.id);
...
async function loadGameLogForDate(date) {
  ...
  try {
    resultsContainer.innerHTML = '<div style="text-align: center; padding: 20px; color: #777;"><div style="display: inline-block; width: 20px; height: 20px; border: 3px solid #f3f3f3; border-top: 3px solid #17408B; border-radius: 50%; animation: spin 1s linear infinite;"></div><br>Loading game data...</div>';
...
// Helper function to get valid season year
async function getValidSeasonYear(sport = 'basketball', league = 'nba', athleteId = null) {
  const currentDate = new Date();
  const currentYear = currentDate.getFullYear();
  const currentMonth = currentDate.getMonth() + 1; // 0-based, so add 1
  
  // For NBA, the season runs from October to June
  // If it's July-September, we're in the off-season, so use the previous season
  if (currentMonth >= 7 && currentMonth <= 9) {
    return currentYear; // Off-season, use current year as season identifier
  } else if (currentMonth >= 10) {
    return currentYear + 1; // New season starting, use next year
  } else {
    return currentYear; // Mid-season, use current year
  }
}
...
async function showPlayerComparison(player1, player2) {
  try {
    ...
    const [player1Result, player2Result] = await Promise.all([
      fetchAthleteStats('basketball', 'nba', seasonYear1, player1.id),
      fetchAthleteStats('basketball', 'nba', seasonYear2, player2.id)
    ]);
